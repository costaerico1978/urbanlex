...
def buscador_anexos():
    return render_template('buscador_anexos.html',
        ...
        active_group='buscador', **tmpl_ctx())
...
def pg_buscador():
    ...
    return render_template('buscador.html', active_group='buscador', active_page=page_map.get(request.path, 'buscador-painel'), **tmpl_ctx())
...
def api_buscador_especifica_estado():
    """Retorna estado atual da busca especifica: job em andamento OU ultima concluida.
    
    Permite que a tela reconecte/restaure mesmo depois de fechar a aba ou trocar de app.
    """
    try:
        # 1. Ha job especifica ATIVO em memoria?
        _job_ativo = None
        for _jid, _j in _buscador_jobs.items():
            if _j.get('tipo') == 'especifica' and not _j.get('done') and not _j.get('cancelled'):
                _job_ativo = (_jid, _j)
                break
        
        if _job_ativo:
            _jid, _j = _job_ativo
            # Pegar metadados do banco
            conn = get_db()
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cur.execute("""SELECT id, municipio, estado, iniciado_em, legislacao_tipo,
                                  legislacao_numero, legislacao_ano
                           FROM buscas_historico WHERE job_id=%s LIMIT 1""", (_jid,))
            r = cur.fetchone()
            ...
            return jsonify({
                ...
                'modo': 'em_andamento',
                'job_id': _jid,
                'municipio': r['municipio'] if r else '',
                'estado': r['estado'] if r else '',
                'iniciado_em': r['iniciado_em'].isoformat() if r and r['iniciado_em'] else None,
                'legislacao_tipo': r['legislacao_tipo'] if r else '',
                'legislacao_numero': r['legislacao_numero'] if r else '',
                'legislacao_ano': r['legislacao_ano'] if r else '',
            })
        
        # 2. Nenhum job ativo — pegar ultima busca especifica CONCLUIDA
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""SELECT id, job_id, municipio, estado, iniciado_em, concluido_em,
                              sucesso, legislacao_tipo, legislacao_numero, legislacao_ano,
                              legislacao_link, zip_path, relatorio_path, tabela_path
                       FROM buscas_historico
        ...
        if not r:
            return jsonify({'success': True, 'modo': 'vazio'})
        
        return jsonify({
            'success': True,
            'modo': 'ultima_concluida',
            'job_id': r['job_id'],
            'hist_id': r['id'],
            'municipio': r['municipio'],
            'estado': r['estado'],
            ...
            'sucesso': r['sucesso'],
            'legislacao_tipo': r['legislacao_tipo'] or '',
            'legislacao_numero': r['legislacao_numero'] or '',
            'legislacao_ano': r['legislacao_ano'] or '',
            'legislacao_link': r['legislacao_link'] or '',
            'zip_url': r['zip_path'] or '',
            'relatorio_url': r['relatorio_path'] or '',
            'tabela_url': r['tabela_path'] or '',
        })
    except Exception as e:
...
def pagina_manutencao():
    ...
    if session.get('role') != 'admin':
        return redirect('/')
    return render_template('manutencao.html', active_page='manutencao', active_group='sistema', **tmpl_ctx())


@app.route('/api/manutencao/status')
@login_required
def api_manutencao_status():
    """Retorna status do servidor (disco, memoria, processos, etc)."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        import shutil as _sh, psutil as _ps, os as _os, subprocess as _sp
        # Disco
        _du = _sh.disk_usage('/')
        disco_pct = round(_du.used / _du.total * 100, 1)
        # Memoria
        _mem = _ps.virtual_memory()
        mem_pct = round(_mem.percent, 1)
        # Buscas
        conn = get_db(); cur = conn.cursor()
        cur.execute("SELECT COUNT(*), COUNT(*) FILTER (WHERE protegida) FROM buscas_historico")
        total_buscas, protegidas = cur.fetchone()
        # Buscas com arquivos (pelo menos um path nao-nulo)
        cur.execute("""SELECT COUNT(*) FROM buscas_historico
                       WHERE COALESCE(zip_path,'')<>''
                          OR COALESCE(relatorio_path,'')<>''
                          OR COALESCE(tabela_path,'')<>''
                          OR COALESCE(pdf_path,'')<>''""")
        buscas_com_arquivo = cur.fetchone()[0]
        cur.close(); conn.close()
        # Tamanho do diretorio downloads
        downloads_dir = '/var/www/urbanlex/static/downloads'
        try:
            _r = _sp.run(['du','-sb',downloads_dir], capture_output=True, text=True, timeout=10)
            downloads_bytes = int(_r.stdout.split()[0]) if _r.stdout else 0
        except Exception:
            downloads_bytes = 0
        # Logs (gunicorn + journal)
        log_files = ['/var/log/urbanlex-error.log', '/var/log/urbanlex-access.log']
        logs_bytes = sum(_os.path.getsize(f) for f in log_files if _os.path.exists(f))
        try:
            _r2 = _sp.run(['journalctl','--disk-usage'], capture_output=True, text=True, timeout=5)
            # Output: "Archived and active journals take up X.XM in the file system."
            import re as _re
            _m = _re.search(r'(\d+(?:\.\d+)?)([KMG])', _r2.stdout)
            if _m:
                _val = float(_m.group(1))
                _unit = _m.group(2)
                _mult = {'K':1024,'M':1024**2,'G':1024**3}[_unit]
                logs_bytes += int(_val * _mult)
        except Exception:
            pass
        # Chromium orfaos
        try:
            _rc = _sp.run(['pgrep','-c','chromium'], capture_output=True, text=True, timeout=3)
            chromium_count = int(_rc.stdout.strip()) if _rc.stdout.strip() else 0
        except Exception:
            chromium_count = 0
        return jsonify({
            'success': True,
            'disco_pct': disco_pct,
            'disco_usado_gb': round(_du.used / 1024**3, 1),
            'disco_total_gb': round(_du.total / 1024**3, 1),
            'disco_livre_gb': round(_du.free / 1024**3, 1),
            'memoria_pct': mem_pct,
            'memoria_usado_mb': round(_mem.used / 1024**2),
            'memoria_total_mb': round(_mem.total / 1024**2),
            'buscas_total': total_buscas,
            'buscas_protegidas': protegidas,
            'buscas_com_arquivo': buscas_com_arquivo,
            'downloads_bytes': downloads_bytes,
            'logs_bytes': logs_bytes,
            'chromium_orfaos': chromium_count,
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/manutencao/buscas')
@login_required
def api_manutencao_buscas():
    """Lista buscas com filtros opcionais."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        filtro = request.args.get('filtro', 'todas')  # todas, antigas30, antigas7, com_erro, protegidas
        ...
        params = []
        if filtro == 'antigas30':
            where.append("iniciado_em < NOW() - INTERVAL '30 days'")
        elif filtro == 'antigas7':
            where.append("iniciado_em < NOW() - INTERVAL '7 days'")
        elif filtro == 'com_erro':
            where.append("(sucesso = false OR concluido_em IS NULL)")
        elif filtro == 'protegidas':
            where.append("protegida = true")
        # So mostra buscas que tem pelo menos um arquivo (senao nao tem o que apagar)
        where.append("""(COALESCE(zip_path,'')<>'' OR COALESCE(relatorio_path,'')<>''
                        OR COALESCE(tabela_path,'')<>'' OR COALESCE(pdf_path,'')<>'')""")
        if q:
            where.append("(LOWER(municipio) LIKE %s OR LOWER(legislacao_numero) LIKE %s OR LOWER(legislacao_tipo) LIKE %s)")
            params.extend([f'%{q}%', f'%{q}%', f'%{q}%'])
        where_sql = ' AND '.join(where) if where else 'true'
        cur.execute(f"""SELECT id, tipo, municipio, estado, iniciado_em, concluido_em,
                              sucesso, protegida, legislacao_tipo, legislacao_numero,
                              legislacao_ano, zip_path, relatorio_path, tabela_path, pdf_path
                       FROM buscas_historico
                       WHERE {where_sql}
                       ORDER BY iniciado_em DESC LIMIT 200""", tuple(params))
        rows = cur.fetchall()
        cur.close(); conn.close()
        import os as _os
        data = []
        for r in rows:
            paths = [r['zip_path'], r['relatorio_path'], r['tabela_path'], r['pdf_path']]
            paths = [p for p in paths if p]
            tamanho_bytes = 0
            for p in paths:
                try:
                    full = '/var/www/urbanlex' + p if p.startswith('/static') else p
                    if _os.path.exists(full):
                        tamanho_bytes += _os.path.getsize(full)
                except Exception: pass
            data.append({
                'id': r['id'],
                'tipo': r['tipo'],
                ...
                'tem_zip': bool(r['zip_path']),
                'tem_relatorio': bool(r['relatorio_path']),
                'tem_tabela': bool(r['tabela_path']),
...
@app.route('/api/manutencao/buscas/<int:bid>/proteger', methods=['POST'])
@login_required
def api_manutencao_proteger(bid):
    """Toggle de protegida."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        valor = request.json.get('protegida', True) if request.is_json else True
        conn = get_db(); cur = conn.cursor()
        cur.execute("UPDATE buscas_historico SET protegida=%s WHERE id=%s", (bool(valor), bid))
        conn.commit(); cur.close(); conn.close()
        return jsonify({'success': True, 'protegida': bool(valor)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/manutencao/buscas/apagar', methods=['POST'])
@login_required
def api_manutencao_apagar_buscas():
    """Apaga arquivos fisicos das buscas selecionadas. Mantem registro no banco."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        ids = request.json.get('ids', []) if request.is_json else []
        if not ids:
            return jsonify({'success': False, 'error': 'sem_ids'})
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        # So apaga arquivos de buscas NAO protegidas
        cur.execute("""SELECT id, zip_path, relatorio_path, tabela_path, pdf_path
                       FROM buscas_historico
                       WHERE id = ANY(%s) AND protegida = false""", (ids,))
        rows = cur.fetchall()
        import os as _os
        apagados = 0
        bytes_liberados = 0
        ids_atualizados = []
        for r in rows:
            for col in ('zip_path','relatorio_path','tabela_path','pdf_path'):
                p = r[col]
                if not p: continue
                full = '/var/www/urbanlex' + p if p.startswith('/static') else p
                try:
                    if _os.path.exists(full):
                        bytes_liberados += _os.path.getsize(full)
                        _os.remove(full)
                        apagados += 1
                except Exception: pass
            ids_atualizados.append(r['id'])
        # Apaga o registro do banco pras buscas afetadas
        if ids_atualizados:
            cur2 = conn.cursor()
            cur2.execute("DELETE FROM buscas_historico WHERE id = ANY(%s)", (ids_atualizados,))
        ...
        return jsonify({
            ...
            'buscas_afetadas': len(ids_atualizados),
            'protegidas_ignoradas': len(ids) - len(ids_atualizados),
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/manutencao/logs/limpar', methods=['POST'])
@login_required
def api_manutencao_logs_limpar():
    """Trunca logs do app + reduz journal do systemd."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        import os as _os, subprocess as _sp
        bytes_antes = 0
        for f in ['/var/log/urbanlex-error.log', '/var/log/urbanlex-access.log']:
            if _os.path.exists(f):
                bytes_antes += _os.path.getsize(f)
                with open(f, 'w') as _ff: _ff.write('')
        # Reduz journal pra 200MB
        try:
            _sp.run(['journalctl','--vacuum-size=200M'], capture_output=True, timeout=30)
        except Exception: pass
        return jsonify({'success': True, 'mb_liberados': round(bytes_antes / 1024**2, 1)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


# ════════════════════════════════════════════════════════════════
# API KEYS — gerenciamento de chaves das IAs
# ════════════════════════════════════════════════════════════════

@app.route('/api/api-keys')
@login_required
def api_listar_keys():
    """Lista as chaves configuradas (mascaradas) + status."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        # IAs suportadas (frontend mostra apenas estas)
        suportadas = ['gemini', 'claude', 'groq', 'openai']

        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT servico, chave, atualizada_em FROM api_keys WHERE servico = ANY(%s)", (suportadas,))
        rows = {r['servico']: r for r in cur.fetchall()}
        cur.close(); conn.close()

        # Fallback: chave do .env quando nao tem no banco (legado)
        ...
        for s in suportadas:
            ...
            atualizada_em = None
            if s in rows:
                chave = rows[s]['chave'] or ''
                origem = 'banco'
                atualizada_em = rows[s]['atualizada_em'].isoformat() if rows[s]['atualizada_em'] else None
            else:
                env_val = os.environ.get(env_map.get(s, ''), '') or ''
                if env_val:
                    chave = env_val
                    origem = 'env'
            mascarada = ('...' + chave[-4:]) if len(chave) >= 4 else ''
            data.append({
                'servico': s,
...
@login_required
def api_salvar_key(servico):
    """Salva ou atualiza chave de IA."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    suportadas = ['gemini', 'claude', 'groq', 'openai']
    if servico not in suportadas:
        return jsonify({'success': False, 'error': 'servico invalido'}), 400
    try:
        chave = (request.json.get('chave') or '').strip() if request.is_json else ''
        if not chave or len(chave) < 10:
            return jsonify({'success': False, 'error': 'chave muito curta'}), 400
        conn = get_db(); cur = conn.cursor()
        cur.execute("""INSERT INTO api_keys (servico, chave, atualizada_em, atualizada_por)
                       VALUES (%s, %s, NOW(), %s)
                       ON CONFLICT (servico) DO UPDATE
                       SET chave = EXCLUDED.chave,
                           atualizada_em = NOW(),
                           atualizada_por = EXCLUDED.atualizada_por""",
                    (servico, chave, session.get('user_id')))
        conn.commit(); cur.close(); conn.close()
        # Atualiza tambem na env pra modulos que ainda leem de la
        env_map = {'gemini': 'GEMINI_API_KEY', 'groq': 'GROQ_API_KEY',
                   'claude': 'ANTHROPIC_API_KEY', 'openai': 'OPENAI_API_KEY'}
        if servico in env_map:
            os.environ[env_map[servico]] = chave
        return jsonify({'success': True, 'mascarada': '...' + chave[-4:]})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})
...
@login_required
def api_remover_key(servico):
    """Remove chave de IA."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        conn = get_db(); cur = conn.cursor()
        cur.execute("DELETE FROM api_keys WHERE servico = %s", (servico,))
        conn.commit(); cur.close(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})
...
# ════════════════════════════════════════════════════════════════
# PROMPTS SALVOS — biblioteca de prompts uploadados pelo usuario
# ════════════════════════════════════════════════════════════════

@app.route('/api/prompts-salvos')
@login_required
def api_listar_prompts():
    """Lista os prompts salvos."""
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        for r in (rows or []):
            data.append({
                'id': r['id'],
                'nome': r['nome'],
                'arquivo_origem': r['arquivo_origem'] or '',
                'tamanho_bytes': r['tamanho_bytes'] or 0,
...
@login_required
def api_obter_prompt(pid):
    """Retorna conteudo completo de um prompt."""
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT id, nome, conteudo, criado_em FROM prompts_salvos WHERE id = %s", (pid,))
...
def api_upload_prompt():
    """Faz upload de UM ou VARIOS arquivos de prompt."""
    try:
        arquivos = request.files.getlist('arquivos')
        if not arquivos:
        ...
        for f in arquivos:
            ...
            # Extrai texto conforme extensao
            conteudo = ''
            try:
                if ext == 'txt':
                    ...
                    try:
                        ...
                        rd = PdfReader(BytesIO(raw))
                        conteudo = '\n'.join((p.extract_text() or '') for p in rd.pages)
                    except Exception:
                        conteudo = '[Erro ao extrair PDF]'
                    ...
                    try:
                        ...
                        conteudo = '[Erro ao extrair DOCX]'
                else:
                    conteudo = raw.decode('utf-8', errors='ignore')
            except Exception as _e_extr:
                conteudo = f'[Erro: {str(_e_extr)[:80]}]'

            cur.execute("""INSERT INTO prompts_salvos (nome, conteudo, arquivo_origem, tamanho_bytes, criado_por)
                           VALUES (%s, %s, %s, %s, %s) RETURNING id""",
                        (nome, conteudo, nome, tamanho, session.get('user_id')))
            criados.append({'id': cur.fetchone()[0], 'nome': nome})
        conn.commit(); cur.close(); conn.close()
...
@login_required
def api_excluir_prompts():
    """Exclui prompts salvos por lista de IDs."""
    try:
        ids = request.json.get('ids', []) if request.is_json else []
        if not ids:
            return jsonify({'success': False, 'error': 'sem ids'}), 400
        conn = get_db(); cur = conn.cursor()
...
# ════════════════════════════════════════════════════════════════
# LANDLY — config + envio + status
# ════════════════════════════════════════════════════════════════

@app.route('/api/landly/config')
@login_required
def api_landly_config_get():
    """Retorna config Landly (URL + chave mascarada)."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT endpoint_url, api_key, atualizada_em FROM landly_config WHERE id = 1")
        r = cur.fetchone(); cur.close(); conn.close()
        endpoint = (r and r['endpoint_url']) or os.environ.get('PLATAFORMA_API_URL', '')
        chave = (r and r['api_key']) or os.environ.get('PLATAFORMA_API_KEY', '')
        return jsonify({
            'success': True,
            'endpoint_url': endpoint or '',
            'api_key_mascarada': ('...' + chave[-4:]) if len(chave) >= 4 else '',
            'tem_chave': bool(chave),
            'atualizada_em': r['atualizada_em'].isoformat() if (r and r['atualizada_em']) else None,
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/landly/config', methods=['POST'])
@login_required
def api_landly_config_set():
    """Salva endpoint e/ou chave Landly."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        d = request.json if request.is_json else {}
        endpoint = (d.get('endpoint_url') or '').strip()
        chave = (d.get('api_key') or '').strip()
        if not endpoint:
            return jsonify({'success': False, 'error': 'endpoint obrigatorio'}), 400
        conn = get_db(); cur = conn.cursor()
        # Se chave veio vazia, preserva a atual (nao apaga)
        if chave:
            cur.execute("""UPDATE landly_config
                           SET endpoint_url = %s, api_key = %s, atualizada_em = NOW()
                           WHERE id = 1""", (endpoint, chave))
        else:
            cur.execute("""UPDATE landly_config
                           SET endpoint_url = %s, atualizada_em = NOW()
                           WHERE id = 1""", (endpoint,))
        conn.commit(); cur.close(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
...
@app.route('/api/landly/testar-conexao', methods=['POST'])
@login_required
def api_landly_testar():
    """Faz uma chamada HEAD/GET no endpoint Landly pra verificar."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    try:
        d = request.json if request.is_json else {}
        endpoint = (d.get('endpoint_url') or '').strip()
        if not endpoint:
            return jsonify({'success': False, 'error': 'endpoint obrigatorio'})
        import requests as _rq, time as _t
        t0 = _t.time()
        try:
            r = _rq.head(endpoint, timeout=10, allow_redirects=True)
            ...
            return jsonify({'success': False, 'error': 'sem conexao: ' + str(e)[:80]})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/landly/enviar', methods=['POST'])
@login_required
def api_landly_enviar():
    """Adiciona planilhas a fila de envio Landly (assincrono)."""
    try:
        ids = request.json.get('ids', []) if request.is_json else []
        if not ids:
            return jsonify({'success': False, 'error': 'sem ids'}), 400
        ...
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT endpoint_url, api_key FROM landly_config WHERE id = 1")
        cfg = cur.fetchone()
        endpoint = (cfg and cfg['endpoint_url']) or os.environ.get('PLATAFORMA_API_URL', '')
        chave = (cfg and cfg['api_key']) or os.environ.get('PLATAFORMA_API_KEY', '')
        if not endpoint:
            cur.close(); conn.close()
            return jsonify({'success': False, 'error': 'endpoint Landly nao configurado'}), 400

        # Marca como enviando
        cur2 = conn.cursor()
        cur2.execute("""UPDATE planilhas_geradas
                        SET landly_status = 'enviando', landly_erro = NULL
                        WHERE id = ANY(%s)""", (ids,))
        conn.commit(); cur2.close()

        # Pega dados das planilhas pra enviar em thread
        cur.execute("""SELECT id, arquivo_path, arquivo_nome, municipio, estado
                       FROM planilhas_geradas WHERE id = ANY(%s)""", (ids,))
        rows = cur.fetchall()
        cur.close(); conn.close()

        # Thread em background pra enviar uma por uma
        import threading, requests as _rq
        def _worker():
            for r in rows:
                _conn = get_db(); _cur = _conn.cursor()
                try:
                    fpath = r['arquivo_path'] or ''
                    if fpath.startswith('/static'):
                        fpath = '/var/www/urbanlex' + fpath
                    if not os.path.exists(fpath):
                        _cur.execute("""UPDATE planilhas_geradas
                                        SET landly_status = 'falhou',
                                            landly_erro = 'arquivo nao encontrado',
                                            landly_endpoint = %s
                                        WHERE id = %s""", (endpoint, r['id']))
                        _conn.commit()
                        continue
                    headers = {'Authorization': f'Bearer {chave}'} if chave else {}
                    with open(fpath, 'rb') as fh:
                        files = {'planilha': (r['arquivo_nome'] or 'planilha.xlsx', fh,
                                              'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')}
                        data = {'municipio': r['municipio'], 'estado': r['estado']}
                        try:
                            resp = _rq.post(endpoint, files=files, data=data,
                                            headers=headers, timeout=120)
                            if 200 <= resp.status_code < 300:
                                _cur.execute("""UPDATE planilhas_geradas
                                                SET landly_status = 'enviada',
                                                    landly_enviado_em = NOW(),
                                                    landly_erro = NULL,
                                                    landly_endpoint = %s
                                                WHERE id = %s""", (endpoint, r['id']))
                            else:
        ...
        return jsonify({'success': True, 'enfileiradas': len(rows)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})


@app.route('/api/manutencao/diagnostico/<acao>', methods=['POST'])
@login_required
def api_manutencao_diagnostico(acao):
    """Acoes de diagnostico: reiniciar, matar_chromium, renovar_flaresolverr."""
    if session.get('role') != 'admin':
        return jsonify({'success': False, 'error': 'unauthorized'}), 403
    import subprocess as _sp
    try:
        if acao == 'matar_chromium':
            _sp.run(['pkill','-9','chromium'], capture_output=True, timeout=5)
            return jsonify({'success': True, 'msg': 'Chromium orfaos terminados'})
        elif acao == 'renovar_flaresolverr':
            import requests as _rq, os as _os
            _old = _os.environ.get('FLARESOLVERR_SESSION', '')
            if _old:
                try: _rq.post('http://localhost:8191/v1', json={'cmd':'sessions.destroy','session':_old}, timeout=5)
                except Exception: pass
            r = _rq.post('http://localhost:8191/v1', json={'cmd':'sessions.create'}, timeout=10)
            ns = r.json().get('session', '')
            if ns:
                _os.environ['FLARESOLVERR_SESSION'] = ns
                _sp.run(['sed','-i',f's/FLARESOLVERR_SESSION=.*/FLARESOLVERR_SESSION={ns}/','/var/www/urbanlex/.env'], capture_output=True)
                return jsonify({'success': True, 'msg': f'Nova sessao: {ns[:8]}...'})
            return jsonify({'success': False, 'error': 'falha_criar_sessao'})
        elif acao == 'reiniciar':
            # Agenda reinicio em 1s pra resposta voltar primeiro
            _sp.Popen(['bash','-c','sleep 1 && systemctl restart urbanlex'])
            return jsonify({'success': True, 'msg': 'Reinicio agendado'})
        else:
            return jsonify({'success': False, 'error': 'acao_invalida'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})
...
@app.route('/api/debug')
def api_debug_log():
    """Endpoint de debug: salva msg em arquivo dedicado."""
    msg = request.args.get('msg', '')
    try:
        with open('/tmp/dossie_debug.log', 'a') as f:
    ...
    try:
        ...
        for r in (rows or []):
            ...
            data.append({
                ...
                'relatorio_path': r.get('relatorio_path'),
                'tabela_path': r.get('tabela_path'),
...
def api_dossie_apagar_dossie(dossie_id):
    """Apaga um dossie (busca_historico) e seus arquivos."""
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
@app.route('/api/gerador/extrair-prompt', methods=['POST'])
@login_required
def api_gerador_extrair_prompt():
    arq = request.files.get('arquivo')
    if not arq: return jsonify({'success': False, 'error': 'arquivo obrigatorio'}), 400
    ...
    try:
        if ext == 'txt':
            ...
            import tempfile as _tmp_pp, subprocess as _sp_pp, os as _os_pp
            with _tmp_pp.NamedTemporaryFile(suffix='.pdf', delete=False) as _t:
                arq.save(_t.name); _path = _t.name
            try:
                _r = _sp_pp.run(['pdftotext', _path, '-'], capture_output=True, text=True, timeout=30)
                texto = _r.stdout if _r.returncode == 0 else ''
            finally:
                try: _os_pp.unlink(_path)
                except: pass
        elif ext in ('doc', 'docx'):
            import tempfile as _tmp_pp, os as _os_pp
            with _tmp_pp.NamedTemporaryFile(suffix='.' + ext, delete=False) as _t:
                arq.save(_t.name); _path = _t.name
            try:
                if ext == 'docx':
                    import docx as _docx
                    ...
                    import subprocess as _sp_pp
                    _r = _sp_pp.run(['antiword', _path], capture_output=True, text=True, timeout=30)
                    texto = _r.stdout if _r.returncode == 0 else ''
            finally:
                try: _os_pp.unlink(_path)
                except: pass
        else:
            return jsonify({'success': False, 'error': f'extensao nao suportada: {ext}'}), 400
        return jsonify({'success': True, 'texto': texto.strip()})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_gerador_iniciar():
    ...
    def _run():
        try:
            ...
            _tokens_in = getattr(resp.usage, 'input_tokens', 0) if hasattr(resp, 'usage') else 0
            _tokens_out = getattr(resp.usage, 'output_tokens', 0) if hasattr(resp, 'usage') else 0
            # Tabela de precos por modelo (USD por 1M tokens) — Claude Sonnet 4: $3 in / $15 out
            _PRECO_IN_M = 3.0
            _PRECO_OUT_M = 15.0
            _custo_usd = (_tokens_in / 1_000_000.0) * _PRECO_IN_M + (_tokens_out / 1_000_000.0) * _PRECO_OUT_M
            _ia_modelo = 'claude-sonnet-4-20250514'
            _ia_provider = 'anthropic'
            job['logs'].append({'nivel':'info','msg':f'💰 Tokens: {_tokens_in} in / {_tokens_out} out · Custo: US$ {_custo_usd:.4f}'})
            # Coleta legislacoes usadas a partir dos compilados (todos os files_content tem 'title' com municipio)
            _legis_usadas = []
            try:
                for comp in compilados:
                    _mun_e = f"{comp.get('municipio','')}/{comp.get('estado','')}"
                    if comp.get('zip_on') and comp.get('zip'):
                        _legis_usadas.append({'municipio': _mun_e, 'tipo': 'zip', 'arquivo': _os_gp.path.basename(comp['zip'])})
                    if comp.get('tabela_on') and comp.get('tabela'):
                        _legis_usadas.append({'municipio': _mun_e, 'tipo': 'tabela', 'arquivo': _os_gp.path.basename(comp['tabela'])})
            except Exception: pass
            # Extrair JSON
            ...
            try:
                ...
                _legis_json = _json_db.dumps(_legis_usadas, ensure_ascii=False)
                for comp in compilados:
                    cur2.execute("""INSERT INTO planilhas_geradas
                        (municipio, estado, arquivo_path, arquivo_nome, zonas_count, prompt,
                         ia_provider, ia_modelo, tokens_in, tokens_out, custo_usd,
                         legislacoes_usadas, criado_por)
                        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s)""",
                        (comp['municipio'], comp['estado'], out_path, nome_arquivo, len(zonas), prompt,
                         _ia_provider, _ia_modelo, _tokens_in, _tokens_out, _custo_usd,
                         _legis_json, session.get('user_id') if session else None))
                conn2.commit(); cur2.close(); conn2.close()
            except Exception as _e_db:
                job['logs'].append({'nivel':'aviso','msg':f'Erro ao salvar no banco: {str(_e_db)[:120]}'})
...
def api_gerador_historico():
    """Lista planilhas geradas com tokens, custo e status Landly."""
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""SELECT id, municipio, estado, arquivo_path, arquivo_nome,
                              zonas_count, criado_em,
                              ia_provider, ia_modelo, tokens_in, tokens_out, custo_usd,
        ...
        for r in rows:
            ...
            arquivo_url = ''
            if r['arquivo_path']:
                arquivo_url = r['arquivo_path'] if r['arquivo_path'].startswith('/static') else '/static/downloads/' + os.path.basename(r['arquivo_path'])
            tamanho_bytes = 0
            if r['arquivo_path']:
                full = '/var/www/urbanlex' + r['arquivo_path'] if r['arquivo_path'].startswith('/static') else r['arquivo_path']
                try:
                    if os.path.exists(full):
                        tamanho_bytes = os.path.getsize(full)
                except Exception: pass
            planilhas.append({
                'id': r['id'],
                'municipio': r['municipio'],
                'estado': r['estado'],
                'arquivo_nome': r['arquivo_nome'] or 'planilha.xlsx',
                'arquivo_url': arquivo_url,
                'tamanho_bytes': tamanho_bytes,
                'zonas_count': r['zonas_count'] or 0,
                'criado_em': r['criado_em'].isoformat() if r['criado_em'] else None,
                'ia_provider': r['ia_provider'] or '',
                'ia_modelo': r['ia_modelo'] or '',
                'tokens_in': tokens_in,
                'tokens_out': tokens_out,
                'tokens_total': tokens_in + tokens_out,
                'custo_usd': float(r['custo_usd'] or 0),
                'legislacoes_usadas': r['legislacoes_usadas'] or [],
                'landly_status': r['landly_status'] or 'nao_enviada',
                'landly_enviado_em': r['landly_enviado_em'].isoformat() if r['landly_enviado_em'] else None,
                'landly_erro': r['landly_erro'] or '',
            })
        # Stats globais
        total_tokens = sum(p['tokens_total'] for p in planilhas)
        total_custo = round(sum(p['custo_usd'] for p in planilhas), 4)
        enviadas = sum(1 for p in planilhas if p['landly_status'] == 'enviada')
        return jsonify({
            'success': True,
            'planilhas': planilhas,
            'stats': {
                ...
                'custo_usd_total': total_custo,
                'landly_enviadas': enviadas,
            },
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)[:200]})
...
def dossie_municipais():
    return render_template('dossie_municipais.html', active_page='dossie-municipais', **tmpl_ctx())
