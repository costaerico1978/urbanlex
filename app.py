...
def _cleanup_old_jobs():
    ...
    now = time.time()
    expired = [k for k, v in _buscador_jobs.items() if now - v['ts'] > 1800]
    for k in expired:
    ...
    def run():
        try:
            ...
            if isinstance(result, dict):
                ...
                if pdf_downloads:
                    job['logs'].append({'nivel': 'ok', 'msg': f'📥 {len(pdf_downloads)} PDF(s) disponível(is) para download'})
...
def api_buscador_job_poll(job_id):
    ...
    if not job:
        return jsonify({'success': False, 'error': 'Job não encontrado', 'done': True})
...
def api_buscador_autocomplete():
    ...
    # Deduplicar por nome (case-insensitive) — evita dois "Rio de Janeiro" com
    # estados diferentes ('RJ' vs '') vindos de registros duplicados no DB.
    # Preferência: registro com estado válido (2 letras) sobre vazio.
    seen = {}  # chave: nome.lower()
    for m in ibge_matches:
        db_key  = (m['nome'], m['estado'])
        db_key2 = (m['nome'], '')  # pode ter sido salvo sem estado
        m['tem_sites'] = db_key in db_com_sites or db_key2 in db_com_sites
        nome_key = m['nome'].lower()
        existing = seen.get(nome_key)
        # Preferir o que tem estado válido, ou o primeiro encontrado
        if not existing or (not existing['estado'] and m['estado']):
            seen[nome_key] = m
    ...
    for (nome, uf) in db_com_sites:
        ...
        if nome_key not in seen:
            seen[nome_key] = {'nome': nome, 'estado': uf, 'tem_sites': True}
        elif not seen[nome_key]['estado'] and uf:
            # Substituir entrada sem estado por uma com estado válido
            seen[nome_key]['estado'] = uf
            seen[nome_key]['tem_sites'] = True
...
def api_buscador_sites_post():
    ...
    # Normalizar estado: apenas UF válida de 2 letras; "Todos"/vazio → ''
    ...
    try:
        ...
        try:
            cur = conn.cursor()
            # Tentar UPDATE case-insensitive primeiro (mantém municipio_nome como está no banco)
            cur.execute("""UPDATE municipio_sites_referencia
                           SET urls_extras = %s, estado = %s, url_diario = '', atualizado_em = NOW()
                           WHERE LOWER(municipio_nome) = LOWER(%s)""",
                        (fontes, estado, municipio))
            if cur.rowcount == 0:
                # Nenhuma linha atualizada — inserir novo registro
                cur.execute("""INSERT INTO municipio_sites_referencia
                                   (municipio_nome, estado, url_diario, urls_extras, atualizado_em)
                               VALUES (%s, %s, '', %s, NOW())
                               ON CONFLICT (municipio_nome, estado)
                               DO UPDATE SET url_diario      = '',
                                             urls_extras     = EXCLUDED.urls_extras,
                                             atualizado_em   = NOW()""",
                            (municipio, estado, fontes))
            conn.commit()
        finally:
            conn.close()
