...
def _cleanup_old_jobs():
    """Remove jobs com mais de 10 minutos."""
    now = time.time()
    expired = [k for k, v in _buscador_jobs.items() if now - v['ts'] > 600]
    for k in expired:
        ...
        try:
            ...
            if isinstance(result, dict):
                job['result'] = {
                    ...
                    'textos_fontes': result.get('textos_fontes', []),
...
def api_buscador_job_poll(job_id):
    ...
    if job['done']:
        ...
        # Limpar job após entrega do resultado (com delay pra re-polls)
        job['ts'] = time.time() - 540  # expira em 60s
