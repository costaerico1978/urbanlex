...
def buscar_publicacoes_legislacao(municipio_id: int,
    ...
    if not url_base:
        resultado['mensagem'] = 'URL do diário não configurada'
        return resultado
    ...
    logger.info(f"  URL: {url_base} | Período: {data_inicio} → {data_fim}")

    # ── 3. Determinar página de busca ──
    ...
    # ── 4. Buscar — tentar cada termo até encontrar resultados ──
    todas_pubs = []
