...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
    ...
    try:
        ...
        if _onclick_fn:
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔧 Executando onclick via JS: {_onclick_fn[:60]}'})
            try:
                # Registrar download handler no contexto ANTES do popup
                _popup_download = [None]
                
                def _on_ctx_download(download):
                    _popup_download[0] = download
                
                try:
                    page.context.on('page', lambda p: p.on('download', _on_ctx_download))
                except Exception:
                    pass
                
                with page.expect_popup(timeout=15000) as popup_info:
                    # Executar o JavaScript diretamente (mais confiável que clicar no elemento)
                    el_or_parent = el.evaluate('el => { let a = el.closest("a") || el; return true; }')
                    el.click()
                
                popup_page = popup_info.value
                
                # Registrar download handler no popup
                _popup_dl = [None]
                try:
                    popup_page.on('download', lambda d: _popup_dl.__setitem__(0, d))
                except Exception:
                    pass
                ...
                logs.append({'nivel': 'ok', 'msg': f'{label}: 🪟 Popup aberto: {popup_url[:100]}'})
                
                # Verificar se houve download
                dl_obj = _popup_dl[0] or _popup_download[0] or _download_obj
                if dl_obj:
                    try:
                ...
                # Tentar ler o conteúdo da popup diretamente (pode ser PDF renderizado)
                try:
                    # Verificar content-type via JS
                    ct = popup_page.evaluate('() => document.contentType || ""') or ''
                    
                    if 'pdf' in ct.lower():
                        # PDF inline — baixar via goto na popup (já tem proxy)
                        logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Popup content-type: {ct}'})
                        try:
                            resp = popup_page.request.get(popup_url)
                            if resp.status == 200 and len(resp.body()) > 1000:
                                dl_path = tempfile.mktemp(suffix='.pdf')
                                with open(dl_path, 'wb') as f_pdf:
                                    f_pdf.write(resp.body())
                                dl_size = len(resp.body()) // 1024
                                logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF da popup: {dl_size}KB'})
                                _cleanup_listeners()
                                return f'Download: {dl_path}'
                        except Exception:
                            pass
                    
                    # Não é PDF — pode ser HTML com conteúdo da legislação
                    body = popup_page.inner_text('body') or ''
                    if len(body) > 200:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Popup HTML: {len(body)} chars'})
                        _cleanup_listeners()
                        return f'Nova aba: {popup_url}'
                    
                except Exception as e_popup_read:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Leitura do popup falhou: {str(e_popup_read)[:40]}'})
                
                _cleanup_listeners()
                return f'Nova aba: {popup_url}'
                
            except Exception as e_popup:
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ expect_popup falhou ({str(e_popup)[:40]}) — tentando clique normal'})
                try:
                    el.click()
                except Exception:
                    pass
