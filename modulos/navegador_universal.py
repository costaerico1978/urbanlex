    ...
    estado = frame.evaluate(r'''() => {
        ...
        document.querySelectorAll('table').forEach((table, ti) => {
            ...
            for (const tr of trs) {
                ...
                // Links na linha (incluindo ícones sem texto)
                tr.querySelectorAll('a[href]').forEach(a => {
                    ...
                    linha.links.push({
                        ...
                        texto: text.substring(0, 100) || '(ícone/imagem)',
                        coluna: colName,
                        tem_icone: hasImg,
                        alt_imagem: imgAlt,
                        title: title
...
- "preencher": preencher um campo de texto/data
- "selecionar": escolher uma opção num dropdown (use o TEXTO da opção, não o value)
- "clicar": clicar num botão ou link
- "navegar": ir para uma URL específica
- "baixar": clicar num link que provavelmente baixa um arquivo (PDF)
...
- "refinar": a busca retornou muitos resultados, preciso refinar
...
- Ícones sem texto em tabelas geralmente são botões de download/visualização
...
{{
    ...
    "acao": {{
        "tipo": "preencher|selecionar|clicar|navegar|baixar|proximo|voltar|concluido|refinar|desistir",
        "alvo": "#id_do_elemento ou descrição",
        "valor": "valor a preencher (se aplicável)",
...
IMPORTANTE: O campo "legislacao_encontrada" só deve ter "encontrada": true quando você TEM CERTEZA 
que identificou a legislação correta e tem o link para ela."""
...
def navegar_como_humano(
    ...
    for passo in range(1, max_passos + 1):
        try:
            ...
            pensamento = decisao.get('decisao', '') or decisao.get('pensamento', '')
            o_que_vejo = decisao.get('o_que_vejo', '')[:100]
            opcoes = decisao.get('opcoes_e_consequencias', [])
            acao = decisao.get('acao', {})
            tipo_acao = acao.get('tipo', '')
            alvo = acao.get('alvo', '')
            valor = acao.get('valor', '')
            href = acao.get('href', '')
            resultado_esperado = decisao.get('resultado_esperado', '')[:100]
            ...
            if opcoes:
                for op in opcoes[:4]:
                    ...
                    logs.append({'nivel': 'info', 'msg': f'{label}:   {aprox} {op.get("opcao","")[:50]} → {op.get("consequencia_provavel","")[:50]}'})
            logs.append({'nivel': 'info', 'msg': f'{label}: 🧠 Decisão: {tipo_acao} — {pensamento[:80]}'})
            
            # 4. Verificar se encontrou
            leg_encontrada = decisao.get('legislacao_encontrada', {})
            if leg_encontrada.get('encontrada'):
                resultado['encontrada'] = True
                resultado['url'] = leg_encontrada.get('url', '')
                resultado['status'] = leg_encontrada.get('status', '')
                resultado['confirmacao'] = leg_encontrada.get('confirmacao', '')
                ...
                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Legislação encontrada! {resultado["url"][:80]}'})
                ...
                historico.append({'passo': passo, 'acao': 'concluido', 'resultado': f'Encontrada: {resultado["url"][:60]}'})
                break
            ...
            exec_resultado = _executar_acao(page, frame, acao, logs, label, passo)
            ...
            logs.append({'nivel': 'aviso', 'msg': f'{label}: Passo {passo} erro: {str(e)[:80]}'})
            historico.append({'passo': passo, 'acao': 'erro', 'resultado': str(e)[:100]})
...
def _executar_acao(page, frame, acao: dict, logs: list, label: str, passo: int) -> str:
    ...
    try:
        if tipo == 'preencher':
            ...
            if not el:
                # Tentar no page principal se frame falhou
                if frame != page:
            ...
            if not el and href:
                # Buscar por href em toda a página
            ...
            try:
                ...
                # Ler info básica do PDF para a IA
                try:
                    import fitz
                    doc = fitz.open(dl_path)
                    n_pages = doc.page_count
                    # Ler primeiras páginas para verificação
                    preview = ''
                    for i in range(min(5, n_pages)):
                        preview += doc[i].get_text()[:500] + '\n--- página ---\n'
                    doc.close()
                ...
                # Não foi download — pode ser popup ou navegação
                try:
                    ...
                    try:
                        ...
                        pass
                    time.sleep(2)
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🪟 Popup aberto: {popup.url[:60]}'})
                    return f'Popup aberto: {popup.url}'
                except Exception:
                    pass
                
                # Navegação normal
                time.sleep(2)
                logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Clicou (sem download): {alvo or href}'[:60]})
                return f'Clicou sem download: {alvo or href}'
...
def _encontrar_elemento(frame, alvo: str, href: str = None):
    """Encontra um elemento na página por ID, seletor CSS, ou href."""
    ...
    # Tentar buscar link na linha da tabela pelo texto do alvo
    if not el and alvo:
        for tr in frame.query_selector_all('tr'):
            tr_text = tr.text_content() or ''
            if alvo.lower() in tr_text.lower():
                links = tr.query_selector_all('a[href]')
                for link in links:
                    has_icon = link.query_selector('img, svg, i')
                    if has_icon:
                        el = link
                        break
                if el:
