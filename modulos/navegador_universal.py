        ...
        if _onclick_fn:
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    ...
                    # route.fetch() captura o body completo (síncrono, thread-safe no loop do Playwright)
                    # route.fulfill() serve o body a partir da memória — proxy não toca mais o stream
                    # Browser recebe Content-Disposition: attachment → dispara download event normalmente
                    try:
                        _resp_cnt = route.fetch(timeout=45000)
                        _ct_cnt   = _resp_cnt.headers.get('content-type', '').lower()
                        _cd_cnt   = _resp_cnt.headers.get('content-disposition', '').lower()
                        _body_cnt = _resp_cnt.body()
                        _sz_cnt   = len(_body_cnt) if _body_cnt else 0
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route.fetch ContadorAcessoAto: {_resp_cnt.status} ct={_ct_cnt[:30]} cd={_cd_cnt[:40]} sz={_sz_cnt}b head={(_body_cnt[:8].hex() if _body_cnt else None)}'})
                        if _sz_cnt > 1000 and _body_cnt[:4] == b'%PDF':
                            _a0_body_captured[0] = _body_cnt
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ route.fetch: PDF capturado {_sz_cnt//1024}KB — fulfilling para browser'})
                        else:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ route.fetch: não é PDF — fulfilling mesmo assim'})
                        route.fulfill(response=_resp_cnt)
                    except Exception as _e_cnt:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route.fetch falhou ({str(_e_cnt)[:80]}) — usando continue_'})
                        try: route.continue_()
