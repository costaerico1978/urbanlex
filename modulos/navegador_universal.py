...
def _add_rulers(screenshot_b64: str) -> str:
    """
    Grid 70x70: colunas A-BR (azul) x linhas 1-70 (vermelho).
    Cada célula ~18x13px. Labels a cada 5 colunas/linhas.
    """
    ...
    # Labels de coluna: A..Z, AA..AZ, BA..BR
    col_labels = []
    for i in range(n_cols):
        if i < 26:
            col_labels.append(chr(65 + i))
        elif i < 52:
            col_labels.append('A' + chr(65 + i - 26))
        else:
            col_labels.append('B' + chr(65 + i - 52))
    
    # Colunas verticais (AZUL)
    for i in range(n_cols + 1):
        ...
        if i % 5 == 0:
            draw.line([(x, 12), (x, h)], fill=(0, 80, 255, 70), width=1)
        else:
            draw.line([(x, 12), (x, h)], fill=(0, 80, 255, 30), width=1)
        
        # Label no topo a cada 5 colunas
        if i < n_cols and i % 5 == 0:
            cx = int((i + 0.5) * col_w)
            lbl = col_labels[i]
            lw = 8 if len(lbl) == 1 else 13
            draw.rectangle([(cx - lw//2, 0), (cx + lw//2 + 1, 11)], fill=(0, 80, 255, 220))
            draw.text((cx - lw//2 + 1, 0), lbl, fill=(255, 255, 255, 255))
    
    # Linhas horizontais (VERMELHO)
    for j in range(n_rows + 1):
        ...
        if j % 5 == 0:
            draw.line([(18, y), (w, y)], fill=(220, 0, 0, 70), width=1)
        else:
            draw.line([(18, y), (w, y)], fill=(220, 0, 0, 30), width=1)
        
        # Número à esquerda a cada 5 linhas
        if j < n_rows and j % 5 == 0:
            cy = int((j + 0.5) * row_h)
            num = str(j + 1)
            draw.rectangle([(0, cy - 5), (17, cy + 6)], fill=(220, 0, 0, 220))
            draw.text((1, cy - 5), num, fill=(255, 255, 255, 255))
...
def _montar_prompt(legislacao: dict, historico: list, passo: int, url_atual: str) -> str:
    """Prompt único: IA decide E informa coordenadas via régua."""
...
Olhe o screenshot. Ha um GRID sobreposto: colunas AZUIS (A-BR, 70 colunas) e linhas VERMELHAS (1-70). Labels a cada 5 linhas/colunas.
As colunas vao de A a Z, depois AA a AZ, depois BA a BR. Conte as linhas finas entre os labels para precisao.

ACOES:
- "clicar": informe a CELULA do elemento (ex: "M28")
- "digitar": informe a celula do campo + texto a digitar
...
11. COORDENADAS: os labels azuis (colunas) aparecem a cada 5 colunas no topo. Os labels vermelhos (linhas) aparecem a cada 5 linhas na lateral esquerda. Conte as linhas finas entre os labels. Exemplo: se o botao esta 2 linhas finas abaixo do label "40" e na 3a coluna depois do label "K", informe celula="N42".
...
def _executar_acao(page, acao: dict, logs: list, label: str) -> str:
    ...
    # Converter célula estilo planilha (ex: "H29" ou "AA15") para pixels
    celula = (acao.get('celula', '') or '').strip().upper()
    n_cols = 70
    n_rows = 70
    ...
    if celula:
        import re as _re
        m = _re.match(r'^([A-Z]{1,2})(\d+)$', celula)
        if m:
            col_str = m.group(1)
            row_num = int(m.group(2))
            
            # Converter coluna: A=0..Z=25, AA=26..AZ=51, BA=52..BR=69
            if len(col_str) == 1:
                col_idx = ord(col_str) - ord('A')
            else:
                col_idx = 26 * (ord(col_str[0]) - ord('A') + 1) + (ord(col_str[1]) - ord('A'))
            ...
            col_w = VIEWPORT_W / n_cols
            row_h = VIEWPORT_H / n_rows
            x = int((col_idx + 0.5) * col_w)
            y = int((row_idx + 0.5) * row_h)
    
    # Fallback para rx/ry se celula não informada
    if x == 0 and y == 0:
        rx = float(acao.get('rx', 0) or 0)
        ry = float(acao.get('ry', 0) or 0)
        x = int(rx * VIEWPORT_W / 100)
        y = int(ry * VIEWPORT_H / 100)
        ...
        if tipo == 'clicar':
            ...
            try:
                ...
                page.evaluate(f"""() => {{
                    ...
                    m.id = 'nav-crosshair';
                    m.style.cssText = 'position:fixed;left:{x-10}px;top:{y-10}px;width:20px;height:20px;border:3px solid red;border-radius:50%;z-index:999999;pointer-events:none;';
                    document.body.appendChild(m);
                    let h = document.createElement('div');
                    h.style.cssText = 'position:fixed;left:{x-15}px;top:{y}px;width:30px;height:2px;background:red;z-index:999999;pointer-events:none;';
                    document.body.appendChild(h);
                    let v = document.createElement('div');
                    v.style.cssText = 'position:fixed;left:{x}px;top:{y-15}px;width:2px;height:30px;background:red;z-index:999999;pointer-events:none;';
                ...
                try:
                    ...
                    ss_path = f'{debug_dir}/click_{label_clean}_{celula}_px{x}x{y}.png'
                    ...
                    pass
                page.evaluate('() => { document.querySelectorAll("#nav-crosshair, [style*=\'z-index:999999\']").forEach(e => e.remove()); }')
            except Exception:
                pass
            ...
            logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Clicou em ({x}, {y}) [célula={celula}]'})
...
def navegar_como_humano(
    ...
    for passo in range(1, max_passos + 1):
        try:
            ...
            # 2. Adicionar réguas ao screenshot
            screenshot_com_regua = _add_rulers(screenshot_b64)
            
            # Salvar versão com réguas para debug
            try:
                debug_dir = '/tmp/nav_screenshots'
                os.makedirs(debug_dir, exist_ok=True)
                import re as _re
                label_clean = _re.sub(r'[^\w\-]', '_', label)
                ss_ruler_path = f'{debug_dir}/ruler_{label_clean}_{passo}.png'
                with open(ss_ruler_path, 'wb') as f_ss:
                    f_ss.write(base64.b64decode(screenshot_com_regua))
                ruler_url = f'/debug/screenshots/img/{os.path.basename(ss_ruler_path)}'
                logs.append({'nivel': 'info', 'msg': f'{label}: 👁️ <a href="{ruler_url}" target="_blank" style="color:#4fc3f7">O que a IA vê (passo {passo})</a>'})
            except Exception:
                pass

            # 3. Prompt único (decisão + coordenadas via régua)
            prompt = _montar_prompt(legislacao, historico, passo, url_atual)
            resp = _chamar_gemini_visao(prompt, screenshot_com_regua, logs, f'{label} passo {passo}')
            ...
            if len(historico) >= 2:
                ...
                if all(t == tipo_acao for t in tipos_recentes):
                    ...
                    if len(set(resultados_recentes)) == 1:
                        ...
                        if tipo_acao == 'clicar':
                            cel = (acao.get('celula', '') or '').strip().upper()
                            if cel:
                                import re as _re_loop
                                m_loop = _re_loop.match(r'^([A-Z]{1,2})(\d+)$', cel)
                                if m_loop:
                                    col_str = m_loop.group(1)
                                    if len(col_str) == 1:
                                        col_idx = ord(col_str) - ord('A')
                                    else:
                                        col_idx = 26 * (ord(col_str[0]) - ord('A') + 1) + (ord(col_str[1]) - ord('A'))
                                    row_idx = int(m_loop.group(2)) - 1
                                    dx = int((col_idx + 0.5) * VIEWPORT_W / 70)
                                    dy = int((row_idx + 0.5) * VIEWPORT_H / 70)
                                else:
                                    dx, dy = 0, 0
                            else:
                                dx, dy = 0, 0
