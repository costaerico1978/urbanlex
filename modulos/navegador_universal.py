        ...
        if _onclick_fn:
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    # NÃO usar route.fetch() — bloqueia o download event por até 90s.
                    # route.continue_() imediato deixa o browser (com proxy) processar a
                    # requisição e disparar o download event, que _on_dl_a0 captura.
                    # O fallback do save_as usa context.request para buscar o HTML do
                    # ContadorAcessoAto (1208b), extrai a URL real do PDF e faz o fetch.
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route: ContadorAcessoAto — continue_ (não bloquear download)'})
                    try: route.continue_()
