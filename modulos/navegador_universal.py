...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
    ...
    try:
        ...
        # Se tem onclick (ex: MostraDocumento), usar expect_popup — abre nova janela
        if _onclick_fn:
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔧 onclick detectado — abrindo popup com intercepção antecipada'})
            try:
                # ── PASSO 1: Registrar listeners no CONTEXTO antes de clicar ──────────
                # Captura toda navegação e downloads que ocorrem no popup,
                # mesmo que o popup se feche imediatamente após servir o documento.
                _popup_nav_history = []   # todas as URLs navegadas no popup
                _popup_dl = [None]        # download disparado pelo popup

                def _setup_popup_listeners(new_popup):
                    """Chamado instantaneamente quando a nova janela é criada."""
                    def _on_nav(frame):
                        try:
                            if frame == new_popup.main_frame:
                                url = frame.url or ''
                                if url and url != 'about:blank' and url not in _popup_nav_history:
                                    _popup_nav_history.append(url)
                        except Exception:
                            pass
                    def _on_dl(d):
                        _popup_dl[0] = d
                ...
                # Capturar URL IMEDIATAMENTE — antes de qualquer wait que possa falhar
                popup_initial_url = ''
                try:
                    popup_initial_url = popup_page.url or ''
                    if popup_initial_url not in _popup_nav_history:
                        _popup_nav_history.insert(0, popup_initial_url)
                except Exception:
                    popup_initial_url = _popup_nav_history[0] if _popup_nav_history else ''

                logs.append({'nivel': 'ok', 'msg': f'{label}: 🪟 Popup aberto: {popup_initial_url[:100]}'})

                # ── PASSO 3: Aguardar sem travar ─────────────────────────────────────
                # NÃO usar wait_for_load_state — mata o contexto quando o popup
                # se fecha sozinho após servir o documento.
                # Espera simples: deixa o popup navegar/fechar no seu ritmo.
                time.sleep(5)

                # ── PASSO 4a: Download foi disparado? ────────────────────────────────
                ...
                if dl_obj:
                    try:
                        ...
                        logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 Download do popup: {dl_size}KB'})
                        _cleanup_listeners()
                        return f'Download: {dl_path}'
                    except Exception:
                        pass

                # ── PASSO 4b: URL final (pode ter mudado por redirect) ───────────────
                final_url = popup_initial_url
                try:
                    current = popup_page.url
                    if current and current != 'about:blank':
                        final_url = current
                        if current not in _popup_nav_history:
                            _popup_nav_history.append(current)
                except Exception:
                    pass
                # Usar última URL navegada (post-redirect)
                if _popup_nav_history:
                    final_url = _popup_nav_history[-1]

                logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 Histórico popup: {" → ".join(u[:50] for u in _popup_nav_history)}'})

                # ── PASSO 4c: Ler conteúdo enquanto popup ainda vivo ────────────────
                body_text = ''
                content_type_popup = ''
                try:
                    content_type_popup = popup_page.evaluate('() => document.contentType || ""') or ''
                    if 'pdf' in content_type_popup.lower():
                        # PDF inline no popup — baixar via request do próprio popup
                        try:
                            resp = popup_page.request.get(final_url, timeout=20000)
                            if resp.status == 200 and len(resp.body()) > 1000:
                                dl_path = tempfile.mktemp(suffix='.pdf')
                                with open(dl_path, 'wb') as f_pdf:
                                    f_pdf.write(resp.body())
                                dl_size = len(resp.body()) // 1024
                                logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF inline do popup: {dl_size}KB'})
                                _cleanup_listeners()
                                return f'Download: {dl_path}'
                        except Exception:
                            pass
                    else:
                        body_text = popup_page.inner_text('body') or ''
                except Exception as e_read:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ Popup fechou antes da leitura ({str(e_read)[:50]}) — tentando fetch direto'})

                # ── PASSO 5: Popup morreu mas temos URL → fetch via Python requests ──
                # Estratégia universal: se o popup se fechou (qualquer site),
                # usar requests Python com os cookies do contexto para buscar o documento.
                if not body_text and final_url and final_url.startswith('http'):
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 Fetch direto: {final_url[:80]}'})
                    try:
                        import requests as _req
                        # Exportar cookies do contexto Playwright para requests
                        cookies = {}
                        try:
                        ...
                        ct_r = r.headers.get('content-type', '')
                        final_fetched_url = r.url  # URL após possíveis redirects

                        if 'pdf' in ct_r.lower() or final_fetched_url.lower().endswith('.pdf'):
                            dl_path = tempfile.mktemp(suffix='.pdf')
                            with open(dl_path, 'wb') as f_pdf:
                                f_pdf.write(r.content)
                            dl_size = len(r.content) // 1024
                            logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF via requests: {dl_size}KB ({final_fetched_url[:60]})'})
                            _cleanup_listeners()
                            return f'Download: {dl_path}'
                        elif len(r.text) > 300:
                            body_text = r.text
                            final_url = final_fetched_url
                            logs.append({'nivel': 'info', 'msg': f'{label}: 📄 HTML via requests: {len(body_text)} chars'})
                    except Exception as e_req:
                        logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Fetch direto falhou: {str(e_req)[:60]}'})

                # ── PASSO 6: Retornar resultado ──────────────────────────────────────
                if body_text and len(body_text) > 200:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Popup conteúdo disponível: {len(body_text)} chars'})

                _cleanup_listeners()
                return f'Nova aba: {final_url}'

            except Exception as e_popup:
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ expect_popup falhou ({str(e_popup)[:60]}) — tentando clique normal'})
                try:
                    el.click()
                except Exception:
                    pass
