...
9. FORMULARIOS: se o formulario tem dropdowns (select) e campos de texto simples, use "preencher_formulario" para preencher tudo de uma vez. Mas se vir datepickers, calendarios, ou widgets especiais, use "clicar" e "digitar" visualmente nesses campos.
10. Se "preencher_formulario" falhou nos passos anteriores (campo nao preenchido, pagina nao mudou), mude de estrategia: use "clicar" e "digitar" visualmente.
11. Se houver campo de RANGE DE DATAS (Data Inicial / Data Final), preencha Data Inicial com a data de publicacao e Data Final com a data de publicacao + 5 dias. Exemplo: se data e 14/01/2019, coloque Inicial=14/01/2019 e Final=19/01/2019.
12. Sempre que possivel, BAIXE O ARQUIVO EM PDF. Prefira clicar em links/botoes de download PDF em vez de abrir versao HTML. PDF e mais confiavel para analise.
13. Se o clique num icone/link nao funcionou (pagina nao mudou), tente clicar em coordenadas ligeiramente diferentes ou use duplo clique (clicar 2x no mesmo ponto).
        ...
        if tipo == 'clicar':
            ...
            try:
                ...
                try:
                    ...
                    label_clean = label.replace(' ', '_').replace('(', '').replace(')', '').replace('/', '_')
            ...
            if len(historico) >= 2:
                ...
                if all(t == tipo_acao for t in tipos_recentes):
                    ...
                    if len(set(resultados_recentes)) == 1:
                        ...
                        if tipo_acao == 'clicar' and acao.get('x') and acao.get('y'):
