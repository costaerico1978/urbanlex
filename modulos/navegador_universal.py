        ...
        if _onclick_fn:
                    ...
                    try:
                        ...
                        for _use_proxy in [False, True]:
                            ...
                            try:
                                ...
                                # 200 sem proxy = resposta real do servidor (pode ter JS redirect)
                                # Parar aqui para parsear o JS correto, não o erro 500 do proxy
                                if not _use_proxy and _r_a.status_code == 200 and _sz_try > 50:
                        ...
                        if 'pdf' in _ct_a or _url_a.lower().endswith('.pdf') or 'octet' in _ct_a:
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: 📄 A½ HTML ({len(_r_a.content)}b status={_r_a.status_code}): {repr(_html_contador[:400])}'})
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    # route.fetch() usa o mesmo stack de rede do browser (inclui proxy).
                    # O WAF bloqueava sem proxy (timeout/247b), mas com proxy funciona.
                    # Captura o body aqui antes que o proxy consuma o stream e save_as cancele.
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route: ContadorAcessoAto — tentando route.fetch() com proxy'})
                    try:
                        ...
                        url = resp.url
                        sz  = len(body_bytes)
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route ContadorAcessoAto: {resp.status} ct={ct[:30]} sz={sz}b url={url[:70]}'})
                        if sz > 1000 and ('pdf' in ct or 'octet' in ct or url.lower().endswith('.pdf') or body_bytes[:4] == b'%PDF'):
                            _ctx_responses.append((url, ct, body_bytes))
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ route: PDF capturado inline {sz//1024}KB'})
                        elif sz > 0:
                            # HTML — parsear JS para extrair URL real do PDF
                            _html_r = body_bytes.decode('utf-8', errors='replace')
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route HTML ContadorAcessoAto ({sz}b): {repr(_html_r[:300])}'})
                            import re as _re_r
                            _redir_r = None
                            _pats_r = [
                                'window\.open\s*\(\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]',
                                'window\.location(?:\.href)?\s*=\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]',
                                'location\.(?:replace|assign)\s*\(\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]',
                                'content=[\x27\x22][^\x27\x22]*url=([^\x27\x22> \\s]+)',
                                'href=[\x27\x22]([^\x27\x22]*(?:\.pdf|AtoDocumento|arquivo)[^\x27\x22]*)[\x27\x22]',
                            ]
                            for _pat_r in _pats_r:
                                _m_r = _re_r.search(_pat_r, _html_r, _re_r.IGNORECASE)
                                if _m_r:
                                    _redir_r = _m_r.group(1)
                                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 route JS redirect: {_redir_r[:80]}'})
                                    break
                            if _redir_r:
                                if not _redir_r.startswith('http'):
                                    from urllib.parse import urljoin as _uj_r
                                    _redir_r = _uj_r(req_url, _redir_r)
                                _ctx_responses.append((_redir_r, 'application/pdf', None))
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 route: URL PDF enfileirada: {_redir_r[:80]}'})
                        try: route.fulfill(response=resp)
                        except Exception: 
                            try: route.continue_()
                            except Exception: pass
                    except Exception as _re:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route ContadorAcessoAto fetch falhou: {str(_re)[:60]} — passando para browser'})
                        try: route.continue_()
                        except Exception: pass
                    return
                try:
            ...
            if _onclick_fn and 'MostraDocumento' in _onclick_fn:
                ...
                try:
                    ...
                    if _a0_dl[0]:
                        ...
                        _saved_a0_ok = False
                        # URL preferida: redirect capturado pelo response listener (URL real do PDF)
                        # Fallback: URL do download object (pode ser ContadorAcessoAto — já consumido)
                        _fetch_url_a0 = _a0_redirect_url[0] or _dl_url_a0
                        logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 dl.url={_dl_url_a0[:60]} redirect={(_a0_redirect_url[0] or "none")[:60]}'})
                        # Tentativa 1: save_as normal
                        try:
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 save_as falhou ({str(_sa_err)[:50]}) — tentando requests direto'})
                        # Tentativa 2: requests Python direto com proxy (não context.request que dá socket hang up)
                        if not _saved_a0_ok and _fetch_url_a0 and _fetch_url_a0.startswith('http'):
                            try:
                                import requests as _req_a0
                                _cookies_a0 = {}
                                try:
                                    for _c in page.context.cookies():
                                        _cookies_a0[_c['name']] = _c['value']
                                except Exception:
                                    pass
                                _proxies_a0 = _get_proxy_requests()
                                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 requests+proxy → {_fetch_url_a0[:80]}'})
                                _r_a0 = _req_a0.get(
                                    _fetch_url_a0,
                                    cookies=_cookies_a0,
                                    proxies=_proxies_a0,
                                ...
                                _body_a0 = _r_a0.content
                                if _body_a0[:4] == b'%PDF' and len(_body_a0) > 1000:
                                    open(_dp_a0, 'wb').write(_body_a0)
                                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — PDF via requests+proxy: {len(_body_a0)//1024}KB{_alert_a0}'})
                                    _saved_a0_ok = True
                                else:
                                    # HTML do ContadorAcessoAto — parsear URL do PDF (window.open, window.location, etc.)
                                    _html_a0 = _body_a0.decode('utf-8', errors='ignore')
                                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 requests HTML ({len(_body_a0)}b status={_r_a0.status_code}): {repr(_html_a0[:200])}'})
                                    import re as _re_a0
                                    _pdf_url_a0 = None
                                    for _pat_a0 in [
                                        r"window\.open\s*\(\s*['\"]([^'\"]+)['\"]",
                                        ...
                                        r'content=["\'][^"\']*url=([^"\'> \s]+)',
                                        r'href=["\']([^"\']*(?:\.pdf|AtoDocumento|arquivo)[^"\']*)["\']',
                                    ]:
                                    ...
                                    if _pdf_url_a0:
                                        ...
                                        try:
                                            _r_pdf_a0 = _req_a0.get(
                                                ...
                                                allow_redirects=True,
                                                headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
                                                         'Referer': _fetch_url_a0}
                                            )
                                            _body_pdf_a0 = _r_pdf_a0.content
                                            if _body_pdf_a0[:4] == b'%PDF' and len(_body_pdf_a0) > 1000:
                                                open(_dp_a0, 'wb').write(_body_pdf_a0)
                                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — PDF via parse HTML+proxy: {len(_body_pdf_a0)//1024}KB{_alert_a0}'})
                                                _saved_a0_ok = True
                                            else:
                                ...
                                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 requests falhou: {str(_cr_err)[:80]}'})
                        if _saved_a0_ok:
