...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
    ...
    try:
        ...
        if _onclick_fn:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # ESTRATÉGIA A — dupla captura: expect_download + expect_popup
            #
            # Fluxo 1 (arquivo grande, ex: LC 270/BuscaFácil):
            #   clique → alert → accept → download direto
            #   handler de dialog aceita automaticamente; expect_download captura
            #
            # Fluxo 2 (arquivo pequeno, ex: LC 198/BuscaFácil):
            #   clique → popup → redirect HTTP → route captura PDF
            #
            # Estratégia: tentar expect_download wrapping o clique primeiro.
            # Se vier popup sem download, capturar via expect_popup em fallback.
            # ═══════════════════════════════════════════════════════════════════
            _dialog_alert = [None]
            ...
            popup_page  = None
            _a_download = None  # download capturado pelo expect_download

            # ── Tentativa 1: expect_download wrapping o clique ────────────────
            # Cobre o caso alert+download. Se vier popup, o expect_download
            # vai apenas dar timeout sem efeito colateral.
            try:
                with page.expect_download(timeout=20000) as _dl_primary:
                    el.click()
                _a_download = _dl_primary.value
                _dp_primary = tempfile.mktemp(suffix='.pdf')
                _a_download.save_as(_dp_primary)
                _sz_primary = os.path.getsize(_dp_primary) // 1024
                if _dialog_alert[0]:
                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A (alert+download): {_sz_primary}KB — "{_dialog_alert[0][:60]}"'})
                else:
                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A (download direto): {_sz_primary}KB'})
                ...
                pass  # sem download — tentar popup

            # ── Tentativa 2: expect_popup (arquivo pequeno / redirect HTTP) ───
            # O clique já foi dado na tentativa 1. Se abriu popup nesse meio
            # tempo, ele está nas pages do contexto. Tentar capturá-lo.
            try:
                # Popup pode já estar aberto (disparado junto com o clique)
                ...
                if len(_all_pages) > 1:
                    ...
                    try:
                        ...
                        pass
                    logs.append({'nivel': 'ok', 'msg': f'{label}: 🖪 Popup já aberto: {(_popup_nav_urls[0] if _popup_nav_urls else "?")[:80]}'})
                else:
                    # Tentar esperar popup (re-clique necessário pois o clique anterior não abriu popup)
                    ...
                    try:
                        ...
                        pass
                    logs.append({'nivel': 'ok', 'msg': f'{label}: 🖪 Popup aberto (re-clique): {(_popup_nav_urls[0] if _popup_nav_urls else "?")[:80]}'})

                # Aguardar route capturar
                ...
                if _dialog_alert[0]:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A — alert aceito mas download/popup não chegaram: {str(e_popup_open)[:60]}'})
                else:
