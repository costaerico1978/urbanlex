...
def _executar_acao(page, frame, acao: dict, logs: list, label: str, passo: int) -> str:
    ...
    try:
        if tipo == 'preencher_e_submeter':
            ...
            # Caso 4: Nada visível — interceptar window.open via JS override
            try:
                captured_url = frame.evaluate('''(el) => {
                    const orig = window.open;
                    let capturedUrl = null;
                    window.open = function(url) { capturedUrl = url; return null; };
                    
                    let onclick = el.getAttribute('onclick') || '';
                    if (!onclick && el.parentElement) onclick = el.parentElement.getAttribute('onclick') || '';
                    if (!onclick && el.closest && el.closest('td')) onclick = el.closest('td').getAttribute('onclick') || '';
                    ...
                    if (!capturedUrl && onclick) {
                        const m = onclick.match(/['"]([^'"]*[.](?:asp|php|html|pdf|jsp)[^'"]*)['"]/i);
                        if (m) capturedUrl = m[1];
                    }
                    
                    return {
                        url: capturedUrl,
                        onclick: onclick ? onclick.substring(0, 150) : '',
                        href: el.getAttribute('href') || ''
                    };
                }''', el)
                
                logs.append({'nivel': 'info', 'msg': f'{label}: 🔧 Interceptação: url={str(captured_url.get("url",""))[:60]} onclick="{captured_url.get("onclick","")[:50]}"'})
                
                captured = captured_url.get('url', '')
                if captured:
                    from urllib.parse import urljoin
                    base = frame.url or page.url
                    full_url = urljoin(base, captured)
                    ...
                    try:
                        ...
                        try:
                            new_page.goto(full_url, wait_until='domcontentloaded', timeout=15000)
                        except Exception:
                            pass
                    time.sleep(2)
                    logs.append({'nivel': 'ok', 'msg': f'{label}: 🪟 Abriu URL capturada: {new_page.url[:60]}'})
                    return f'Popup aberto: {new_page.url}'
                    
            except Exception as e_js:
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Interceptação falhou: {str(e_js)[:60]}'})
...
def _encontrar_elemento(frame, alvo: str, href: str = None):
    ...
    # Estratégia especial: buscar link na coluna "Arquivo" de tabelas
    if not el and alvo:
        alvo_lower = alvo.lower()
        if any(k in alvo_lower for k in ['arquivo', 'coluna', 'ícone', 'icone', 'download', 'icon', 'documento']):
            # Estratégia 1: Encontrar coluna "Arquivo" pelo header e pegar link na mesma posição
            for table in frame.query_selector_all('table'):
                try:
                    headers = table.query_selector_all('th, thead td')
                    arquivo_col_idx = -1
                    for idx, th in enumerate(headers):
                        th_text = (th.text_content() or '').strip().lower()
                        if 'arquivo' in th_text or 'arq' in th_text or 'download' in th_text or 'doc' == th_text:
                            arquivo_col_idx = idx
                            break
                    
                    if arquivo_col_idx >= 0:
                        # Buscar nas linhas do corpo da tabela
                        rows = table.query_selector_all('tbody tr, tr')
                        for row in rows:
                            cells = row.query_selector_all('td')
                            if arquivo_col_idx < len(cells):
                                cell = cells[arquivo_col_idx]
            ...
            # Estratégia 2: Se não encontrou por header, buscar link com img dentro de td (última coluna de tabela de resultados)
            if not el:
                for table in frame.query_selector_all('table'):
                    rows = table.query_selector_all('tbody tr, tr')
                    for row in rows:
                        cells = row.query_selector_all('td')
                        # Buscar de trás para frente (coluna Arquivo costuma ser das últimas)
                        for cell in reversed(cells):
                            link = cell.query_selector('a')
                            if link and link.query_selector('img, svg, i'):
                                link_href = (link.get_attribute('href') or '')
                                # Pular se aponta para domínio claramente irrelevante
                                if link_href.startswith('http') and not any(d in link_href for d in ['smu', 'buscafacil', 'legislacao', 'arquivo', 'download']):
                                    continue
                                el = link
                                break
                        if el:
    ...
    # Último recurso: buscar qualquer link com ícone em tabela que não seja externo
    if not el and alvo and any(k in (alvo or '').lower() for k in ['ícone', 'icone', 'icon', 'imagem', 'img']):
        for a in frame.query_selector_all('td a'):
            if a.query_selector('img, svg, i'):
                a_href = (a.get_attribute('href') or '')
                # Só aceitar se não é link externo óbvio
                if a_href and a_href != '#' and not a_href.startswith('http'):
                    el = a
                    break
                elif a_href == '#' and a.get_attribute('onclick'):
                    el = a
                    break
