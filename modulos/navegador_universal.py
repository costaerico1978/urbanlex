            ...
            if _onclick_fn and 'MostraDocumento' in _onclick_fn:
                ...
                try:
                    ...
                    if _a0_dl[0]:
                        ...
                        # ── Netlog: ler eventos relevantes do ContadorAcessoAto ────────
                        try:
                            import json as _jnl, glob as _gnl
                            # Encontrar netlog mais recente
                            _netlogs = sorted(_gnl.glob('/tmp/buscafacil_netlog_*.json'), key=lambda f: os.path.getmtime(f), reverse=True)
                            if _netlogs:
                                _nlpath = _netlogs[0]
                                _nlsize = os.path.getsize(_nlpath)
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 NETLOG: {_nlpath} ({_nlsize//1024}KB)'})
                                with open(_nlpath, 'r', errors='ignore') as _nlf:
                                    _nldata = _jnl.load(_nlf)
                                # Filtrar eventos relevantes: URL_REQUEST, TCP, SSL, CLOSE
                                _events = _nldata.get('events', [])
                                _relevant = []
                                _contador_ids = set()
                                # 1ª passagem: achar source IDs que tocam ContadorAcessoAto
                                for _ev in _events:
                                    _params = _ev.get('params', {})
                                    _url = _params.get('url', '')
                                    if 'ContadorAcessoAto' in _url or 'ContadorAcesso' in _url:
                                        _src = _ev.get('source', {})
                                        _contador_ids.add(_src.get('id'))
                                # 2ª passagem: todos eventos desses sources
                                for _ev in _events:
                                    _src_id = _ev.get('source', {}).get('id')
                                    if _src_id in _contador_ids:
                                        _relevant.append({
                                            'type': _ev.get('type'),
                                            'phase': _ev.get('phase'),
                                            'params': {k: v for k, v in _ev.get('params', {}).items() if k in ['url','net_error','description','byte_count','headers','error','end_of_file','socket_id']}
                                        })
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 NETLOG eventos ContadorAcessoAto ({len(_relevant)} eventos): {str(_relevant[:40])}'})
                            else:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 NETLOG: nenhum arquivo encontrado'})
                        except Exception as _nl_err:
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 NETLOG erro: {str(_nl_err)[:80]}'})
                        # ───────────────────────────────────────────────────────────────

                        # ══════════════════════════════════════════════════════════════
                        # DIAGNÓSTICO COMPLETO — coletar tudo antes de tentar qualquer fix
                        # ══════════════════════════════════════════════════════════════
                        try:
                            # 1) Download object
                            _diag_url      = _a0_dl[0].url
                            _diag_failure  = _a0_dl[0].failure()
                            _diag_fname    = _a0_dl[0].suggested_filename
                            _diag_path     = None
                            _diag_size     = None
                            _diag_header4  = None
                            try:
                                _diag_path = _a0_dl[0].path()
                            except Exception as _ep:
                                _diag_path = f'ERRO:{_ep}'
                            if _diag_path and isinstance(_diag_path, str) and os.path.exists(_diag_path):
                                _diag_size = os.path.getsize(_diag_path)
                                try:
                                    with open(_diag_path, 'rb') as _f_diag:
                                        _diag_header4 = _f_diag.read(8).hex()
                                except Exception:
                                    pass
                            logs.append({'nivel': 'info', 'msg': (
                                f'{label}: 🔬 DIAG-1 download_obj | '
                                f'url={_diag_url} | '
                                f'suggested_filename={_diag_fname} | '
                                f'failure={_diag_failure} | '
                                f'path={_diag_path} | '
                                f'path_size={_diag_size}b | '
                                f'first8hex={_diag_header4}'
                            )})

                            # 2) Cookies da sessão no momento do download
                            _diag_cookies = {}
                            try:
                                _diag_cookies = {c['name']: c['value'][:20] for c in page.context.cookies()}
                            except Exception:
                                pass
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-2 cookies={list(_diag_cookies.keys())}'})

                            # 3) Respostas HTTP capturadas pelo route para ContadorAcessoAto
                            # (headers de Content-Type, Content-Disposition, Location)
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-3 redirect_url={_a0_redirect_url[0]} ctx_responses={len(_ctx_responses)}'})

                            # 4) Todas as respostas capturadas pelo route interceptor
                            for _i_cr, (_u_cr, _ct_cr, _b_cr) in enumerate(_ctx_responses):
                                _sz_cr = len(_b_cr) if _b_cr else None
                                _head_cr = _b_cr[:80].hex() if _b_cr else None
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-4 ctx_response[{_i_cr}] | '
                                    f'url={_u_cr} | ct={_ct_cr} | size={_sz_cr} | head={_head_cr}'
                                )})

                            # 5) context.request na URL do download — headers + body reais
                            try:
                                _diag_req = page.context.request.get(
                                    _diag_url, timeout=15000,
                                    headers={'Referer': 'https://www2.rio.rj.gov.br/smu/buscafacil/RelacaoDocumentos.asp'}
                                )
                                _diag_rct  = _diag_req.headers.get('content-type', '')
                                _diag_rcd  = _diag_req.headers.get('content-disposition', '')
                                _diag_rloc = _diag_req.headers.get('location', '')
                                _diag_rbody = _diag_req.body()
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-5 context.request({_diag_req.status}) | '
                                    f'ct={_diag_rct} | cd={_diag_rcd} | '
                                    f'location={_diag_rloc} | '
                                    f'body_size={len(_diag_rbody)}b | '
                                    f'body_head={_diag_rbody[:300]}'
                                )})
                            except Exception as _diag_req_err:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-5 context.request falhou: {_diag_req_err}'})

                            # Cookies corretos para requests Python
                            _diag_cookie_jar = {c['name']: c['value'] for c in page.context.cookies()}
                            _diag_headers = {
                                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36',
                                'Referer': 'https://www2.rio.rj.gov.br/smu/buscafacil/RelacaoDocumentos.asp',
                            }
                            import requests as _diag_rq

                            # 6) requests SEM proxy, sem redirect — ver primeiro hop
                            try:
                                _diag_r0 = _diag_rq.get(
                                    _diag_url, timeout=10, allow_redirects=False,
                                    cookies=_diag_cookie_jar, headers=_diag_headers
                                )
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-6 requests_no_proxy_nored({_diag_r0.status_code}) | '
                                    f'ct={_diag_r0.headers.get("content-type","")} | '
                                    f'cd={_diag_r0.headers.get("content-disposition","")} | '
                                    f'location={_diag_r0.headers.get("location","")} | '
                                    f'body={_diag_r0.content[:150]}'
                                )})
                            except Exception as _e6:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-6 falhou: {_e6}'})

                            # 7) requests SEM proxy, COM redirect — URL final + tipo
                            try:
                                _diag_r1 = _diag_rq.get(
                                    _diag_url, timeout=15, allow_redirects=True,
                                    cookies=_diag_cookie_jar, headers=_diag_headers
                                )
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-7 requests_no_proxy_red({_diag_r1.status_code}) | '
                                    f'final_url={_diag_r1.url} | '
                                    f'ct={_diag_r1.headers.get("content-type","")} | '
                                    f'cd={_diag_r1.headers.get("content-disposition","")} | '
                                    f'body_size={len(_diag_r1.content)}b | '
                                    f'body_head={_diag_r1.content[:8].hex()}'
                                )})
                            except Exception as _e7:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-7 falhou: {_e7}'})

                            # 8) requests COM proxy, COM redirect — URL final via proxy
                            try:
                                _diag_prx = _get_proxy_requests()
                                _diag_r2 = _diag_rq.get(
                                    _diag_url, timeout=20, allow_redirects=True,
                                    proxies=_diag_prx, cookies=_diag_cookie_jar, headers=_diag_headers
                                )
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-8 requests_proxy_red({_diag_r2.status_code}) | '
                                    f'final_url={_diag_r2.url} | '
                                    f'ct={_diag_r2.headers.get("content-type","")} | '
                                    f'body_size={len(_diag_r2.content)}b | '
                                    f'body_head={_diag_r2.content[:8].hex()}'
                                )})
                            except Exception as _e8:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-8 requests_proxy falhou: {_e8}'})

                            # 9) curl_cffi + proxy na URL do download — TLS Chrome + IP residencial
                            try:
                                from curl_cffi import requests as _cffi_diag
                                _cffi_s = _cffi_diag.Session(impersonate='chrome120', trust_env=False)
                                _diag_prx2 = _get_proxy_requests()
                                _diag_r3 = _cffi_s.get(
                                    _diag_url, timeout=20, allow_redirects=True,
                                    cookies=_diag_cookie_jar, headers=_diag_headers,
                                    proxies=(_diag_prx2 or {})
                                )
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-9 curl_cffi_proxy({_diag_r3.status_code}) | '
                                    f'final_url={_diag_r3.url} | '
                                    f'ct={_diag_r3.headers.get("content-type","")} | '
                                    f'body_size={len(_diag_r3.content)}b | '
                                    f'body_head={_diag_r3.content[:8].hex()}'
                                )})
                            except Exception as _e9:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG-9 curl_cffi falhou: {_e9}'})

                            # 10) Estado da página no momento do download
                            try:
                                logs.append({'nivel': 'info', 'msg': (
                                    f'{label}: 🔬 DIAG-10 page_state | '
                                    f'url={page.url} | '
                                    f'title={page.title()}'
                                )})
                            except Exception:
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔬 DIAG erro geral: {_diag_err}'})
                        # ══════════════════════════════════════════════════════════════
                        ...
                        # Tentativa 1b: downloads_path — browser salvou automaticamente no dir temp
                        if not _saved_a0_ok:
                            try:
                                import glob as _gl_a0, shutil as _sh_a0b
                                _dl_dir_a0 = page.context._impl_obj._options.get('downloads_path', '') if hasattr(page.context, '_impl_obj') else ''
                                if not _dl_dir_a0:
                                    # Tentar obter via suggested_filename no diretório padrão do browser
                                    _sfname = _a0_dl[0].suggested_filename or ''
                                    _candidate_dirs = ['/tmp', os.path.expanduser('~'), os.getcwd()]
                                    for _cd in _candidate_dirs:
                                        _candidates = _gl_a0.glob(os.path.join(_cd, '**', '*.pdf'), recursive=True)
                                        _recent = [f for f in _candidates if os.path.getmtime(f) > time.time() - 120]
                                        if _recent:
                                            _best = max(_recent, key=os.path.getmtime)
                                            _sz_best = os.path.getsize(_best)
                                            if _sz_best > 10000:
                                                _sh_a0b.copy2(_best, _dp_a0)
                                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — PDF encontrado em disco: {_best} {_sz_best//1024}KB{_alert_a0}'})
                                                _saved_a0_ok = True
                                                break
                                if _dl_dir_a0 and not _saved_a0_ok:
                                    _pdfs_dir = _gl_a0.glob(os.path.join(_dl_dir_a0, '**', '*.pdf'), recursive=True)
                                    _pdfs_dir = [f for f in _pdfs_dir if os.path.getsize(f) > 10000]
                                    if _pdfs_dir:
                                        _best = max(_pdfs_dir, key=os.path.getmtime)
                                        _sh_a0b.copy2(_best, _dp_a0)
                                        logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — downloads_path: {_sz_best//1024}KB{_alert_a0}'})
                                        _saved_a0_ok = True
                            except Exception as _dl_dir_err:
                                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 downloads_path busca falhou: {str(_dl_dir_err)[:60]}'})

                        # Tentativa 1c: save_as normal
                        if not _saved_a0_ok:
                          try:
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 save_as falhou ({str(_sa_err)[:50]}) — tentando context.request'})
