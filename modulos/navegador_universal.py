    ...
    try:
        ...
        if _onclick_fn:
            ...
            if _direct_url:
                ...
                logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 A1 — navegação direta: {_abs_url_a1[:80]}'})
                try:
                    with page.context.expect_event('download', timeout=20000) as _dl_a1:
                        page.evaluate(f"window.location.href = '{_abs_url_a1}'")
                    _dl_a1_val = _dl_a1.value
                    _dp_a1 = tempfile.mktemp(suffix='.pdf')
                    _dl_a1_val.save_as(_dp_a1)
                    _sz_a1 = os.path.getsize(_dp_a1) // 1024
                    _alert_a1 = f' — "{_dialog_a1[0][:60]}"' if _dialog_a1[0] else ''
                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A1 — download via navegação direta: {_sz_a1}KB{_alert_a1}'})
                    for _dt_a1 in (page, page.context):
                    ...
                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A1 — sem download: {str(_e_a1)[:80]}'})
                    for _dt_a1 in (page, page.context):
                        try: _dt_a1.remove_listener('dialog', _on_dialog_a1)
                        except Exception: pass

            # ═══════════════════════════════════════════════════════════════════
            # ESTRATÉGIA A — dupla captura: context-level dialog + download/popup
            #
            ...
            # Timeout 15s: click → JS → alert → accept → server → download pode
            # levar vários segundos com proxy. LC 198 não dispara download →
            # timeout silencioso → vai para popup.
            try:
                with page.context.expect_event('download', timeout=15000) as _dl_primary:
                    el.click()
