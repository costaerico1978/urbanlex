...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
    ...
    try:
        ...
        if _onclick_fn:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # ESTRATÉGIA K — VPS Brasileiro (IP limpo São Paulo)
            # Quando todas as estratégias falham por causa do proxy bloqueando
            # o download do PDF, delega para o VPS com IP residencial brasileiro.
            # ═══════════════════════════════════════════════════════════════════
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 K — VPS brasileiro'})
            _vps_url = os.environ.get('VPS_WORKER_URL', 'http://216.238.116.121:5001')
            _vps_key = os.environ.get('VPS_WORKER_KEY', 'urbanlex-worker-key-2024')
            try:
                import requests as _req_vps
                # Extrair cookies do storage_state para enviar ao VPS
                _cookies_k = []
                try:
                    if _storage_state and _storage_state.get('cookies'):
                        _cookies_k = [
                            {'name': c['name'], 'value': c['value'], 'domain': c.get('domain', ''), 'path': c.get('path', '/')}
                            for c in _storage_state['cookies']
                        ]
                except Exception:
                    pass
                _vps_resp = _req_vps.post(
                    f'{_vps_url}/baixar-pdf',
                    json={'api_key': _vps_key, 'url': final_url, 'cookies': _cookies_k},
                    timeout=90
                )
                if _vps_resp.status_code == 200:
                    _vps_data = _vps_resp.json()
                    _vps_b64 = _vps_data.get('pdf_base64', '')
                    if _vps_b64:
                        _vps_bytes = base64.b64decode(_vps_b64)
                        if len(_vps_bytes) > 500:
                            import tempfile as _tmpf_k
                            _k_path = _tmpf_k.mktemp(suffix='.pdf')
                            with open(_k_path, 'wb') as _fk:
                                _fk.write(_vps_bytes)
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ K — VPS baixou PDF: {len(_vps_bytes)}b → {_k_path}'})
                            _remove_ctx_listeners()
                            _cleanup_listeners()
                            return f'Download: {_k_path}'
                        else:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ K — VPS retornou PDF pequeno ({len(_vps_bytes)}b) — provavelmente erro'})
                    else:
                        logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ K — VPS sem pdf_base64: {str(_vps_data)[:80]}'})
                else:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ K — VPS status {_vps_resp.status_code}: {_vps_resp.text[:80]}'})
            except Exception as _e_k:
                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ K erro: {str(_e_k)[:80]}'})

            # Tudo falhou
            logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Todas as estratégias (A½→K) falharam | URL disponível para inspeção manual: {final_url[:100]}'})
            _remove_ctx_listeners()
            _cleanup_listeners()
            return f'Nova aba: {final_url}'
        else:
