...
def _executar_acao(page, frame, acao: dict, logs: list, label: str, passo: int) -> str:
    ...
    try:
        if tipo == 'preencher_e_submeter':
            ...
            # Clicar e detectar popup/navegação
            url_antes = page.url
            _popup_page = None
            ...
            try:
                ...
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Clique falhou: {str(e_click)[:60]}'})
            ...
            if _popup_page:
                try:
                    _popup_page.wait_for_load_state('networkidle', timeout=15000)
                except Exception:
                    pass
                time.sleep(1)
                logs.append({'nivel': 'ok', 'msg': f'{label}: 🪟 Nova aba: {_popup_page.url[:60]}'})
                return f'Popup aberto: {_popup_page.url}'
            ...
            # Clicar UMA VEZ e detectar o que aconteceu
            url_antes = page.url
            _popup_page = None
            _download_obj = None
            
            # Registrar listeners ANTES do clique
            def _on_popup(p):
                nonlocal _popup_page
                _popup_page = p
            ...
            try:
                el.click()
                time.sleep(4)  # Dar tempo para popup/download/navegação
            except Exception as e_click:
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Clique falhou: {str(e_click)[:60]}'})
            ...
            if _popup_page:
                try:
                    ...
                    pass
                time.sleep(2)
                popup_url = _popup_page.url
                logs.append({'nivel': 'ok', 'msg': f'{label}: 🪟 Nova aba: {popup_url[:60]}'})
                return f'Popup aberto: {popup_url}'
            
            # Caso 2: Download iniciou
            if _download_obj:
                try:
            ...
            if url_depois != url_antes:
                logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Navegou para: {url_depois[:60]}'})
                return f'Navegou para: {url_depois}'
            
            # Caso 4: Nada aconteceu visivelmente
