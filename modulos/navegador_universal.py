        ...
        if _onclick_fn:
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    ...
                    def _bg_fetch_r():
                        try:
                            _resp_bg = route.fetch(timeout=25000)
                            _ct_bg = _resp_bg.headers.get('content-type', '').lower()
                            _body_bg = _resp_bg.body()
                            _url_bg = _resp_bg.url
                            _sz_bg = len(_body_bg)
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 bg fetch ContadorAcessoAto: {_resp_bg.status} ct={_ct_bg[:25]} sz={_sz_bg}b url={_url_bg[:70]}'})
                            if _sz_bg > 1000 and (_body_bg[:4] == b'%PDF' or 'pdf' in _ct_bg or 'octet' in _ct_bg):
                                _ctx_responses.append((_url_bg, _ct_bg, _body_bg))
                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ bg fetch: PDF capturado {_sz_bg//1024}KB'})
                            elif _sz_bg > 100:
                                # HTML com window.open — parsear e guardar URL do PDF
                                import re as _re_bg
                                _html_bg = _body_bg.decode('utf-8', errors='ignore')
                                for _p_bg in [
                                    r'window\.open\s*\(\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]',
                                    r'window\.location(?:\.href)?\s*=\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]',
                                    r'href=[\x27\x22]([^\x27\x22]*(?:\.pdf|AtoDocumento)[^\x27\x22]*)[\x27\x22]',
                                ]:
                                    _m_bg = _re_bg.search(_p_bg, _html_bg, _re_bg.IGNORECASE)
                                    if _m_bg:
                                        _redir_bg = _m_bg.group(1)
                                        if not _redir_bg.startswith('http'):
                                            from urllib.parse import urljoin as _uj_bg
                                            _redir_bg = _uj_bg(_bg_url_r, _redir_bg)
                                        _ctx_responses.append((_redir_bg, 'application/pdf', None))
                                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 bg fetch: URL PDF enfileirada: {_redir_bg[:80]}'})
                                        break
                        except Exception as _e_bg:
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 bg fetch ContadorAcessoAto falhou: {str(_e_bg)[:60]}'})
                    _t_bg = _thr_r.Thread(target=_bg_fetch_r, daemon=True)
                    _t_bg.start()
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route: ContadorAcessoAto — continue_ + bg fetch'})
                    try: route.continue_()
            ...
            if _onclick_fn and 'MostraDocumento' in _onclick_fn:
                ...
                try:
                    ...
                    if _a0_dl[0]:
                        ...
                        # Aguardar bg fetch thread (máx 20s) antes de tentar save_as
                        try:
