...
def _executar_acao(page, frame, acao: dict, logs: list, label: str, passo: int) -> str:
    ...
    try:
        if tipo == 'preencher_e_submeter':
            ...
            # Tentar detectar popup após clique
            try:
                with page.expect_popup(timeout=5000) as popup_info:
                    el.click()
                popup_pg = popup_info.value
                try:
                    popup_pg.wait_for_load_state('networkidle', timeout=10000)
                except Exception:
                    pass
                time.sleep(2)
                logs.append({'nivel': 'info', 'msg': f'{label}: 🪟 Popup aberto: {popup_pg.url[:60]}'})
                return f'Popup aberto: {popup_pg.url}'
            except Exception:
                pass  # Não abriu popup — navegação normal
