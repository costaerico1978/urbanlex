    ...
    try:
        ...
        if _onclick_fn:
            ...
            def _tentar_browser_isolado(usar_proxy: bool, storage_state: dict = None) -> str | None:
                """Lança browser isolado para navegar para final_url.
                ...
                try:
                    ...
                    with _sync_pw2() as _pw2:
                        ...
                        try:
                            ...
                            # Injetar cookies mesmo com storage_state (redundância)
                            try:
                            ...
                            # Espera inteligente: até URL mudar ou download aparecer
                            _last2   = _page2.url
                            ...
                            # Ficou preso na URL inicial?
                            if _fu2 == final_url or 'ContadorAcesso' in _fu2:
                                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ Browser isolado ({modo}) preso em {_fu2[:60]} | Causa provável: servidor valida IP da sessão — cookie de proxy não aceito sem proxy'})
                            else:
