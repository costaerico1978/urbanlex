    ...
    try:
        ...
        if _onclick_fn:
            try:
                ...
                # Interceptar o conteúdo da resposta do popup ANTES de esperar load
                _popup_content = [None]
                _popup_ct = [None]
                try:
                    def _on_popup_response(response):
                        try:
                            ct = response.headers.get('content-type', '')
                            if 'pdf' in ct.lower() or response.url.lower().endswith('.pdf'):
                                _popup_ct[0] = ct
                                _popup_content[0] = response.body()
                        except Exception:
                            pass
                    popup_page.on('response', _on_popup_response)
                except Exception:
                    pass
                ...
                # Tentar pegar PDF interceptado via response listener
                if _popup_content[0] and len(_popup_content[0]) > 1000:
                    dl_path = tempfile.mktemp(suffix='.pdf')
                    with open(dl_path, 'wb') as f_pdf:
                        f_pdf.write(_popup_content[0])
                    dl_size = len(_popup_content[0]) // 1024
                    logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF interceptado do popup: {dl_size}KB'})
                    try:
                        page.remove_listener('download', _on_download)
                    except Exception:
                        pass
                ...
                # Fallback: tentar requests com proxy
                try:
                    import requests as _req_popup
                    import urllib3
                    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
                    
                    cookies = {}
                    try:
                        for c in popup_page.context.cookies():
                    ...
                    resp_popup = _req_popup.get(popup_url, headers=headers, cookies=cookies, 
                                                timeout=30, verify=False, proxies=proxies)
                    ct = resp_popup.headers.get('content-type', '').lower()
                    
                    if resp_popup.status_code == 200 and len(resp_popup.content) > 1000:
                        if 'pdf' in ct or resp_popup.content[:5] == b'%PDF-':
                            dl_path = tempfile.mktemp(suffix='.pdf')
                            with open(dl_path, 'wb') as f_pdf:
                                f_pdf.write(resp_popup.content)
                            dl_size = len(resp_popup.content) // 1024
                            logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF baixado do popup via proxy: {dl_size}KB'})
                            try:
                                page.remove_listener('download', _on_download)
                            except Exception:
                                pass
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Popup HTTP {resp_popup.status_code} ({len(resp_popup.content)} bytes)'})
                    else:
                        logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Popup HTTP {resp_popup.status_code} ({len(resp_popup.content)} bytes)'})
                except Exception as e_dl_popup:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Download do popup falhou: {str(e_dl_popup)[:60]}'})
                ...
                # Popup não abriu — tentar clique normal (pode ser navegação na mesma página)
                try:
                    el.click()
                except Exception:
                    pass
