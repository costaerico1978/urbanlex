    ...
    try:
        ...
        if _onclick_fn:
            ...
            try:
                ...
                if _dialog_alert[0]:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 💬 Alert detectado: "{_dialog_alert[0][:80]}" — tentando expect_download'})
                    try:
                        with page.expect_download(timeout=20000) as dl_alert_info:
                            pass  # download já foi disparado pelo clique anterior
                        _dl_alert = dl_alert_info.value
                        _dp_alert = tempfile.mktemp(suffix='.pdf')
                        _dl_alert.save_as(_dp_alert)
                        _sz_alert = os.path.getsize(_dp_alert) // 1024
                        logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — download via alert: {_sz_alert}KB'})
                        try: page.remove_listener('dialog', _on_dialog)
                        except Exception: pass
                        _remove_ctx_listeners()
                        _cleanup_listeners()
                        return f'Download: {_dp_alert}'
                    except Exception:
                        # Download não estava pendente — re-clicar com expect_download
                        try:
                            with page.expect_download(timeout=25000) as dl_alert_info2:
                                el.click()
                            _dl_alert2 = dl_alert_info2.value
                            _dp_alert2 = tempfile.mktemp(suffix='.pdf')
                            _dl_alert2.save_as(_dp_alert2)
                            _sz_alert2 = os.path.getsize(_dp_alert2) // 1024
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0 — download re-clique: {_sz_alert2}KB'})
                            try: page.remove_listener('dialog', _on_dialog)
                            except Exception: pass
                            _remove_ctx_listeners()
                            _cleanup_listeners()
                            return f'Download: {_dp_alert2}'
                        except Exception as _e_a0:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 — download falhou após alert: {str(_e_a0)[:60]}'})
                else:
