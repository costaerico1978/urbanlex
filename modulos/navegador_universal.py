    ...
    try:
        ...
        if _onclick_fn:
            try:
                ...
                # SEMPRE tentar baixar o conteúdo do popup (pode ser PDF inline)
                try:
                    import requests as _req_popup
                    import urllib3
                    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
                    
                    # Pegar cookies da sessão do browser
                    cookies = {}
                    try:
                    ...
                    resp_popup = _req_popup.get(popup_url, headers=headers, cookies=cookies, timeout=30, verify=False)
                    ...
                    if resp_popup.status_code == 200 and len(resp_popup.content) > 1000:
                        if 'pdf' in ct or resp_popup.content[:5] == b'%PDF-':
                            ...
                            logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 PDF baixado do popup: {dl_size}KB'})
                            try:
                                page.remove_listener('download', _on_download)
                            except Exception:
                                pass
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Popup é HTML ({len(resp_popup.text)} chars)'})
                    else:
