        ...
        if _onclick_fn:
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route: ContadorAcessoAto interceptado'})
                    try:
                        ...
                        url = resp.url
                        sz  = len(body_bytes)
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route ContadorAcessoAto → {resp.status} {ct[:30]} {sz}b → {url[:70]}'})
                ...
                try:
                    ...
                    while time.time() < _deadline_a0:
                        ...
                        try:
                            ...
                            pass
                        # Se dialog foi disparado, o download ESTÁ vindo — extender prazo
                        if _a0_dialog[0] and not getattr(_a0_dialog, '_extended', False):
                            _a0_dialog._extended = True
                            _novo_prazo = time.time() + 25
                            if _novo_prazo > _deadline_a0:
                                logs.append({'nivel': 'info', 'msg': f'{label}: ⏳ A0: dialog detectado — extendendo espera 25s'})
