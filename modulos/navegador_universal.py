...
def _executar_acao(page, acao: dict, logs: list, label: str) -> str:
    ...
    try:
        if tipo == 'clicar':
            ...
            # Mover mouse para posição ANTES de clicar
            try:
                page.mouse.move(x, y)
                time.sleep(0.3)
                
                # Desenhar crosshair na posição e tirar screenshot
                page.evaluate(f'''() => {{
                    let marker = document.createElement('div');
                    marker.id = 'nav-crosshair';
                    marker.style.cssText = 'position:fixed;left:{x-10}px;top:{y-10}px;width:20px;height:20px;border:3px solid red;border-radius:50%;z-index:999999;pointer-events:none;';
                    let dot = document.createElement('div');
                    dot.style.cssText = 'position:fixed;left:{x-3}px;top:{y-3}px;width:6px;height:6px;background:red;border-radius:50%;z-index:999999;pointer-events:none;';
                ...
                # Salvar screenshot com crosshair
                try:
                    debug_dir = '/tmp/nav_screenshots'
                    os.makedirs(debug_dir, exist_ok=True)
                    ss_bytes = page.screenshot(type='png', full_page=False)
                    # Usar label limpo para nome do arquivo
                    import re as _re
                    label_clean = _re.sub(r'[^\w\-]', '_', label)
                    ss_path = f'{debug_dir}/click_{label_clean}_norm{x_norm}x{y_norm}_px{x}x{y}.png'
            ...
            logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Clicou em ({x}, {y}) [norm: {x_norm},{y_norm}] box={acao.get("box_2d","")}'})
...
def navegar_como_humano(
    ...
    for passo in range(1, max_passos + 1):
        try:
            ...
            # 5b. Se ação precisa de coordenadas, fazer SEGUNDA chamada para localização precisa
                ...
                if elemento_desc:
                    ...
                    if resp_loc:
                        try:
                            ...
                            if box and isinstance(box, list) and len(box) == 4:
                                acao['box_2d'] = box
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🎯 Localização: box={box}'})
                        except Exception as e_loc:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Localização falhou: {str(e_loc)[:40]}'})
