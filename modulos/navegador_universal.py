...
13. LEISMUNICIPAIS.COM.BR — siga EXATAMENTE esta sequencia, sem pular passos:
    ...
    (e) Filtrar por ano: clicar em "Qualquer data" para expandir o filtro de datas. Usar acao "digitar" no campo "A partir de" com "01/01/ANO" (ANO = ano real da lei, ex: 01/01/2023). O campo "Ate" ja vem preenchido com "Proximos 12 meses" por padrao — NAO e necessario clicar nele. Apenas preencha o campo "A partir de" com "01/01/ANO" e prossiga diretamente para (f) clicando em "Pesquisar". Apos preencher os dois campos prosseguir imediatamente para (f). CRITICO: apos preencher as datas, o dropdown de tipo de ato pode exibir "1 Ato selecionado" — isso e NORMAL e significa que o tipo ja esta corretamente selecionado desde o passo (c). NAO clique nele novamente.
    (f) Clicar em "Pesquisar".
    (g) Nos resultados: SE a pagina mostrar "0 atos encontrados" ou "nenhum resultado" — tente UMA vez reformular a busca sem o filtro de ano (deixe AnoInicio e AnoFinal em branco) antes de desistir. Se ainda retornar 0 resultados, usar acao "desistir" com status "nenhum_resultado_relevante". O sistema fara o fallback automaticamente.
    Caso haja resultados, clicar SOMENTE no TITULO da legislacao (texto em vermelho). NUNCA clique em links encurtados como "http://leis.org/..." ou "bit.ly/..." — ignorar completamente.
...
12. CAMPOS COM AUTOCOMPLETE (campo de cidade/municipio): NUNCA use preencher_formulario nesses campos. Use a sequencia: (a) "digitar" no campo; (b) aguarde a lista de sugestoes aparecer (ela aparece abaixo do campo); (c) CRITICO: SEMPRE verificar o estado do municipio na lista — NUNCA clicar na primeira opcao automaticamente. Procure a opcao que contem o estado correto (ex: se busca e "Santa Cruz/RN", procure "Santa Cruz/RN" na lista e NAO "Santa Cruz/PE" ou qualquer outro estado). Se a opcao correta nao estiver visivel, use scroll no dropdown para encontra-la. Somente clique na opcao com o estado correto. Se nao aparecer lista imediatamente e a pagina mostrar "Pesquisando..." isso significa que a lista AINDA ESTA CARREGANDO — tire outro screenshot para ver se ela ja apareceu. Tente tambem digitar so as primeiras 3 letras para forcar o autocomplete. O sistema REJEITA o municipio se voce nao SELECIONAR da lista — digitar sem clicar na sugestao nunca funciona. Somente desista se apos multiplas tentativas a lista realmente nao aparecer.
...
22. RECUPERACAO APOS ERRO DE NAVEGACAO: Se apos um erro (timeout, campo nao encontrado, scroll sem resultado) a pagina atual nao mostrar mais o formulario de busca que estava sendo usado, use a acao "navegar" para voltar a URL inicial do portal (ex: https://leismunicipais.com.br para o LeisMunicipais, ou a URL raiz do portal atual) e recomecar o processo de busca do inicio. NAO fique em loop de scroll tentando achar um formulario que nao esta mais na pagina.
            ...
            if resultado.get('encontrada') and resultado.get('url'):
                try:
                    ...
                    if _html_sessao and len(_html_sessao) > 10000 and not _lm_loading:
                        # Usar HTML da sessão se for maior (mais conteúdo carregado)
                        if not resultado.get('html') or len(_html_sessao) > len(resultado.get('html', '')):
