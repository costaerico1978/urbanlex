...
def _add_rulers(screenshot_b64: str) -> str:
    """
    Adiciona réguas numeradas (0-100) nas bordas do screenshot.
    A IA usa esses números para indicar posições com precisão.
    """
    ...
    # Régua X (topo) — marcas a cada 5, números a cada 10
    for i in range(0, 101, 5):
        x = int(i * w / 100)
        if x >= w:
            x = w - 1
        if i % 10 == 0:
            draw.line([(x, 0), (x, 18)], fill='red', width=2)
            draw.text((x + 2, 1), str(i), fill='red')
        else:
            draw.line([(x, 0), (x, 10)], fill='red', width=1)
            draw.text((x + 1, 1), str(i), fill='red')
    
    # Régua Y (esquerda) — marcas a cada 5, números a cada 10
    for j in range(0, 101, 5):
        y = int(j * h / 100)
        if y >= h:
            y = h - 1
        if j % 10 == 0:
            draw.line([(0, y), (18, y)], fill='red', width=2)
            draw.text((1, y + 1), str(j), fill='red')
        else:
            draw.line([(0, y), (10, y)], fill='red', width=1)
...
Olhe o screenshot. Ha REGUAS VERMELHAS nas bordas com numeros de 0 a 100.
Use essas reguas para informar a posicao EXATA dos elementos.

ACOES:
- "clicar": informe rx (posicao na regua X, 0-100) e ry (posicao na regua Y, 0-100)
- "digitar": informe rx, ry do campo + texto a digitar
...
11. COORDENADAS: olhe as reguas vermelhas. Informe rx (horizontal, 0=esquerda, 100=direita) e ry (vertical, 0=topo, 100=rodape). Exemplo: se o botao esta alinhado com o numero 72 na regua de cima e o numero 35 na regua da esquerda, informe rx=72 e ry=35.
...
def _executar_acao(page, acao: dict, logs: list, label: str) -> str:
    ...
    # Converter coordenadas de regua (0-100) para pixels
        ...
        if tipo == 'clicar':
            ...
            try:
                ...
                try:
                    ...
                    ss_path = f'{debug_dir}/click_{label_clean}_rx{int(rx)}ry{int(ry)}_px{x}x{y}.png'
            ...
            logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Clicou em ({x}, {y}) [rx={rx:.0f}, ry={ry:.0f}]'})
    ...
    for passo in range(1, max_passos + 1):
        try:
            ...
            if len(historico) >= 2:
                ...
                if all(t == tipo_acao for t in tipos_recentes):
                    ...
                    if len(set(resultados_recentes)) == 1:
                        ...
                        if tipo_acao == 'clicar':
                            dx = int(float(acao.get('rx', 0) or 0) * VIEWPORT_W / 100)
                            dy = int(float(acao.get('ry', 0) or 0) * VIEWPORT_H / 100)
                            if dx > 0 and dy > 0:
            ...
            if tipo_acao in ('clicar', 'digitar', 'selecionar'):
                rx_val = acao.get('rx', '')
                ry_val = acao.get('ry', '')
                coord_info = f' rx={rx_val},ry={ry_val}'
