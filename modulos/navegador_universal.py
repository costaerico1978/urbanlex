            ...
            if _onclick_fn and 'MostraDocumento' in _onclick_fn:
                ...
                try:
                    ...
                    if _a0_dl[0]:
                        ...
                        # Tentativa 2: context.request (tem proxy + cookies da sessão)
                        if not _saved_a0_ok and _dl_url_a0 and _dl_url_a0.startswith('http'):
                            try:
                                logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 buscando via context.request: {_dl_url_a0[:80]}'})
                                _ctx_resp_a0 = page.context.request.get(_dl_url_a0, timeout=60000)
                                ...
                                if _body_a0[:4] == b'%PDF' and len(_body_a0) > 1000:
                                    ...
                                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 context.request retornou não-PDF ({len(_body_a0)}b)'})
