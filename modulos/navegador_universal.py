        ...
        if _onclick_fn:
            ...
            def _route_interceptor(route):
                ...
                if 'ContadorAcessoAto' in req_url or 'ContadorAcesso' in req_url:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route: ContadorAcessoAto interceptado — passando para browser+proxy'})
                    # NÃO usar route.fetch() — o WAF bloqueia requisições diretas do Railway.
                    # route.continue_() deixa o browser (com proxy) seguir o redirect
                    # naturalmente e disparar o evento de download que _on_dl_a0 escuta.
                    try: route.continue_()
                    except Exception: pass
                    return
                if False:  # bloco desativado
                    try:
                        ...
                        if 'pdf' in ct or 'octet' in ct or url.lower().endswith('.pdf'):
                            ...
                            # HTML — parsear JS para extrair URL do PDF
                            ...
                            for _pat_r in [
                                ...
                                r"""href=['"]([^'"]*(?:\.pdf|download|arquivo)[^'"]*)['"]\s""",
                            ]:
                                _m_r = _re_r.search(_pat_r, _html_r)
                                if _m_r:
                                    _redir_r = _m_r.group(1)
                                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 route JS redirect encontrado: {_redir_r[:80]}'})
                                    break
                            if _redir_r:
                                if not _redir_r.startswith('http'):
                                    from urllib.parse import urljoin as _uj_r
                                    _redir_r = _uj_r(req_url, _redir_r)
                                # Guardar para A0 buscar depois do clique
                                _ctx_responses.append((_redir_r, 'application/pdf', None))
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 route: URL real enfileirada para fetch: {_redir_r[:80]}'})
                        route.fulfill(response=resp)
                    except Exception as _re:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔀 route ContadorAcessoAto fetch falhou: {str(_re)[:60]}'})
                        try: route.continue_()
                        except Exception: pass
                    return
                try:
