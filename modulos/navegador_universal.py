...
13. LEISMUNICIPAIS.COM.BR — siga EXATAMENTE esta sequencia, sem pular passos:
    ...
    (e) Filtrar por ano: clicar em "Qualquer data" para expandir o filtro de datas. Usar acao "digitar" no campo "A partir de" com "01/01/ANO" (ANO = ano real da lei, ex: 01/01/2023). Em seguida OBRIGATORIAMENTE usar preencher_formulario no campo "Ate" com tipo_campo "select" e valor "Proximos 12 meses" — MESMO QUE O CAMPO JA MOSTRE ESTE TEXTO, voce DEVE executar esta acao para garantir que o valor esta realmente selecionado. Apos preencher os dois campos prosseguir imediatamente para (f). CRITICO: apos preencher as datas, o dropdown de tipo de ato pode exibir "1 Ato selecionado" — isso e NORMAL e significa que o tipo ja esta corretamente selecionado desde o passo (c). NAO clique nele novamente.
