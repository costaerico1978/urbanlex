...
11. RANGE DE DATAS: se o formulario tem campos "Data Inicial" e "Data Final" e NAO e um diario oficial, SEMPRE preencha Data Inicial com a data de publicacao e Data Final com a data + 5 dias. Exemplo: se data = 15/01/2019, entao Data Inicial = "15/01/2019" e Data Final = "20/01/2019". NUNCA coloque a mesma data nos dois campos.
...
{{{{
    ...
    "acao": {{{{
        ...
        "campos": [
            ...
            {{{{"label": "Data Final", "valor": "19/01/2019", "tipo_campo": "date"}}}}
        ],
...
- Preencha o array "campos" com TODOS os campos visiveis no formulario.
...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
        ...
        if href_extraido:
            ...
            if href_extraido.startswith('/'):
                try:
                    ...
                    pass
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 Href extraído: {(href_extraido or "")[:60]}'})
    except Exception:
        pass
    ...
    try:
        ...
        # Browser pode ter morrido — tentar recuperar com href
        if href_extraido and href_extraido.startswith('http'):
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 Browser morreu — retornando href extraído'})
            return f'Navegou: {href_extraido}'
    ...
    try:
        ...
        # Browser morreu mas temos href
        if href_extraido and href_extraido.startswith('http'):
            logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 Browser morreu — retornando href extraído'})
            return f'Navegou: {href_extraido}'
    
    # Se nada mudou mas temos href, informar
    if href_extraido and href_extraido.startswith('http'):
