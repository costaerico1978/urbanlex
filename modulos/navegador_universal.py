    ...
    try:
        ...
        if _onclick_fn:
            try:
                ...
                # Detectar se o popup é um PDF (inline ou download)
                _is_pdf = False
                try:
                    ...
                    pass
                if not _is_pdf and ('pdf' in popup_url.lower() or 'contador' in popup_url.lower() or 'documento' in popup_url.lower()):
                    # Verificar se parece PDF pelo título ou conteúdo
                    try:
                        title = popup_page.title() or ''
                        if '.pdf' in title.lower() or len((popup_page.inner_text('body') or '').strip()) < 50:
                            _is_pdf = True
                    except Exception:
                        pass
                
                if _is_pdf:
                    # Baixar o PDF direto da URL do popup
                    logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Popup é PDF — baixando...'})
                    try:
                        ...
                        resp_pdf = _req_popup.get(popup_url, headers=headers, timeout=30, verify=False)
                        if resp_pdf.status_code == 200 and len(resp_pdf.content) > 1000:
                            dl_path = tempfile.mktemp(suffix='.pdf')
                            with open(dl_path, 'wb') as f_pdf:
                                f_pdf.write(resp_pdf.content)
                            dl_size = len(resp_pdf.content) // 1024
                        ...
                        logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Download do popup falhou: {str(e_dl_popup)[:40]}'})
