...
def _clicar_por_texto(page, texto_elemento: str, logs: list, label: str) -> str:
    ...
    try:
        ...
        if _onclick_fn:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # ESTRATÉGIA A0 — alert + download direto (arquivo grande)
            # Alguns sites (ex: BuscaFácil) mostram window.alert() antes de
            # disparar o download diretamente na página — sem popup.
            # Registrar handler de dialog ANTES do clique para aceitar o alert.
            # ═══════════════════════════════════════════════════════════════════
            _dialog_alert = [None]
            ...
            # ═══════════════════════════════════════════════════════════════════
            # ESTRATÉGIA A — expect_popup + route (arquivo pequeno/médio)
            ...
            try:
                ...
                # expect_popup falhou — verificar se foi alert+download (arquivo grande)
                if _dialog_alert[0]:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 💬 Alert detectado: "{_dialog_alert[0][:80]}" — tentando popup+download'})
                    _a0_saved = None

                    # A0a: popup abriu após alert?
                    try:
                        with page.expect_popup(timeout=8000) as _a0_popup_info:
                            pass
                        _a0_popup = _a0_popup_info.value
                        _deadline_a0 = time.time() + 10
                        while time.time() < _deadline_a0:
                            if _ctx_responses:
                                break
                            time.sleep(0.3)
                        for _cap_url, _cap_ct, _cap_body in _ctx_responses:
                            if 'pdf' in _cap_ct or _cap_url.lower().endswith('.pdf') or 'octet' in _cap_ct:
                                _a0_saved = _salvar_corpo(_cap_url, _cap_ct, _cap_body)
                                if _a0_saved:
                                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0a — route capturou PDF após alert+popup'})
                                    break
                        if not _a0_saved:
                            try:
                                _a0_dl = _a0_popup.wait_for_event('download', timeout=10000)
                                _a0_dp = tempfile.mktemp(suffix='.pdf')
                                _a0_dl.save_as(_a0_dp)
                                _a0_saved = _a0_dp
                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0a — download do popup após alert'})
                            except Exception:
                                pass
                    except Exception:
                        pass

                    # A0b: download direto na página (sem popup, já disparado)
                    if not _a0_saved:
                        try:
                            with page.expect_download(timeout=15000) as _dl_a0b:
                                pass
                            _dp_a0b = tempfile.mktemp(suffix='.pdf')
                            _dl_a0b.value.save_as(_dp_a0b)
                            _a0_saved = _dp_a0b
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0b — download direto após alert: {os.path.getsize(_dp_a0b)//1024}KB'})
                        except Exception:
                            pass

                    # A0c: re-clicar com expect_download
                    if not _a0_saved:
                        try:
                            with page.expect_download(timeout=25000) as _dl_a0c:
                                el.click()
                            _dp_a0c = tempfile.mktemp(suffix='.pdf')
                            _dl_a0c.value.save_as(_dp_a0c)
                            _a0_saved = _dp_a0c
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ A0c — download re-clique: {os.path.getsize(_dp_a0c)//1024}KB'})
                        except Exception as _e_a0:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A0 — todas tentativas após alert falharam: {str(_e_a0)[:60]}'})

                    if _a0_saved:
                        try: page.remove_listener('dialog', _on_dialog)
                        except Exception: pass
                        _remove_ctx_listeners()
                        _cleanup_listeners()
                        return f'Download: {_a0_saved}'
                else:
                    logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ A — expect_popup falhou: {str(e_popup_open)[:80]} | Causa provável: proxy encerrou contexto ao abrir segunda conexão'})
                time.sleep(4)
            ...
            for cap_url, cap_ct, cap_body in _ctx_responses:
                if cap_body and len(cap_body) > 300:
                    saved = _salvar_corpo(cap_url, cap_ct, cap_body)
                    if saved:
                        logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ F — HTML do route'})
                        _remove_ctx_listeners()
                        _cleanup_listeners()
            ...
            try:
                ...
                if _fetch_result and _fetch_result.get('ok'):
                    ...
                    if _size_h > 500:
                        ...
                        if _saved_h:
                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ H — fetch hijack: {_size_h//1024}KB'})
                            _remove_ctx_listeners()
                            _cleanup_listeners()
                            return f'Download: {_saved_h}' if _saved_h.endswith('.pdf') else f'Nova aba: {_url_h}'
                        else:
                            logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ H — fetch retornou {_size_h}b mas não PDF | ct={_ct_h[:40]}'})
                    else:
                        logs.append({'nivel': 'info', 'msg': f'{label}: ℹ️ H — fetch retornou vazio ({_size_h}b) | Causa: servidor exige sessão de popup — fetch sem popup é bloqueado por política do servidor'})
                else:
