...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        with sync_playwright() as pw:
            ...
            prompt = f"""Imagine que você é uma pessoa sentada na frente do computador, olhando para esta página web.
Seu objetivo: encontrar o texto integral da {descricao} do município de {municipio}.

PÁGINA ABERTA:
- Título: {titulo_pagina}
- URL: {url_atual}

TEXTO VISÍVEL NA PÁGINA (menus, labels, conteúdo):
{texto_visivel[:2500]}

LINKS E MENUS VISÍVEIS:
...
PENSE COMO UM HUMANO:
- Olhe para a página inteira. O que ela oferece? É um portal, um buscador, uma página de resultados?
- Existe algum campo de busca? Mesmo que seja só UMA barra de pesquisa genérica?
- Existe algum menu ou link que leve a uma busca de legislação?
- Qual seria a maneira mais natural de buscar essa legislação neste site?

EXEMPLOS DE AÇÃO:
- Se existe uma barra de busca (mesmo genérica, mesmo 1 campo só): digite "{tipo_lei or 'Lei Complementar'} {numero_lei} {municipio}" e submeta
- Se existe formulário com campo de tipo + número: preencha os campos adequados
- Se existe formulário com campo de data: use SEMPRE formato brasileiro dd/mm/aaaa (ex: {data_pub_fmt or '14/01/2019'}). NUNCA use formato ISO yyyy-mm-dd.
- IMPORTANTE: datas em sites brasileiros são SEMPRE dd/mm/aaaa.
- Se o formulário tem campos de DATA INICIAL e DATA FINAL (período de busca): use {data_pub_fmt or '14/01/2019'} como data inicial e {data_final_str or '21/01/2019'} como data final. NUNCA coloque a mesma data nos dois campos — use um intervalo de 7 dias.
- Se a página tem campo type="search" ou placeholder com "Buscar", "Pesquisar": USE-O

REGRAS:
- Para SELECT: LEIA a lista de options na seção SELECTS acima. Encontre a option cujo TEXTO corresponde ao que você quer, e use o VALUE dessa option. NUNCA chute valores — sempre consulte as options disponíveis.
  Exemplo: se as options são [{{"value":"3","text":"Municipal"}}, {{"value":"1","text":"Federal"}}], e você quer Municipal, use "3" (não "1").
- NUNCA confunda "número da edição" com "número da lei"
- Se não há botão submit visível, coloque "ENTER" no botao_submit
- Preencha o MÍNIMO necessário — menos filtros = mais resultados
- Se tem QUALQUER campo de busca utilizável, confiança deve ser >= 0.7

Responda APENAS com JSON:
{{
  "campos": [
    {{"seletor": "#id_ou_seletor_css", "tipo": "input|select|textarea", "valor": "valor a digitar"}}
  ],
  "botao_submit": "#seletor_do_botao ou ENTER",
  "confianca": 0.0 a 1.0,
  "estrategia": "o que você faria como humano nesta página"
}}"""
