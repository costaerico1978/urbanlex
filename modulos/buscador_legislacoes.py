...
Se não encontrar, deixe "". NÃO invente."""
    ...
    if not _data_confirmada and _tem_do:
        ...
        for _i1b, _furl1b in enumerate(fontes_prioritarias):
            ...
            _nav1b = _navegar_formulario_com_ia(
                _furl1b, tipo_inferido or tipo, numero, ano,
                '',  # sem data — buscando justamente ela
                logs, _lbl1b, municipio=municipio
            )
            _fontes_ja_processadas.add(_i1b)

            if _nav1b:
                for _r1b in _nav1b[:3]:
                    _texto1b = _r1b.get('_texto_direto', '') or ''
                    # Tentar extrair data do título/metadados do resultado
                    _titulo1b = _r1b.get('titulo', '') or ''
                    _data1b = _extrair_data_do_texto(_titulo1b + ' ' + _texto1b[:2000])
                    if _data1b:
                        data_descoberta = _data1b
                        _data_confirmada = True
                        logs.append({'nivel': 'ok', 'msg': f'📅 1B: Data confirmada em {_dom1b}: {data_descoberta}'})
                        # Guardar resultado para ETAPA 2 não precisar rebuscar
                        _r1b['_fonte'] = 'site_informado'
                        _r1b['_label_etapa2'] = _lbl1b.replace(' [1B]', '')
                        break
            if _data_confirmada:
                break
