...
def _busca_manual_core(params, logs, fontes_status):
    ...
    if not data_descoberta or not tipo_inferido:
        # PASSO 1: Perguntar direto ao Gemini
        esfera_txt = esfera or 'municipal'
        if esfera_txt.lower() == 'municipal':
            local_txt = f'Município de {municipio}, {estado}' if municipio else estado
        elif esfera_txt.lower() == 'estadual':
            local_txt = f'Estado de {estado}' if estado else ''
        else:
            local_txt = 'esfera Federal'
        
        prompt_direto = f"""Qual é a data de publicação da {tipo or 'legislação'} nº {numero}/{ano}, {'da ' + local_txt if 'esfera' in local_txt else 'do ' + local_txt}?

Responda APENAS com JSON:
{{
    "tipo": "Lei Complementar|Lei Ordinária|Decreto|Resolução|...",
    "data_publicacao": "AAAA-MM-DD",
    "ementa_resumida": "breve descrição do assunto"
}}

Se não souber com certeza, deixe os campos vazios (""). NÃO invente."""

        resp_direto = _chamar_llm(prompt_direto, logs, '🔍 Gemini direto')
        
        if resp_direto:
            try:
                resp_direto = re.sub(r'^```(?:json)?\s*|\s*```$', '', resp_direto.strip())
                info_direta = json.loads(resp_direto)
                
                if not tipo_inferido and info_direta.get('tipo'):
                    tipo_inferido = info_direta['tipo']
                    logs.append({'nivel': 'ok', 'msg': f'📋 Tipo: {tipo_inferido}'})
                
                if not data_descoberta and info_direta.get('data_publicacao'):
                    _data_cand = info_direta['data_publicacao']
                ...
                if info_direta.get('ementa_resumida'):
                    logs.append({'nivel': 'info', 'msg': f'📝 Assunto: {info_direta["ementa_resumida"][:100]}'})
            except (json.JSONDecodeError, ValueError):
                pass
        
        # PASSO 2: Se Gemini não soube, buscar na web como fallback
        if not data_descoberta:
            logs.append({'nivel': 'info', 'msg': '🔍 Gemini não soube a data — buscando na web...'})
            
            query_fallback = f'"{tipo or "lei"}" "{numero}" "{ano}" {municipio} {estado} data publicação'.strip()
            snippets_web = _pesquisar_web(query_fallback, logs, '🔍 Descoberta', max_results=5)
