...
def _busca_manual_core(params, logs, fontes_status):
    ...
    if not data_descoberta or not tipo_inferido:
        # Pedir para a IA montar a melhor query de busca
        info_disponivel = []
        if tipo: info_disponivel.append(f'Tipo: {tipo}')
        if numero: info_disponivel.append(f'Número: {numero}')
        if ano: info_disponivel.append(f'Ano: {ano}')
        if municipio: info_disponivel.append(f'Município: {municipio}')
        if estado: info_disponivel.append(f'Estado: {estado}')
        if data_pub: info_disponivel.append(f'Data: {data_pub}')
        
        prompt_query = f"""Monte a melhor query de busca no Google/DuckDuckGo para encontrar esta legislação brasileira e descobrir sua data de publicação.

Informações disponíveis:
{chr(10).join(info_disponivel)}

Responda APENAS com JSON:
{{
    "queries": ["query 1 mais específica com aspas", "query 2 alternativa"]
}}

Use aspas duplas nos termos importantes. Inclua o tipo completo (ex: "Lei Complementar", não "lei").
Máximo 2 queries. NÃO invente dados."""

        resp_queries = _chamar_llm(prompt_query, logs, '🔍 Query IA')
        
        queries_busca = []
        if resp_queries:
            try:
                resp_queries = re.sub(r'^```(?:json)?\s*|\s*```$', '', resp_queries.strip())
                q_info = json.loads(resp_queries)
                queries_busca = q_info.get('queries', [])[:2]
            except (json.JSONDecodeError, ValueError):
                pass
        
        # Fallback: query simples se IA falhou
        if not queries_busca:
            queries_busca = [f'"{tipo or "lei"}" "{numero}" "{ano}" {municipio} {estado} data publicação'.strip()]
        
        for query_descoberta in queries_busca:
...
Se não encontrar, deixe "". NÃO invente."""
                ...
                # Se encontrou data, não precisa tentar outra query
                if data_descoberta:
                    break
