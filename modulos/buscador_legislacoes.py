...
def _busca_manual_core(params, logs, fontes_status):
    ...
    if not data_descoberta or not tipo_inferido:
        tipo_query = tipo or ''
        # Se não tem tipo, adicionar contexto legislativo pra não retornar lixo
        if not tipo_query:
            tipo_query = 'lei decreto legislação'
        query_descoberta = f'{tipo_query} {numero} {ano} {municipio} {estado} data publicação'.strip()
