...
{{
    ...
    # Tentar Gemini (com e sem grounding)
    texto = None
    logs.append({'nivel': 'info', 'msg': 'Tentando Gemini com Google Search grounding...'})
    try:
        from google.generativeai.types import Tool
        search_tool = Tool(google_search={})
        response = model.generate_content(prompt, tools=[search_tool])
        texto = response.text.strip()
        logs.append({'nivel': 'ok', 'msg': f'Gemini com grounding respondeu: {len(texto)} chars'})
    except Exception as e:
        logs.append({'nivel': 'aviso', 'msg': f'Grounding falhou: {str(e)[:120]}'})

    if texto is None:
        logs.append({'nivel': 'info', 'msg': 'Tentando Gemini sem grounding...'})
        try:
    ...
    if texto is None and GROQ_API_KEY:
        logs.append({'nivel': 'info', 'msg': '🔄 Gemini indisponível. Tentando GROQ como fallback...'})
        try:
            from groq import Groq
            client = Groq(api_key=GROQ_API_KEY)
            logs.append({'nivel': 'ok', 'msg': 'GROQ client criado (llama-3.3-70b-versatile)'})
            chat = client.chat.completions.create(
                model='llama-3.3-70b-versatile',
                messages=[
                    {'role': 'system', 'content': 'Você é um especialista em legislação urbanística brasileira. Responda APENAS com JSON válido, sem markdown, sem texto adicional.'},
            ...
            logs.append({'nivel': 'erro', 'msg': 'Biblioteca groq não instalada'})
        except Exception as e:
            logs.append({'nivel': 'erro', 'msg': f'GROQ falhou: {str(e)[:200]}'})
    elif texto is None:
        logs.append({'nivel': 'erro', 'msg': 'Gemini falhou e GROQ_API_KEY não configurada'})

    if not texto:
        return {'legislacoes': [], 'erro': 'Nenhum modelo de IA respondeu. Verifique quota/billing.', 'logs': logs}
