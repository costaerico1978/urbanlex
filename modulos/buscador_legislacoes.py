...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        with sync_playwright() as pw:
            ...
            # 2A) Esperar SPA renderizar — polling até aparecerem campos de formulário
            for _spa_wait in range(6):  # até 6 segundos extras
            ...
            # 2B) Detectar e entrar em IFRAMES (comum em sites ASP clássicos)
            # Se a página principal tem pouco conteúdo mas tem iframes, navegar pra dentro
            frames = page.frames
            target_frame = page  # padrão: frame principal
            if len(frames) > 1:
                logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Página tem {len(frames)} frames — procurando formulário...'})
                for frame in frames:
                    if frame == page.main_frame:
                        continue
                    try:
                        # Verificar se este frame tem formulário
                        ...
                        if has_form > 0:
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Formulário encontrado em frame ({has_form} campos): {frame_url[:60]}'})
                            target_frame = frame
                            break
                    except Exception:
                        continue

            # Também checar se há framesets — se o body só tem <frameset>, navegar direto
            has_frameset = page.evaluate('''() => {
                const fs = document.querySelector('frameset, iframe');
                if (!fs) return null;
                if (fs.tagName === 'IFRAME') return fs.src || null;
                // frameset: pegar o frame principal (geralmente o maior ou o que tem form)
                const frames = document.querySelectorAll('frame');
                for (const f of frames) {
                    const name = (f.name || '').toLowerCase();
                    if (name.includes('main') || name.includes('conteudo') || name.includes('content') || name.includes('corpo')) {
                        return f.src || null;
                    }
                }
                // Fallback: último frame (geralmente o de conteúdo)
                return frames.length > 0 ? frames[frames.length - 1].src : null;
            }''')

            if has_frameset and target_frame == page:
                from urllib.parse import urljoin
                frame_url_full = urljoin(url_base, has_frameset)
                logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Frameset detectado — navegando pra {frame_url_full[:60]}'})
                try:
                    page.goto(frame_url_full, wait_until='networkidle', timeout=15000)
                    _time.sleep(1)
                    target_frame = page
                except Exception as ef:
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Não conseguiu abrir frame: {str(ef)[:60]}'})
            ...
            if not has_form:
                logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Página não tem formulário de busca (0 campos após {_spa_wait+1}s)'})
                browser.close()
                return []
...
Se não encontrar, deixe "". NÃO invente."""
        ...
        if nav_results:
            ...
            for nav_r in nav_results[:5]:
                if nav_r.get('_texto_direto'):
                    ...
                    num = (numero or '').strip()
                    if num and num in re.sub(r'\s+', ' ', texto_nav.lower()):
                        textos_extraidos.append({
                            'url': nav_r['url'], 'texto': texto_nav,
                            'nome': site['label'], 'relevancia': 0.8,
                            '_fonte': site['tipo_fonte'],
                        })
                        fontes_status.append({'nome': site['label'], 'url': nav_r['url'], 'encontrou': True})
                        encontrou_site = True
                        logs.append({'nivel': 'ok', 'msg': f'{site["label"]}: ✅ Conteúdo direto!'})
                        break
                    else:
                        logs.append({'nivel': 'aviso', 'msg': f'{site["label"]}: resultado inline sem "{num}" — pulando'})
                        continue

                result = _acessar_pagina(
                    nav_r['url'], termos_busca, headers_http, logs, site['label'],
                    tipo_lei=tipo_inferido or tipo, numero_lei=numero
                )
                if result:
    ...
    # 3C: DuckDuckGo no DO (se Playwright falhou)
    if url_diario and not any(f.get('encontrou') and '🏛️' in f.get('nome','') for f in fontes_status):
        if data_descoberta:
            logs.append({'nivel': 'info', 'msg': '🏛️ DO: formulário falhou — DuckDuckGo por data...'})
