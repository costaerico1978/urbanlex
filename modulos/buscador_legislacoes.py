...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        with sync_playwright() as pw:
            ...
            prompt = f"""Você é um assistente que navega sites de legislação municipal.
Precisa encontrar: {descricao}
Município: {municipio}
...
COMO PREENCHER — siga esta lógica:

1. IDENTIFIQUE o tipo de formulário:
   - Busca por número/tipo de ato → preencher TIPO + NÚMERO (sem ano, sem data)
   - Busca por texto/palavra-chave → usar "{tipo_lei or 'Lei Complementar'} {numero_lei}"
   - Busca por data/período → preencher data inicial: {data_pub_fmt or 'não disponível'}

2. REGRAS IMPORTANTES:
   - NUNCA preencha campo de "ano" ou "número da edição" com o número da lei
   - Para SELECT de tipo de ato: use o VALUE da option, não o texto
   - Observe o PLACEHOLDER de cada campo — ele mostra o formato esperado
   - Preencha o MÍNIMO de campos necessários (menos filtros = mais resultados)
   - Se tem campo de número do ato E campo de tipo, preencha os dois
   - Se só tem campo de busca textual, use: "{tipo_lei or ''} {numero_lei}"

Responda APENAS com JSON:
{{
  "campos": [
    {{"seletor": "#id_ou_seletor_css", "tipo": "input|select|textarea", "valor": "valor"}}
  ],
  "botao_submit": "#seletor_do_botao",
  "confianca": 0.0 a 1.0,
