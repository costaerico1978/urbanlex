...
- Para SELECT: encontre a option cujo TEXTO corresponde ao dado da legislacao e use o VALUE dela.
  CRITICO: NAO assuma valores. LEIA as options listadas na secao SELECTS acima.
  Exemplo: se options sao [{{"value":"3","text":"Municipal"}}, {{"value":"1","text":"Federal"}}],
  e a esfera buscada e Municipal, o valor correto e "3" (nao "1").
...
{{
  ...
  "estrategia": "resumo do que fez"
            ...
            # Auto-corrigir selects: verificar se o valor da IA bate com as options reais
            campos_ia = instrucoes.get('campos', [])
            _mapa_esperado = {}
            if municipio:
                _mapa_esperado['esfera'] = 'municipal'
            if tipo_lei:
                _mapa_esperado['tipo'] = tipo_lei.lower()
            
            for campo_ia in campos_ia:
                sel_id = campo_ia.get('seletor', '')
                if campo_ia.get('tipo') != 'select':
                    continue
                # Encontrar o select correspondente nos selects_visiveis
                for sv in selects_visiveis:
                    sv_sel = f"#{sv.get('id', '')}" if sv.get('id') else f"[name='{sv.get('name', '')}']"
                    if sv_sel != sel_id:
                        continue
                    # Verificar se o label indica esfera
                    sv_label = (sv.get('label', '') or sv.get('name', '') or sv.get('id', '')).lower()
                    options = sv.get('options', [])
                    
                    if 'esfera' in sv_label and _mapa_esperado.get('esfera'):
                        # Buscar option com texto "Municipal"
                        for opt in options:
                            if 'municipal' in (opt.get('text', '') or '').lower():
                                if campo_ia['valor'] != opt['value']:
                                    logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Corrigindo {sel_id}: IA escolheu "{campo_ia["valor"]}" mas Municipal é "{opt["value"]}"'})
                                    campo_ia['valor'] = opt['value']
                                break
                    
                    elif ('tipo' in sv_label or 'ato' in sv_label) and tipo_lei:
                        # Buscar option cujo texto contenha o tipo
                        tipo_lower = tipo_lei.lower()
                        for opt in options:
                            opt_text = (opt.get('text', '') or '').lower()
                            if tipo_lower in opt_text or opt_text in tipo_lower:
                                if campo_ia['valor'] != opt['value']:
                                    logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Corrigindo {sel_id}: IA escolheu "{campo_ia["valor"]}" mas {tipo_lei} é "{opt["value"]}"'})
                                    campo_ia['valor'] = opt['value']
                                break
...
{{
    ...
    "texto_inicio": "primeiros 200 chars do texto da lei (se encontrou)"
                    ...
                    logs.append({'nivel': 'aviso', 'msg': f'{label}: Nenhum resultado no formulário'})
