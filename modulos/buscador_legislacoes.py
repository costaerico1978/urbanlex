        ...
        if 'pdf' in content_type or url.lower().endswith('.pdf') or '/download/' in url.lower():
            ...
            try:
                ...
                if numero_lei and numero_lei.strip() and total_pages > 20:
                    ...
                    # PASSO 1: Encontrar a página onde a lei COMEÇA
                    ...
                    for i in range(total_pages):
                        ...
                        if tipo_lei:
                            ...
                            for tc in tipos_check:
                                ...
                                sep = r'(?:\s+(?:no?\s*))?\s*'
                                if re.search(tr + sep + re.escape(num), pn):
                                    found_here = True
                                    break
                            if found_here:
                                pagina_inicio = i
...
onde NNN é o número da ÚLTIMA página que ainda faz parte da legislação (incluindo anexos)."""
                            ...
                            resp_fim = _chamar_llm(prompt_fim, logs, f'📄 Leitura pág {batch_end}')
                            if resp_fim:
                                try:
...
def busca_manual(params: dict) -> dict:
    ...
    def _truncar_inteligente(txt, limite=5000):
        if len(txt) <= limite:
            return txt
        metade = limite // 2
