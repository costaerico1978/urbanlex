        ...
        if 'pdf' in content_type or url.lower().endswith('.pdf') or '/download/' in url.lower():
            ...
            try:
                ...
                if numero_lei and numero_lei.strip() and total_pages > 20:
                    ...
                    preposicoes_ref = r'(?:da|na|pela|conforme|previsto|nos\s+termos|art(?:igo)?\.?\s+\d+.*?d[aeo]|§\s*\d+.*?d[aeo]|inciso.*?d[aeo])\s+'
                        ...
                        if tipo_lei:
                            ...
                            for tc in tipos_check:
                                ...
                                for m in re.finditer(padrao_tipo_num, pn):
                                    ...
                                    eh_referencia = bool(re.search(preposicoes_ref + r'$', trecho_antes))
...
onde NNN é o número da ÚLTIMA página que ainda faz parte da legislação (incluindo anexos)."""
        ...
        return {'url': url, 'texto': texto[:50000], 'nome': label, 'relevancia': relevancia}
    except Exception as e:
            ...
            if data_base:
                for delta in range(0, 11):  # data_publicação até 10 dias depois
                    ...
                    for ddg in ddg_results:
                        ...
                        if result:
                            # PÓS-FILTRO: verificar se o texto do DO contém a data da edição
                            # As primeiras 500 chars do PDF geralmente tem a data da edição
                            texto_inicio = result.get('texto', '')[:2000].lower()
                            ...
                            tem_data_certa = (
                                data_fmt1 in texto_inicio or
                                data_fmt2 in texto_inicio or
                                (dia_busca in texto_inicio and mes_busca in texto_inicio and ano_busca in texto_inicio)
                            )

                            if not tem_data_certa:
                                # Tentar achar qual data realmente tem no início
                                datas_achadas = re.findall(r'\d{1,2}\s+de\s+\w+\s+de\s+\d{4}', texto_inicio)
                                if not datas_achadas:
                                    datas_achadas = re.findall(r'\d{2}/\d{2}/\d{4}', texto_inicio)
