...
def busca_manual(params: dict) -> dict:
    ...
    # ══════════════════════════════════════════════════════════════════
    # Se não informou URL, tenta descobrir automaticamente
    if not url_diario and municipio:
        logs.append({'nivel': 'info', 'msg': f'🏛️ ETAPA 1: URL do Diário Oficial não informada — tentando descobrir automaticamente...'})
        busca_do = f'diário oficial "{municipio}" {estado} site oficial'
        ddg_do = _pesquisar_web(busca_do, logs, '🏛️ Descoberta DO', max_results=5)
        # Procurar domínios típicos de diários oficiais
        dominios_do = ['diariooficial', 'doweb', 'imprensaoficial', 'dom.', 'diariomunicipal', 'ioerj', 'ioesp']
        for r in ddg_do:
            ...
            if any(d in url_candidata for d in dominios_do) or 'diario' in url_candidata:
                ...
                logs.append({'nivel': 'ok', 'msg': f'🏛️ Diário Oficial descoberto: {url_diario}'})
                break
        if not url_diario:
    ...
    if url_diario:
        ...
        logs.append({'nivel': 'info', 'msg': '🏛️ ETAPA 1: Sem URL do Diário Oficial e sem município para descobrir (pulando)'})
    ...
    if municipio:
        ...
        if melhores:
            ...
            if ddg_results:
                snippet_text = ' | '.join([f"{d['titulo']}: {d['snippet']}" for d in ddg_results])
                if len(snippet_text) > 50:
                    textos_extraidos.append({'url': 'https://leismunicipais.com.br', 'texto': snippet_text[:3000], 'nome': '📖 LeisMunicipais (snippets)', 'relevancia': 0.3, '_fonte': 'leismunicipais'})
    ...
    if melhores:
        ...
        if ddg_results:
            snippet_text = ' | '.join([f"{d['titulo']}: {d['snippet']}" for d in ddg_results])
            if len(snippet_text) > 50:
                textos_extraidos.append({'url': '', 'texto': snippet_text[:3000], 'nome': '🔎 Web (snippets)', 'relevancia': 0.3, '_fonte': 'google'})
