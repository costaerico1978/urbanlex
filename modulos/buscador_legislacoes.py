            ...
            if resp_resultado:
                ...
                if acao.get('tipo') == 'ler_texto_inline':
                    ...
                    if texto_completo and len(texto_completo) < 5000:
                        ...
                        try:
                            _page_links = page.evaluate('''() => {
                                return Array.from(document.querySelectorAll('a[href]')).map(a => ({
                                    url: a.href,
                                    text: a.textContent.trim().substring(0, 150)
                                })).filter(l => l.text.length > 3 && !l.url.includes('javascript:'));
                            }''') or []
                            
                            # Filtrar links que mencionam o numero da lei ou 'ver', 'abrir', 'texto', 'integral'
                            num = (numero_lei or '').strip()
                            _links_relevantes = []
                            for _pl in _page_links:
                                ...
                                if num and num in _txt:
                                    _links_relevantes.append(_pl)
                                elif any(x in _txt for x in ['ver texto', 'texto integral', 'ver ato', 'abrir', 'visualizar', 'íntegra']):
                                    _links_relevantes.append(_pl)
                                elif num and num in _url:
                                    _links_relevantes.append(_pl)
                                elif '.pdf' in _url:
                                    _links_relevantes.append(_pl)
                            ...
                            if _links_relevantes:
                                ...
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 Seguindo link: {_best_link.get("text","")[:60]} → {_follow_url[:80]}'})
                                ...
                                try:
                                    ...
                                    for _a in _click_frame.query_selector_all('a[href]'):
                                        ...
                                        _a_text = (_a.text_content() or '').strip()
                                        if _follow_url.endswith(_a_href) or _a_href.endswith(_follow_url.split('/')[-1]):
                                            _link_el = _a
                                            break
                                        elif num and num in _a_text and tipo_lei.lower() in _a_text.lower():
                                            _link_el = _a
                                            break
                                    ...
                                    # Capturar texto da nova pagina (tentar frame e page)
                                    _follow_text = ''
                                    ...
                                    if len(_follow_text) > len(texto_completo) + 500:
                                        # Substituir o resultado anterior pelo texto mais completo
                                        resultados[-1] = {
