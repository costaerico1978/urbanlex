...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        with sync_playwright() as pw:
            ...
            # ════════════════════════════════════════════════════════════════
            # FASE 1: IA ANALISA A PÁGINA E DECIDE COMO PREENCHER
            # ════════════════════════════════════════════════════════════════
            ...
            # Capturar texto visível e links da página para contexto
            texto_visivel = ''
            links_nav = []
            try:
                texto_visivel = target_frame.evaluate('''() => {
                    const body = document.body;
                    if (!body) return '';
                    // Pegar texto dos menus, headers, labels — tudo que dá contexto
                    return body.innerText.substring(0, 3000);
                }''')
                links_nav = target_frame.evaluate('''() => {
                    const links = [];
                    document.querySelectorAll('a[href], [onclick]').forEach(a => {
                        const text = (a.textContent || '').trim().substring(0, 80);
                        const href = a.href || a.getAttribute('onclick') || '';
            ...
            prompt = f"""Voce esta olhando para uma pagina web e precisa preencher um formulario para buscar uma legislacao.

LEGISLACAO BUSCADA:
- Esfera: {'Municipal' if municipio else 'Federal'}
- Tipo: {tipo_lei or 'Lei Complementar'}
- Numero: {numero_lei or '?'}
- Ano: {ano or '?'}
- Municipio: {municipio or '?'}
- Data publicacao: {data_pub_fmt or 'desconhecida'}
- Data final busca: {data_final_str or 'desconhecida'}

PAGINA: {titulo_pagina} ({url_atual})

TEXTO VISIVEL:
{texto_visivel[:2500]}

LINKS E MENUS:
{json.dumps(links_nav[:25], ensure_ascii=False)}

CAMPOS INPUT:
{json.dumps(inputs_visiveis[:20], ensure_ascii=False)}

SELECTS (com options disponiveis):
{json.dumps(selects_visiveis[:10], ensure_ascii=False)}

TEXTAREAS:
{json.dumps(textareas_visiveis[:5], ensure_ascii=False)}

BOTOES:
{json.dumps(buttons_visiveis[:10], ensure_ascii=False)}

INSTRUCOES:

PASSO 1: ANALISE CADA CAMPO DO FORMULARIO
Para cada campo (input, select, textarea) visivel, responda mentalmente:
- O que este campo representa? (esfera, tipo de ato, numero, data, palavra-chave, etc.)
- Este campo e relevante para a busca desta legislacao?
- Se for SELECT: quais sao TODAS as options disponiveis (value e text)?
- Qual valor devo usar? Para select, qual option tem o TEXTO que corresponde ao dado da legislacao?

PASSO 2: DECIDA O PREENCHIMENTO
- Preencha APENAS os campos relevantes (minimo necessario - menos filtros = mais resultados)
- Para SELECT: use o TEXTO VISIVEL da option, NAO o value numerico.
  Exemplo: se as options sao Municipal, Federal, Estadual e a esfera e Municipal, use "Municipal".
  LEIA as options listadas na secao SELECTS acima e escolha pelo TEXTO que aparece na tela.
- Para campos de data: formato SEMPRE dd/mm/aaaa (ex: {data_pub_fmt or '16/01/2024'}).
  NUNCA use formato ISO yyyy-mm-dd.
- Se houver DATA INICIAL e DATA FINAL: use {data_pub_fmt or '16/01/2024'} como inicial
  e {data_final_str or '23/01/2024'} como final. NUNCA a mesma data nos dois campos.
- NUNCA confunda "numero da edicao" com "numero da lei"
- Se nao ha botao submit visivel, coloque "ENTER" no botao_submit

PASSO 3: RESPONDA COM JSON (inclua a analise dos campos)
{{
  "analise_campos": [
    {{"campo": "#id", "representa": "descricao do campo", "opcoes": "se select liste value:text", "valor_escolhido": "valor", "motivo": "por que este valor"}}
  ],
  "campos": [
    {{"seletor": "#id_ou_seletor_css", "tipo": "input|select|textarea", "valor": "valor a preencher"}}
  ],
  "botao_submit": "#seletor_do_botao ou ENTER",
  "confianca": 0.0 a 1.0,
  "estrategia": "resumo do que fez"
}}"""
            ...
            estrategia = instrucoes.get('estrategia', '')
            logs.append({'nivel': 'info', 'msg': f'{label}: IA: {estrategia}'})

            # Log analise_campos para debug
            analise = instrucoes.get('analise_campos', [])
            for ac in analise[:8]:
                campo_id = ac.get('campo', '?')
                representa = ac.get('representa', '?')
                opcoes = ac.get('opcoes', '')
                valor_esc = ac.get('valor_escolhido', '?')
                motivo = ac.get('motivo', '')
                logs.append({'nivel': 'info', 'msg': f'{label}: 🔍 {campo_id}: {representa} → valor={valor_esc} ({motivo[:60]})'})
                if opcoes:
                    logs.append({'nivel': 'info', 'msg': f'{label}:    options: {str(opcoes)[:120]}'})
            ...
            # 5) Preencher os campos (ordenar: selects primeiro, esfera antes de tipo)
            campos = instrucoes.get('campos', [])
            
            # Ordenar: selects com "esfera" primeiro, depois outros selects, depois inputs
            def _ordem_campo(c):
                sel_lower = (c.get('seletor', '') + c.get('valor', '')).lower()
                if c.get('tipo') == 'select' and 'esfera' in sel_lower:
                    return 0  # esfera primeiro
                elif c.get('tipo') == 'select':
                    return 1  # outros selects
                else:
                    return 2  # inputs por ultimo
            campos.sort(key=_ordem_campo)
            ...
            for campo in campos:
                ...
                # Auto-corrigir datas ISO → brasileiro (mas NÃO para input type="date" que exige ISO)
                _iso_date_match = re.match(r'^(\d{4})-(\d{2})-(\d{2})$', valor)
                if _iso_date_match:
                    # Verificar se o campo é type="date" (HTML5 exige yyyy-mm-dd)
                    _is_html_date = False
                    try:
                        _el_check = target_frame.query_selector(sel)
                        if _el_check:
                            _input_type = _el_check.get_attribute('type') or ''
                            _is_html_date = _input_type.lower() in ('date', 'datetime-local')
                    except Exception:
                        pass
                    if not _is_html_date:
                        valor = f'{_iso_date_match.group(3)}/{_iso_date_match.group(2)}/{_iso_date_match.group(1)}'
                        logs.append({'nivel': 'info', 'msg': f'{label}: 📅 Data convertida para formato BR: {valor}'})

                # Auto-corrigir datas BR → ISO para campos type="date" (HTML5 exige yyyy-mm-dd)
                _br_date_match = re.match(r'^(\d{2})/(\d{2})/(\d{4})$', valor)
                if _br_date_match:
                    try:
                        _el_check2 = target_frame.query_selector(sel)
                        if _el_check2:
                            _input_type2 = (_el_check2.get_attribute('type') or '').lower()
                            if _input_type2 in ('date', 'datetime-local'):
                                valor = f'{_br_date_match.group(3)}-{_br_date_match.group(2)}-{_br_date_match.group(1)}'
                                logs.append({'nivel': 'info', 'msg': f'{label}: 📅 Data convertida para ISO (campo type=date): {valor}'})
                    except Exception:
                        pass
                ...
                try:
                    ...
                    if tipo == 'select':
                        # Tentar por label (texto visivel) primeiro, fallback por value
                        try:
                            ...
                            try:
                                ...
                                logs.append({'nivel': 'aviso', 'msg': f'{label}: Select {sel} falhou: {str(e_sel)[:60]}'})
                                continue
                        # Disparar evento change (pode falhar se postback ASP ja iniciou)
                        try:
                            el.evaluate('el => el.dispatchEvent(new Event("change", {bubbles: true}))')
                        except Exception:
                            pass
                        # SEMPRE aguardar apos select (postback ASP pode recarregar campos)
                        _time.sleep(2)
                        try:
                            target_frame.wait_for_load_state('networkidle', timeout=5000)
                        except Exception:
                            pass
                    elif tipo == 'textarea':
                        el.click()
                        ...
                        el.click()
                        _time.sleep(0.2)
                        # Verificar se é type="date" HTML5
                        _el_type_check = ''
                        try:
                            _el_type_check = (el.get_attribute('type') or '').lower()
                        except Exception:
                            pass
                        
                        if _el_type_check in ('date', 'datetime-local'):
                            # Para type="date" HTML5: usar fill() com ISO (yyyy-mm-dd)
                            el.fill(valor)
                            logs.append({'nivel': 'info', 'msg': f'{label}: 📅 Campo type=date preenchido com fill(): {valor}'})
                        else:
                            # Limpar campo antes
            ...
            # DEBUG: Capturar valores reais do formulário antes de submeter
            try:
                _debug_vals = target_frame.evaluate('''() => {
                    const vals = {};
                    document.querySelectorAll('select, input, textarea').forEach(el => {
                        const id = el.id || el.name || '?';
                        if (el.type === 'hidden' && (id.startsWith('__') || id.length > 30)) return;
                        if (el.tagName === 'SELECT') {
                            const opt = el.options[el.selectedIndex];
                            vals[id] = { value: el.value, text: opt ? opt.text : '?' };
                        } else {
                            vals[id] = { val: el.value, type: el.type || 'text' };
                        }
                    });
                    return vals;
                }''')
                for _dk, _dv in _debug_vals.items():
                    if isinstance(_dv, dict):
                        if 'text' in _dv:
                            logs.append({'nivel': 'info', 'msg': f'{label}: 📋 FORM #{_dk} = "{_dv.get("value","")}" (texto: "{_dv.get("text","")}")'})
                        else:
                            _val = _dv.get('val', '')
                            _typ = _dv.get('type', 'text')
                            if _val:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 📋 FORM #{_dk} [{_typ}] = "{_val}"'})
                    elif _dv:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 📋 FORM #{_dk} = "{_dv}"'})
            except Exception as e_debug:
                logs.append({'nivel': 'info', 'msg': f'{label}: ⚠️ Debug form falhou: {str(e_debug)[:60]}'})
            ...
            # Se a IA disse "ENTER", pressionar Enter diretamente
            if botao_sel.upper().strip() == 'ENTER':
                try:
                    page.keyboard.press('Enter')
                    submeteu = True
                    logs.append({'nivel': 'info', 'msg': f'{label}: ⌨️ Submit via Enter'})
                except Exception:
                    pass
            elif botao_sel:
                try:
            ...
            # ════════════════════════════════════════════════════════════════
            # FASE 2: AGUARDAR E ANALISAR RESULTADOS COM IA
            # ════════════════════════════════════════════════════════════════
            try:
            ...
            # Extrair todo o conteúdo visível + links da página
            pagina_info = page.evaluate('''() => {
                const info = { links: [], texto: '', title: document.title || '' };
                ...
                document.querySelectorAll('a[href]').forEach(a => {
                    ...
                    if (href && text && href !== '#' && !href.startsWith('javascript:') && !href.startsWith('mailto:')) {
                        const isPdf = href.toLowerCase().includes('.pdf') || href.includes('/download/');
                        info.links.push({ url: href, text: text, pdf: isPdf });
                    }
                });

                // Texto principal da página (primeiros 5000 chars)
                const main = document.querySelector('main, .content, .resultado, #conteudo, #resultado, .panel, table') || document.body;
                info.texto = (main?.innerText || '').substring(0, 5000);
            ...
            texto_pagina = pagina_info.get('texto', '')
            links_pagina = pagina_info.get('links', [])

            # Filtrar links óbvios de lixo
            _lixo = ['facebook', 'twitter', 'whatsapp', 'linkedin', 'instagram', 'youtube',
                      'telegram', 'share', 'mailto:', 'javascript:', 'login', 'cadastr']
            links_limpos = [l for l in links_pagina
                           if not any(x in l.get('url','').lower() + l.get('text','').lower() for x in _lixo)]

            # Pedir pra IA analisar a página de resultados
            prompt_resultado = f"""Você submeteu um formulário buscando: {descricao}
Município: {municipio}

A PÁGINA DE RESULTADOS mostra:

TÍTULO: {pagina_info.get('title', '')}
URL: {url_resultado}

TEXTO VISÍVEL (primeiros 5000 chars):
{texto_pagina[:5000]}

LINKS NA PÁGINA ({len(links_limpos)} links):
{json.dumps(links_limpos[:30], ensure_ascii=False)}

ANALISE e responda com JSON:
{{
    "situacao": "encontrou_texto" | "encontrou_link" | "nao_encontrou" | "erro_pagina",
    "explicacao": "o que você vê na página",
    "acao": {{
        "tipo": "ler_texto_inline" | "clicar_link" | "baixar_pdf" | "nenhuma",
        "url": "URL do link/PDF a clicar (se aplicável)",
        "texto_lei": "texto da lei se já está inline na página (primeiros 2000 chars)"
    }}
}}

COMO DECIDIR:
1. Se o texto da lei ({tipo_lei} nº {num}) já aparece na página → "ler_texto_inline"
2. Se existe um link para ver/baixar a lei → "clicar_link" ou "baixar_pdf"
3. Se a página mostra "nenhum resultado" ou erro → "nao_encontrou"
4. Para PDFs e downloads, prefira "baixar_pdf"
5. Se há vários resultados, escolha o que MELHOR corresponde à legislação buscada"""

            resp_resultado = _chamar_llm(prompt_resultado, logs, f'{label} IA resultados', max_retries=1)

            if resp_resultado:
                try:
                    resp_resultado = re.sub(r'^```(?:json)?\s*|\s*```$', '', resp_resultado.strip())
                    info_res = json.loads(resp_resultado)
                except (json.JSONDecodeError, ValueError):
                    info_res = {'situacao': 'erro_pagina', 'acao': {'tipo': 'nenhuma'}}

                situacao = info_res.get('situacao', '')
                acao = info_res.get('acao', {})
                explicacao = info_res.get('explicacao', '')
                logs.append({'nivel': 'info', 'msg': f'{label}: IA: {situacao} — {explicacao[:100]}'})

                # ── AÇÃO: Texto já está inline na página ──
                # Dispara se IA pediu ler_texto_inline OU se situacao é encontrou_texto (IA pode errar o acao.tipo)
                if acao.get('tipo') == 'ler_texto_inline' or (situacao in ('encontrou_texto', 'encontrou_link') and acao.get('tipo') not in ('clicar_link', 'baixar_pdf')):
                    # Extrair texto completo da página (tentar frame e page)
                    texto_completo = ''
                    for _cap_frame in [target_frame, page]:
                        try:
                            _cap_text = _cap_frame.evaluate('() => document.body?.innerText || ""') or ''
                            if len(_cap_text) > len(texto_completo):
                                texto_completo = _cap_text
                        except Exception:
                            pass
                    if texto_completo and len(texto_completo) > 200:
                        resultados.append({
                            ...
                            'snippet': texto_completo[:300],
                            '_texto_direto': texto_completo,
                        })
                        logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto inline: {len(texto_completo)} chars'})

                    # Se texto inline é curto (listagem), tentar seguir links para o texto completo
                    if texto_completo and len(texto_completo) < 5000:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 📄 Texto curto ({len(texto_completo)} chars) — buscando links para texto completo...'})
                        try:
                            _link_extract_frame = target_frame if target_frame != page else page
                            _page_links = _link_extract_frame.evaluate('''() => {
                                return Array.from(document.querySelectorAll('a[href]')).map(a => {
                                    // Capturar TUDO sobre o link - texto, titulo, icone, contexto
                                    const text = a.textContent.trim().substring(0, 150);
                                    const title = (a.getAttribute('title') || '').trim();
                                    const ariaLabel = (a.getAttribute('aria-label') || '').trim();
                                    const alt = a.querySelector('img') ? (a.querySelector('img').getAttribute('alt') || '') : '';
                                    const imgSrc = a.querySelector('img') ? (a.querySelector('img').src || '') : '';
                                    const hasIcon = !!(a.querySelector('img, svg, i, span.icon, .fa, .glyphicon, [class*="icon"]'));
                                    const href = a.href || '';
                                    
                                    // Texto da linha/celula pai (contexto de tabela)
                                    let rowText = '';
                                    const tr = a.closest('tr');
                                    if (tr) rowText = tr.textContent.trim().substring(0, 300);
                                    
                                    // Texto da celula pai
                                    const td = a.closest('td, th');
                                    const cellHeader = td ? (td.getAttribute('data-label') || '') : '';
                                    
                                    // Classe do link (pode indicar download/arquivo)
                                ...
                                }).filter(l => !l.url.includes('javascript:') && l.url.length > 5);
                            }''') or []
                            
                            # Filtrar links relevantes com scoring inteligente
                            num = (numero_lei or '').strip()
                            _links_scored = []
                            for _pl in _page_links:
                                score = 0
                                _txt = (_pl.get('text','') or '').lower()
                                _url = (_pl.get('url','') or '').lower()
                                _title = (_pl.get('title','') or '').lower()
                                _alt = (_pl.get('alt','') or '').lower()
                                _aria = (_pl.get('ariaLabel','') or '').lower()
                                _row = (_pl.get('rowText','') or '').lower()
                                _cell = (_pl.get('cellHeader','') or '').lower()
                                _classes = (_pl.get('classes','') or '').lower()
                                _img_src = (_pl.get('imgSrc','') or '').lower()
                                _has_icon = _pl.get('hasIcon', False)
                                _all_meta = f'{_txt} {_title} {_alt} {_aria} {_cell} {_classes}'
                                
                                # Ignorar links de navegacao/menu/social
                                _skip_words = ['voltar', 'anterior', 'proximo', 'próximo', 'imprimir',
                                              'facebook', 'twitter', 'whatsapp', 'instagram', 'linkedin',
                                              'busca por', 'consultas', 'principal', 'créditos']
                                if any(s in _txt for s in _skip_words) and not _has_icon:
                                    continue
                                
                                # ── Score: link leva ao texto/PDF da legislacao ──
                                
                                # Link PDF ou download
                                if '.pdf' in _url or '/download/' in _url:
                                    score += 30
                                if any(x in _all_meta for x in ['download', 'baixar', 'arquivo', 'pdf']):
                                    score += 25
                                if any(x in _all_meta for x in ['ver texto', 'texto integral', 'ver ato',
                                                                 'visualizar', 'íntegra', 'inteiro teor',
                                                                 'abrir documento', 'ver documento']):
                                    score += 35
                                    
                                # Icone sem texto (tipico de botoes de download em tabelas)
                                if _has_icon and len(_txt) < 5:
                                    score += 15
                                    # Se o icone esta numa linha que menciona o numero da lei
                                    if num and num in _row:
                                        score += 40
                                    # Se a imagem sugere documento/arquivo
                                    if any(x in f'{_img_src} {_alt} {_classes}' for x in ['doc', 'file', 'pdf', 'download', 'arquivo', 'paper', 'page']):
                                        score += 20
                                
                                # Texto ou URL menciona numero da lei
                                if num and num in _txt:
                                    score += 20
                                if num and num in _url:
                                    score += 15
                                    
                                # Linha da tabela menciona o tipo + numero
                                if num and num in _row and tipo_lei.lower() in _row:
                                    score += 25
                                
                                # Celula "arquivo" ou similar
                                if any(x in _cell for x in ['arquivo', 'documento', 'download', 'pdf']):
                                    score += 20
                                
                                # Penalizar links ancora (#) e links para a mesma pagina
                                if _url.endswith('#') or _url.endswith('#/') or _url == '#':
                                    score -= 50
                                
                                if score > 0:
                                    _pl['_score'] = score
                                    _links_scored.append(_pl)
                            
                            # Ordenar por score e pegar o melhor
                            _links_scored.sort(key=lambda x: x.get('_score', 0), reverse=True)
                            _links_relevantes = _links_scored[:3]  # top 3
                            
                            if _links_relevantes:
                                 logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 {len(_links_relevantes)} link(s) candidato(s) (scores: {[l.get("_score",0) for l in _links_relevantes]})'})
                                 
                                 # ═══ ESTRATÉGIA DIRETA: buscar links na linha da tabela que contém a lei ═══
                                 _click_frame = target_frame if target_frame != page else page
                                 _got_full_text = False
                                 num = (numero_lei or '').strip()
                                 
                                 # Passo 1: Encontrar a linha da tabela com a lei
                                 _table_links = []  # links encontrados direto na tabela
                                 if num:
                                     _rows = _click_frame.query_selector_all('tr')
                                     for _row_el in _rows:
                                         _row_txt = _row_el.text_content() or ''
                                         if num not in _row_txt:
                                             continue
                                         # Linha menciona o número — coletar TODOS os links dela
                                         _row_links = _row_el.query_selector_all('a[href]')
                                         for _rl in _row_links:
                                             _rl_href = (_rl.get_attribute('href') or '').strip()
                                             _rl_text = (_rl.text_content() or '').strip()
                                             _rl_has_icon = bool(_rl.query_selector('img, svg, i'))
                                             _rl_title = (_rl.get_attribute('title') or '').strip()
                                             if not _rl_href or _rl_href == '#':
                                                 continue
                                             _table_links.append({
                                                 'el': _rl, 'href': _rl_href, 'text': _rl_text,
                                                 'has_icon': _rl_has_icon, 'title': _rl_title
                                             })
                                         if _table_links:
                                             break  # Usar primeira linha que tem links
                                 
                                 # Passo 2: Tentar links da tabela primeiro, depois os candidatos do scoring
                                 _attempts = []
                                 
                                 # Prioridade 1: links com ícone/imagem na linha da tabela (o botão de download)
                                 for _tl in _table_links:
                                     if _tl['has_icon'] or '.pdf' in _tl['href'].lower() or 'arquivo' in _tl['href'].lower() or 'download' in _tl['href'].lower():
                                         _attempts.append(('tabela_icone', _tl))
                                 
                                 # Prioridade 2: outros links da tabela
                                 for _tl in _table_links:
                                     entry = ('tabela_link', _tl)
                                     if entry not in _attempts:
                                         _attempts.append(entry)
                                 
                                 # Prioridade 3: candidatos do scoring (exceto links #)
                                 for _lr in _links_relevantes:
                                     _lr_url = _lr.get('url', '')
                                     if _lr_url.endswith('#') or _lr_url.endswith('#/'):
                                         continue  # Pular links ancora
                                     _attempts.append(('scoring', _lr))
                                 
                                 logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 {len(_attempts)} tentativa(s) de link para seguir'})
                                 
                                 for _att_idx, (_att_tipo, _att_data) in enumerate(_attempts):
                                     if _got_full_text:
                                         break
                                     
                                     try:
                                         if _att_tipo in ('tabela_icone', 'tabela_link'):
                                             _att_el = _att_data['el']
                                             _att_desc = _att_data.get('text','') or _att_data.get('title','') or '(ícone)'
                                             _att_href = _att_data.get('href', '')
                                             logs.append({'nivel': 'info', 'msg': f'{label}: 🎯 Tentativa {_att_idx+1} ({_att_tipo}): {_att_desc[:40]} → {_att_href[:60]}'})
                                         else:
                                             _att_el = None
                                             _att_desc = _att_data.get('text','') or _att_data.get('title','') or '(link)'
                                             _att_href = _att_data.get('url', '')
                                             logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 Tentativa {_att_idx+1} (scoring): {_att_desc[:40]} → {_att_href[:60]}'})
                                             # Buscar elemento pelo href
                                             for _a in _click_frame.query_selector_all('a[href]'):
                                                 _a_h = _a.get_attribute('href') or ''
                                                 if _att_href.endswith(_a_h) or _a_h.endswith(_att_href.split('/')[-1]):
                                                     _att_el = _a
                                                     break
                                         
                                         if not _att_el:
                                             logs.append({'nivel': 'info', 'msg': f'{label}: Elemento não encontrado, pulando...'})
                                             continue
                                         
                                         # ── Tentar 3 modos: download, popup, navegação ──
                                         _follow_text = ''
                                         
                                         # Modo 1: Download de arquivo
                                         try:
                                             with page.expect_download(timeout=8000) as _dl_info:
                                                 _att_el.click()
                                             _download = _dl_info.value
                                             import tempfile as _tempfile
                                             _dl_tmp = _tempfile.mktemp(suffix='.pdf')
                                             _download.save_as(_dl_tmp)
                                             logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 Download: {_download.suggested_filename}'})
                                             
                                             import fitz as _fitz_f
                                             _doc_f = _fitz_f.open(_dl_tmp)
                                             _n_p = _doc_f.page_count
                                             for _pg in _doc_f:
                                                 _follow_text += _pg.get_text() + '\n'
                                             _doc_f.close()
                                             try:
                                                 import os; os.remove(_dl_tmp)
                                             except Exception:
                                                 pass
                                             
                                             if len(_follow_text) > len(texto_completo) + 500:
                                                 resultados[-1] = {
                                                     'url': _att_href,
                                                     'titulo': f'{tipo_lei} nº {num} — PDF (BuscaFácil)',
                                                     'snippet': _follow_text[:300],
                                                     '_texto_direto': _follow_text,
                                                 }
                                                 logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ PDF completo: {len(_follow_text)} chars, {_n_p} págs'})
                                                 _got_full_text = True
                                             else:
                                                 logs.append({'nivel': 'info', 'msg': f'{label}: PDF pequeno ({len(_follow_text)} chars), tentando próximo...'})
                                             continue  # Proximo candidato se nao foi suficiente
                                         except Exception:
                                             pass  # Nao foi download
                                         
                                         # Modo 2: Popup / nova janela
                                         try:
                                             with page.expect_popup(timeout=5000) as _popup_info:
                                                 _att_el.click()
                                             _popup = _popup_info.value
                                             try:
                                                 _popup.wait_for_load_state('networkidle', timeout=10000)
                                             except Exception:
                                                 pass
                                             _time.sleep(2)
                                             _follow_text = _popup.evaluate('() => document.body?.innerText || ""') or ''
                                             _popup_url = _popup.url
                                             logs.append({'nivel': 'info', 'msg': f'{label}: 🪟 Popup aberto: {_popup_url[:60]} ({len(_follow_text)} chars)'})
                                             
                                             if len(_follow_text) > len(texto_completo) + 500:
                                                 resultados[-1] = {
                                                     'url': _popup_url,
                                                     'titulo': f'{tipo_lei} nº {num} — texto completo',
                                                     'snippet': _follow_text[:300],
                                                     '_texto_direto': _follow_text,
                                                 }
                                                 logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto completo via popup: {len(_follow_text)} chars'})
                                                 _got_full_text = True
                                             try:
                                                 _popup.close()
                                             except Exception:
                                         ...
                                         # Modo 3: Navegação normal (a pagina mudou)
                                         try:
                                         ...
                                         _follow_text = ''
                                         for _tf in [_click_frame, page]:
                                             try:
                                                 _ft = _tf.evaluate('() => document.body?.innerText || ""') or ''
                                                 if len(_ft) > len(_follow_text):
                                                     _follow_text = _ft
                                             except Exception:
                                                 pass
                                         ...
                                         if len(_follow_text) > len(texto_completo) + 500:
                                             ...
                                             logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto completo via nav: {len(_follow_text)} chars'})
                                             _got_full_text = True
                                         elif len(_follow_text) < 50:
                                             logs.append({'nivel': 'info', 'msg': f'{label}: Link retornou {len(_follow_text)} chars, tentando próximo...'})
                                             # Voltar à página de resultados para tentar próximo link
                                             try:
                                             ...
                                             logs.append({'nivel': 'info', 'msg': f'{label}: Link: {len(_follow_text)} chars (insuficiente, tentando próximo...)'})
                                     
                                     except Exception as e_att:
                                         logs.append({'nivel': 'info', 'msg': f'{label}: Tentativa {_att_idx+1} falhou: {str(e_att)[:60]}'})
                                 
                                 if not _got_full_text:
                                     logs.append({'nivel': 'info', 'msg': f'{label}: Nenhum link trouxe texto completo'})
                            else:
                                logs.append({'nivel': 'info', 'msg': f'{label}: Nenhum link relevante encontrado na listagem'})
                        except Exception as e_links:
                            logs.append({'nivel': 'info', 'msg': f'{label}: Erro buscando links: {str(e_links)[:60]}'})

                # ── AÇÃO: Clicar num link ou baixar PDF ──
                elif acao.get('tipo') in ('clicar_link', 'baixar_pdf'):
                    nav_url = acao.get('url', '')
                    if nav_url:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Acessando: {nav_url[:80]}'})
                        url_antes = page.url

                        # Tentar como download primeiro
                        baixou_pdf = False
                        if acao.get('tipo') == 'baixar_pdf' or '.pdf' in nav_url.lower() or '/download/' in nav_url.lower():
                            try:
                                with page.expect_download(timeout=30000) as dl_info:
                                    link_el = page.query_selector(f'a[href*="{nav_url.split("/")[-1]}"]')
                                    if link_el:
                                        link_el.click()
                                    else:
                                ...
                                logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 Download: {download.suggested_filename} ({dl_size}KB)'})

                                # Ler PDF e buscar a lei dentro
                                import fitz as _fitz_nav
                                doc_pdf = _fitz_nav.open(dl_path)
                                total_pags = len(doc_pdf)
                                logs.append({'nivel': 'info', 'msg': f'{label}: 📄 PDF: {total_pags} páginas'})

                                # Buscar número da lei no PDF
                                pagina_inicio = -1
                                for i in range(total_pags):
                                    page_text = doc_pdf[i].get_text()
                                    if num and num in page_text:
                                        # Verificar se é cabeçalho (tipo + número + data)
                                        ctx_lower = page_text.lower()
                                        if tipo_lower and tipo_lower in ctx_lower:
                                            pagina_inicio = i
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: 🎯 CABEÇALHO pág {i+1}'})
                                            break
                                        elif pagina_inicio < 0:
                                            pagina_inicio = i  # candidata

                                if pagina_inicio >= 0:
                                    # Extrair da página encontrada em diante (máx 100 págs)
                                    paginas_text = []
                                    for i in range(pagina_inicio, min(pagina_inicio + 100, total_pags)):
                                        paginas_text.append(doc_pdf[i].get_text())
                                    texto_lei = '\n'.join(paginas_text)

                                    resultados.append({
                                        'url': nav_url,
                                        'titulo': f'{tipo_lei} nº {num} — PDF pág {pagina_inicio+1}',
                                        'snippet': texto_lei[:300],
                                        '_texto_direto': texto_lei,
                                    })
                                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Lei extraída do PDF: {len(texto_lei)} chars (págs {pagina_inicio+1}-{min(pagina_inicio+100, total_pags)})'})
                                else:
                                    # Não achou a lei, mas extrair tudo do PDF
                                    all_text = '\n'.join(doc_pdf[i].get_text() for i in range(min(50, total_pags)))
                                    if len(all_text) > 200:
                                        resultados.append({
                                            'url': nav_url,
                                            'titulo': f'PDF: {download.suggested_filename}',
                                            'snippet': all_text[:300],
                                            '_texto_direto': all_text,
                                        })
                                        logs.append({'nivel': 'info', 'msg': f'{label}: PDF extraído (lei não localizada): {len(all_text)} chars'})
                                ...
                                logs.append({'nivel': 'info', 'msg': f'{label}: Download não disparou ({str(e_dl)[:60]}) — tentando como navegação...'})

                        # Se não baixou PDF, navegar normalmente
                        if not baixou_pdf:
                            try:
                                ...
                                try:
                                    ...
                                    logs.append({'nivel': 'aviso', 'msg': f'{label}: Não conseguiu abrir {nav_url[:60]}'})
                                    browser.close()
                                    return resultados
                            ...
                            # ════════════════════════════════════════════════
                            # FASE 3: IA ANALISA A PÁGINA DO RESULTADO
                            # ════════════════════════════════════════════════
                            texto_pagina2 = ''
                            try:
                                texto_pagina2 = page.inner_text('body') or ''
                            except Exception:
                                pass

                            if len(texto_pagina2) > 300:
                                # Verificar se é a lei ou precisa clicar em mais um link
                                prompt_fase3 = f"""Naveguei para uma página tentando encontrar: {descricao}

URL: {page.url}
TÍTULO: {page.title() or ''}

CONTEÚDO (primeiros 4000 chars):
{texto_pagina2[:4000]}

Esta página contém o texto da {tipo_lei or 'legislação'} nº {num}?

Responda com JSON:
{{
    "contem_lei": true/false,
    "explicacao": "breve",
    "texto_inicio": "primeiros 200 chars do texto da lei (se encontrou)"
}}"""
                                resp_fase3 = _chamar_llm(prompt_fase3, logs, f'{label} IA fase3', max_retries=1)
                                if resp_fase3:
                                    try:
                                        resp_fase3 = re.sub(r'^```(?:json)?\s*|\s*```$', '', resp_fase3.strip())
                                        info3 = json.loads(resp_fase3)
                                        if info3.get('contem_lei'):
                                            resultados.append({
                                                'url': nav_url,
                                                'titulo': f'{tipo_lei} nº {num}',
                                                'snippet': texto_pagina2[:300],
                                                '_texto_direto': texto_pagina2,
                                            })
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto da lei: {len(texto_pagina2)} chars'})
                                        else:
                                            logs.append({'nivel': 'info', 'msg': f'{label}: Página não contém a lei — {info3.get("explicacao","")}'})
                                    except (json.JSONDecodeError, ValueError):
                                        # Se não parseou, mas tem bastante texto, usar mesmo assim
                                        if len(texto_pagina2) > 1000:
                                            resultados.append({
                                                'url': nav_url,
                                                'titulo': f'Resultado: {page.title() or ""}',
                                                'snippet': texto_pagina2[:300],
                                                '_texto_direto': texto_pagina2,
                                            })
                                else:
                                    # IA não respondeu, mas a página tem conteúdo
                                    if len(texto_pagina2) > 1000:
                                        resultados.append({
                                            'url': nav_url,
                                            'titulo': f'Resultado: {page.title() or ""}',
                                ...
                                logs.append({'nivel': 'info', 'msg': f'{label}: Página com pouco conteúdo ({len(texto_pagina2)} chars)'})

                elif situacao in ('nao_encontrou', 'erro_pagina'):
                    logs.append({'nivel': 'aviso', 'msg': f'{label}: Nenhum resultado no formulario ({situacao})'})
                    
                    # ═══ RETRY SEM DATAS ═══
                    # Se tinha campos de data preenchidos, tentar novamente sem eles
                    campos_data = [c for c in campos if 'data' in c.get('seletor','').lower() or 'data' in c.get('label','').lower()]
                    campos_sem_data = [c for c in campos if c not in campos_data]
                    
                    if campos_data and campos_sem_data:
                        logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 Tentando novamente SEM campos de data...'})
                        try:
                            # Voltar ao formulario
                            page.goto(url_base, wait_until='networkidle', timeout=20000)
                            _time.sleep(2)
                            
                            # Re-detectar target_frame (pode ser iframe/frameset)
                            retry_frame = page
                            for fr in page.frames:
                                if fr != page.main_frame and fr.url and 'about:blank' not in fr.url:
                                    retry_frame = fr
                                    break
                            
                            # Preencher campos (sem data)
                            for campo_r in campos_sem_data:
                                sel_r = campo_r.get('seletor', '')
                                valor_r = str(campo_r.get('valor', ''))
                                tipo_r = campo_r.get('tipo', 'input')
                                if not sel_r or not valor_r:
                                    continue
                                
                                el_r = retry_frame.query_selector(sel_r)
                                if not el_r:
                                    for alt_r in [sel_r.replace('#',''), f'[id="{sel_r.lstrip("#")}"]', f'[name="{sel_r.lstrip("#")}"]']:
                                        el_r = retry_frame.query_selector(alt_r)
                                        if el_r:
                                            break
                                if not el_r:
                                    continue
                                ...
                                if tipo_r == 'select':
                                    try:
                                        el_r.select_option(label=valor_r)
                                    except Exception:
                                        try:
                                            el_r.select_option(value=valor_r)
                                        except Exception:
                                            continue
                                    try:
                                        el_r.evaluate('el => el.dispatchEvent(new Event("change", {bubbles: true}))')
                                    except Exception:
                                        pass
                                    _time.sleep(2)
                                    try:
                                        retry_frame.wait_for_load_state('networkidle', timeout=5000)
                                    except Exception:
                                        pass
                                else:
                                    el_r.click()
                                ...
                                logs.append({'nivel': 'info', 'msg': f'{label}: ✏️ Retry preencheu {sel_r} = "{valor_r}"'})
                            
                            _time.sleep(1)
                            
                            # Submeter
                            botao_r = instrucoes.get('botao_submit', '')
                            if botao_r.upper().strip() == 'ENTER':
                                page.keyboard.press('Enter')
                            elif botao_r:
                                btn_r = retry_frame.query_selector(botao_r)
                                if btn_r:
                                    btn_r.click()
                                else:
                                    page.keyboard.press('Enter')
                            ...
                            logs.append({'nivel': 'info', 'msg': f'{label}: 🖱️ Retry submit sem datas...'})
                            ...
                            # Capturar texto da pagina resultado
                            retry_text = retry_frame.evaluate('() => document.body ? document.body.innerText : ""') or ''
                            retry_url = page.url
                            
                            if 'nenhum documento' in retry_text.lower() or 'nenhum resultado' in retry_text.lower():
                                logs.append({'nivel': 'aviso', 'msg': f'{label}: Retry sem datas tambem nao encontrou'})
                            elif len(retry_text) > 500:
                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Retry sem datas: pagina com {len(retry_text)} chars'})
                                # Verificar se ha links para a legislacao
                                num = (numero_lei or '').strip()
                                if num and num in retry_text:
                                    # Buscar links na pagina de resultados
                                    retry_links = retry_frame.evaluate('''() => {
                                        return Array.from(document.querySelectorAll('a[href]')).map(a => ({
                                            texto: a.textContent.trim().substring(0, 100),
                                            href: a.href
                                        })).filter(l => l.texto.length > 5);
                                    }''') or []
                                    for rl in retry_links:
                                        if num in rl.get('texto',''):
                                            resultados.append({
                                                'url': rl.get('href', retry_url),
                                                'titulo': rl.get('texto', '')[:100],
                                                'snippet': rl.get('texto', '')[:200],
                                            })
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Retry encontrou: {rl.get("texto","")[:60]}'})
                                    if not resultados:
                                        # Nao achou link especifico mas pagina tem conteudo
                                        resultados.append({
                                            'url': retry_url,
                                            'titulo': f'{tipo_lei} no {numero_lei}',
                                            'snippet': retry_text[:300],
                                            '_texto_direto': retry_text,
                                        })
                        except Exception as e_retry:
                            logs.append({'nivel': 'aviso', 'msg': f'{label}: Retry sem datas falhou: {str(e_retry)[:80]}'})

            else:
                # IA não respondeu — fallback: se a página tem conteúdo significativo, usar
                if len(texto_pagina) > 2000 and num and num in texto_pagina:
                    resultados.append({
                        'url': url_resultado,
                        'titulo': f'{tipo_lei} nº {num} — conteúdo da página',
                        'snippet': texto_pagina[:300],
                        '_texto_direto': texto_pagina,
                    })
                    logs.append({'nivel': 'info', 'msg': f'{label}: Fallback — número {num} encontrado na página ({len(texto_pagina)} chars)'})
...
def _busca_manual_core(params, logs, fontes_status):
    ...
    if not data_descoberta or not tipo_inferido:
        query_descoberta = f'{tipo or "lei"} {numero} {ano} {municipio} {estado} data publicação'.strip()
...
Se não encontrar, deixe "". NÃO invente."""
            ...
            if resp_ia:
                try:
                    ...
                    if not data_descoberta and info.get('data_publicacao'):
                        ...
                        # Validar: se o ano informado existe, a data deve ter o mesmo ano
    ...
    for site in sites_prioritarios:
        ...
        # ── Diário Oficial: função dedicada (dia a dia, download PDF completo) ──
        if site['tipo_fonte'] == 'diario_oficial':
            # Se não tem data, tentar mais uma busca focada antes de desistir do DO
            data_para_do = data_descoberta or data_pub
            if not data_para_do and numero and ano:
                logs.append({'nivel': 'info', 'msg': f'{site["label"]}: Sem data — tentando descobrir com busca extra...'})
                tipo_q = tipo_inferido or tipo or 'lei'
                queries_data = [
                    f'"{tipo_q} {numero}" "{ano}" site:doweb OR site:diariooficial OR site:camara',
                    f'"{tipo_q} nº {numero}" {municipio} {ano}',
                ]
                for qd in queries_data:
                    snps = _pesquisar_web(qd.strip(), logs, '📅 Data DO', max_results=3)
                    if snps:
                        for s in snps:
                            txt = (s.get('titulo','') + ' ' + s.get('snippet','')).lower()
                            # Tentar extrair data por regex
                            meses_map = {'janeiro':'01','fevereiro':'02','março':'03','marco':'03','abril':'04',
                                         'maio':'05','junho':'06','julho':'07','agosto':'08','setembro':'09',
                                         'outubro':'10','novembro':'11','dezembro':'12'}
                            m = re.search(r'(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})', txt)
                            if m and m.group(2) in meses_map:
                                _yr = m.group(3)
                                if ano and _yr != str(ano):
                                    continue
                                data_para_do = f'{_yr}-{meses_map[m.group(2)]}-{m.group(1).zfill(2)}'
                                logs.append({'nivel': 'ok', 'msg': f'📅 Data encontrada: {data_para_do}'})
                                data_descoberta = data_para_do
                                break
                            m2 = re.search(r'(\d{2})/(\d{2})/(\d{4})', txt)
                            if m2:
                                _yr2 = m2.group(3)
                                if ano and _yr2 != str(ano):
                                    continue
                                data_para_do = f'{_yr2}-{m2.group(2)}-{m2.group(1)}'
                                logs.append({'nivel': 'ok', 'msg': f'📅 Data encontrada: {data_para_do}'})
                                data_descoberta = data_para_do
                                break
                    if data_para_do:
