...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        ...
        logs.append({'nivel': 'info', 'msg': f'{label}: Playwright não disponível — pulando navegação por formulário'})
        return []
    ...
    logs.append({'nivel': 'info', 'msg': f'{label}: 🌐 Abrindo página com navegador...'})
...
def _verificar_se_referencia(texto_norm: str, match_pos: int, match_text: str, tipo_lei: str, numero_lei: str) -> dict:
    ...
    if tipo_norm:
        # Procurar cabeçalhos de OUTROS tipos de legislação no início do documento (primeiros 600 chars)
        inicio_doc = texto_norm[:600]
