    ...
    for site in sites_prioritarios:
        ...
        if nav_results:
            ...
            def _prioridade_link(r):
                ...
                # Prioridade 0: URL contém o nome da legislação (ex: /Lei%20Complementar%20270)
                if num_busca and tipo_busca and num_busca in combined and any(w in combined for w in tipo_busca.split()):
                    return 0
                # Prioridade 1: URL contém o número da lei
                if num_busca and num_busca in combined:
                    return 1
                # Prioridade 2: Download de página específica (ex: download/6889/119)
                if re.search(r'/download/\d+/\d+', url_lower):
                    return 2
                # Prioridade 3: Link de visualização
                if '/ver/' in url_lower or 'visualizar' in titulo_lower:
                    return 3
                # Prioridade 4: Qualquer PDF/download
                if '.pdf' in url_lower or '/download/' in url_lower:
                    return 4
                # Prioridade 5: HTML viewer
                if '/ver-html/' in url_lower or 'visualizar html' in titulo_lower:
                    return 5
                # Prioridade 9: Resto
                return 9
