                    ...
                    if texto_completo and len(texto_completo) < 5000:
                        ...
                        try:
                            ...
                            if _links_relevantes:
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 {len(_links_relevantes)} link(s) candidato(s) encontrado(s)'})
                                _best_link = _links_relevantes[0]
                                _follow_url = _best_link.get('url', '')
                                _link_desc = _best_link.get('text','') or _best_link.get('title','') or _best_link.get('alt','') or '(ícone)'
                                _link_score = _best_link.get('_score', 0)
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🔗 Melhor link (score={_link_score}): {_link_desc[:50]} → {_follow_url[:80]}'})
                                
                                try:
                                    # Tentar clicar no link (mantém sessão/cookies/frame)
                                    _click_frame = target_frame if target_frame != page else page
                                    _link_el = None
                                    
                                    # Estratégia 1: Buscar por href exato
                                    for _a in _click_frame.query_selector_all('a[href]'):
                                        _a_href = _a.get_attribute('href') or ''
                                        if _follow_url.endswith(_a_href) or _a_href.endswith(_follow_url.split('/')[-1]):
                                            _link_el = _a
                                            break
                                    
                                    # Estratégia 2: Se não achou, buscar icone na linha com o numero
                                    if not _link_el and num:
                                        _rows = _click_frame.query_selector_all('tr')
                                        for _row_el in _rows:
                                            _row_txt = _row_el.text_content() or ''
                                            if num in _row_txt and tipo_lei.lower() in _row_txt.lower():
                                                # Achou a linha - buscar link com icone/imagem
                                                _row_links = _row_el.query_selector_all('a[href]')
                                                for _rl in _row_links:
                                                    _rl_text = (_rl.text_content() or '').strip()
                                                    _rl_has_icon = _rl.query_selector('img, svg, i')
                                                    _rl_href = (_rl.get_attribute('href') or '').lower()
                                                    # Preferir links com icone, PDF, ou download
                                                    if _rl_has_icon or '.pdf' in _rl_href or 'download' in _rl_href or 'arquivo' in _rl_href:
                                                        _link_el = _rl
                                                        _rl_href_str = _rl.get_attribute('href') or ''
                                                        logs.append({'nivel': 'info', 'msg': f'{label}: 🎯 Encontrou link na linha da lei: {_rl_href_str[:80]}'})
                                                        break
                                                if _link_el:
                                                    break
                                    
                                    if _link_el:
                                        # Tentar como download primeiro (pode ser PDF)
                                        _download_path = None
                                        try:
                                            ...
                                            _download_path = _dl_tmp
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: 📥 Download via link: {_download.suggested_filename}'})
                                        except Exception:
                                            # Nao foi download — é navegação normal
                                            _download_path = None
                                    else:
                                        page.goto(_follow_url, wait_until='networkidle', timeout=15000)
                                        _download_path = None
                                    
                                    if _download_path:
                                        # Extrair texto do PDF baixado
                                        try:
                                            import fitz as _fitz_follow
                                            _doc_follow = _fitz_follow.open(_download_path)
                                            _n_pgs = _doc_follow.page_count
                                            _follow_text = ''
                                            for _pg in _doc_follow:
                                                _follow_text += _pg.get_text() + '\n'
                                            _doc_follow.close()
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: 📄 PDF extraído: {len(_follow_text)} chars, {_n_pgs} págs'})
                                            
                                            if len(_follow_text) > len(texto_completo) + 500:
                                                resultados[-1] = {
                                                    'url': _follow_url,
                                                    'titulo': f'{tipo_lei} nº {num} — PDF completo (BuscaFácil)',
                                                    'snippet': _follow_text[:300],
                                                    '_texto_direto': _follow_text,
                                                }
                                                logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto completo via PDF: {len(_follow_text)} chars'})
                                            else:
                                                logs.append({'nivel': 'info', 'msg': f'{label}: PDF com pouco conteudo adicional ({len(_follow_text)} chars)'})
                                        except Exception as e_pdf_follow:
                                            logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Erro ao ler PDF: {str(e_pdf_follow)[:60]}'})
                                        finally:
                                            try:
                                                import os as _os_follow
                                                _os_follow.remove(_download_path)
                                            except Exception:
                                                pass
                                    else:
                                        # Navegação normal — capturar texto da pagina
                                        try:
                                            page.wait_for_load_state('networkidle', timeout=10000)
                                        except Exception:
                                            pass
                                        ...
                                        # Capturar texto (tentar frame e page)
                                        _follow_text = ''
                                        for _tf in [_click_frame, page]:
                                            try:
                                                ...
                                                pass
                                        _follow_url_final = page.url
                                        
                                        if len(_follow_text) > len(texto_completo) + 500:
                                            resultados[-1] = {
                                                'url': _follow_url_final,
                                                'titulo': f'{tipo_lei} nº {num} — texto completo',
                                                'snippet': _follow_text[:300],
                                                '_texto_direto': _follow_text,
                                            }
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Texto completo via link: {len(_follow_text)} chars'})
                                        elif 'nao permitida' in _follow_text.lower() or 'erro' in _follow_text[:200].lower():
                                            logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Link retornou erro/bloqueio'})
                                        else:
                                            logs.append({'nivel': 'info', 'msg': f'{label}: Link seguido mas sem mais conteudo ({len(_follow_text)} chars)'})
                                except Exception as e_follow:
                                    logs.append({'nivel': 'aviso', 'msg': f'{label}: ⚠️ Erro ao seguir link: {str(e_follow)[:60]}'})
                            else:
