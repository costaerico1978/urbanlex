...
def _navegar_formulario_com_ia(url_base: str, tipo_lei: str, numero_lei: str,
    ...
    try:
        with sync_playwright() as pw:
            ...
            # Bloqueio DEFINITIVO — desistir imediatamente
            ...
            # Cloudflare/verificação de segurança — AGUARDAR (pode resolver sozinha)
            ...
            if any(p in _body_lower or p in _titulo_lower for p in _cf_patterns):
                logs.append({'nivel': 'info', 'msg': f'{label}: ⏳ Verificação de segurança detectada — aguardando até 15s...'})
                _cf_resolveu = False
                for _cf_wait in range(15):
                    ...
                    try:
                        ...
                        # Se o conteúdo mudou e não tem mais os padrões CF → resolveu
                        ...
                        pass
                if not _cf_resolveu:
                    logs.append({'nivel': 'aviso', 'msg': f'{label}: ⏳ Verificação não resolveu em 15s — pulando'})
                    browser.close()
                    return []
...
- Se existe formulário com campo de data: use {data_pub_fmt or 'a data de publicação'}
