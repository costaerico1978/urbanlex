...
def _get_proxy_config():
    ...
    if proxy_url:
        try:
            ...
            # Formato Evomi: socks5://host:port:user:pass (sem @)
            _m = _re_proxy.match(r'^(socks5|socks4|http|https)://([^:@]+):(\d+):([^:]+):(.+)$', proxy_url)
            if _m:
                _scheme, _host, _port, _user, _pw = _m.groups()
                return {
                    'server': f'{_scheme}://{_host}:{_port}',
                    'username': _user,
                    'password': _pw,
                }
            # Formato padrão: http://user:pass@host:port
...
def _get_proxy_requests():
    """Retorna dict de proxies para requests/curl_cffi a partir das env vars."""
    ...
    if user and pw:
        ...
        p = _up(server)
        proxy_url = f'{p.scheme}://{user}:{pw}@{p.hostname}:{p.port}'
    else:
        proxy_url = server
...
def _buscar_leismunicipais_direto(municipio: str, estado: str, tipo: str, numero: str, ano: str, logs: list) -> list:
    ...
    Usa curl_cffi para impersonar TLS do Chrome e bypassar Cloudflare Bot Management.
    """
    from urllib.parse import quote_plus

    # curl_cffi impersona TLS fingerprint (JA3/JA4) do Chrome real —
    # bypassa Cloudflare Bot Management que detecta Python pelo TLS.
    # cloudscraper só muda headers, não funciona contra Bot Management.
    try:
        from curl_cffi import requests as cffi_requests
        session = cffi_requests.Session(impersonate='chrome120', trust_env=False)
        _cffi_proxies = _get_proxy_requests()
        logs.append({'nivel': 'info', 'msg': f'LeisMunicipais: usando curl_cffi (Chrome TLS fingerprint) proxy={"sim" if _cffi_proxies else "nao"}'})
    except ImportError:
        try:
            import cloudscraper
            session = cloudscraper.create_scraper(browser={'browser': 'chrome', 'platform': 'windows', 'mobile': False})
            session.trust_env = False
            logs.append({'nivel': 'info', 'msg': 'LeisMunicipais: curl_cffi não instalado — usando cloudscraper'})
        except ImportError:
            import requests
            session = requests.Session()
            session.trust_env = False
            logs.append({'nivel': 'aviso', 'msg': 'LeisMunicipais: sem curl_cffi nem cloudscraper — usando requests'})
    ...
    try:
        ...
        resp = session.get(url_busca, headers=headers, timeout=15, allow_redirects=True, proxies=(_cffi_proxies or {}))
    ...
    if not resultados and numero and municipio:
        ...
        if tipo_slug:
            ...
            try:
                resp = session.get(url_direta, headers=headers, timeout=15, allow_redirects=True, proxies=(_cffi_proxies or {}))
            ...
            if fs_result.get('encontrada') and fs_result.get('url'):
                ...
                if _html_lei:
                    ...
                    if _texto_lei and len(_texto_lei) > 200:
                        ...
                        # Extrair e baixar anexos S3
                        import time as _time_anx, os as _os_anx, requests as _req_anx
                        ...
                        for _a in _soup_lm.find_all('a', attrs={'data-s3-expires': True}):
                            ...
                            if not _href:
                                continue
                            try:
                                _r_anx = _req_anx.get(_href, timeout=60)
                                if _r_anx.status_code == 200:
                                    _ext = '.zip' if 'zip' in _href.lower() else '.pdf'
                                    _fname = f'lm_anexo_{len(_anexos)+1}{_ext}'
                                    _fpath = _os_anx.path.join(_pdf_dir_anx, _fname)
                                    open(_fpath, 'wb').write(_r_anx.content)
                                    _anexos.append({'url': f'/static/downloads/{_fname}', 'nome': _txt})
                                    logs.append({"nivel": "ok", "msg": f"📎 LeisMunicipais: anexo '{_txt}' baixado ({len(_r_anx.content)//1024}KB)"})
                                else:
                                    logs.append({"nivel": "aviso", "msg": f"📎 LeisMunicipais: anexo '{_txt}' status {_r_anx.status_code}"})
                            except Exception as _e_anx:
                                logs.append({"nivel": "aviso", "msg": f"📎 LeisMunicipais: erro ao baixar anexo: {str(_e_anx)[:60]}"})
                        if _anexos:
        ...
        with sync_playwright() as pw:
            launch_args = {
                ...
                'args': [
                    ...
                    # Log de rede — captura TCP/TLS/HTTP completo
...
def _consultar_site_somente_data(url_base: str, tipo_lei: str, numero_lei: str,
                                  ano: str, logs: list, label: str,
                                  municipio: str = '') -> str:
    """
    Versão leve para ETAPA 1B: preenche o formulário de busca via Playwright,
    lê o HTML do resultado e extrai apenas a data de publicação.
    NÃO tenta baixar o documento — evita cascade A½→J que quebra a sessão.
    Retorna data no formato 'YYYY-MM-DD' ou '' se não encontrar.
    """
    import re as _re_d
    try:
        from playwright.sync_api import sync_playwright as _spw_d
    except ImportError:
        return ''

    _proxy = _get_proxy_config()

    def _tentar(usar_proxy):
        try:
            with _spw_d() as pw:
                br_args = {'headless': True, 'args': ['--no-sandbox']}
                if usar_proxy and _proxy:
                    br_args['proxy'] = _proxy
                browser = pw.chromium.launch(**br_args)
                ctx = browser.new_context(
                ...
                # Detectar WAF / Cloudflare
                _html_check = ''
                try:
                    _html_check = page.content()
                except Exception:
                    pass
                _waf = ('just a moment' in _html_check.lower() or
                        'checking your browser' in _html_check.lower() or
                        'cf-browser-verification' in _html_check.lower())
                if _waf and not usar_proxy:
                    browser.close()
                    return None  # sinaliza: tentar com proxy

                # Preencher formulário: Esfera + Tipo + Número (sem datas)
                _campos = [
                    (['esfera', 'âmbito', 'esfera_'], 'Municipal', 'select'),
                    (['tipo de ato', 'tipodeato', 'tipo_ato', 'tipoato'], tipo_lei or '', 'select'),
                    (['nº do ato', 'numero do ato', 'nr do ato', 'nro do ato', 'núm', 'numero_ato', 'nroato', 'numato'], numero_lei or '', 'input'),
                ]
                for _labels_c, _val_c, _tipo_c in _campos:
                    if not _val_c:
                        continue
                    for _lc in _labels_c:
                        try:
                            if _tipo_c == 'select':
                                _sel = page.locator(f'select').filter(has_text=re.compile(_lc, re.I)).first
                                if _sel.count() == 0:
                                    _sel = page.get_by_label(_lc, exact=False).first
                                _sel.select_option(label=_val_c, timeout=3000)
                            else:
                                _inp = page.get_by_label(_lc, exact=False).first
                                _inp.fill(_val_c, timeout=3000)
                            break
                        except Exception:
                            continue
                ...
                # Submeter
                try:
                    _btn = page.get_by_role('button', name=re.compile(r'consultar|buscar|pesquisar|ok', re.I)).first
                    _btn.click(timeout=5000)
                    page.wait_for_load_state('domcontentloaded', timeout=10000)
                except Exception:
                    pass

                # Ler HTML do resultado e procurar data
                _html_result = ''
                try:
                    _html_result = page.content()
                except Exception:
                    pass
                # Texto visível da página
                _txt_result = ''
                try:
                    _txt_result = page.inner_text('body')[:5000]
                except Exception:
                    pass

                browser.close()

                # Verificar se resultado é para o número correto
                if numero_lei and numero_lei not in (_txt_result + _html_result):
                    return ''

                return _extrair_data_do_texto(_txt_result + ' ' + _html_result[:3000])
        except Exception as _e:
            logs.append({'nivel': 'info', 'msg': f'{label}: 1B rápido erro: {str(_e)[:60]}'})
            return ''

    # Tentar sem proxy, depois com proxy
    result = _tentar(usar_proxy=False)
    if result is None:
        logs.append({'nivel': 'aviso', 'msg': f'{label}: WAF — tentando com proxy...'})
        result = _tentar(usar_proxy=True)
    return result or ''
        ...
        with sync_playwright() as pw:
            ...
            if _bloqueado:
                ...
                if proxy_config:
                    ...
                    import tempfile as _tf_ctx
                    _har_path = _tf_ctx.mktemp(prefix='buscafacil_har_', suffix='.har')
                    ctx = browser.new_context(
                        viewport={'width': 1280, 'height': 900},
                        user_agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                        accept_downloads=True,
                        record_har_path=_har_path,
                    ...
                    if any(p in _proxy_body_lower or p in _proxy_titulo_lower for p in _cf_patterns_proxy):
                        logs.append({'nivel': 'info', 'msg': f'{label}: ⏳ Proxy: Cloudflare detectado — aguardando até 45s...'})
                        _proxy_cf_ok = False
                        for _pcf in range(45):
                            _time.sleep(1)
                            try:
                                # Checar se cf_clearance cookie apareceu (indica challenge resolvido)
                                _cookies_now = ctx.cookies()
                                _has_clearance = any(c['name'] == 'cf_clearance' for c in _cookies_now)
                                _pt = page.title() or ''
                                _pb = (page.inner_text('body') or '')[:500]
                                _cf_gone = not any(p in _pb.lower() or p in _pt.lower() for p in _cf_patterns_proxy)
                                if _cf_gone or _has_clearance:
                                    logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ Proxy: Cloudflare resolveu após {_pcf+1}s (clearance={_has_clearance})'})
                                    titulo_pagina = _pt
                                    body_text = _pb
                                    url_atual = page.url
                                    _proxy_cf_ok = True
                                ...
                                pass
                        if not _proxy_cf_ok:
                            # Cloudflare não resolveu em 20s no Playwright com proxy.
                            # Extrair os cookies da sessão (incluindo cf_clearance parcial)
                            # e tentar curl_cffi com esses cookies — pode ter sido resolvido
                            # mas sem que a página tenha mudado visivelmente ainda.
                            logs.append({'nivel': 'aviso', 'msg': f'{label}: ❌ Proxy: Cloudflare não resolveu em 20s — extraindo cookies para curl_cffi'})
                            try:
                                _all_cookies = ctx.cookies()
                                _cf_cookies = {c['name']: c['value'] for c in _all_cookies if 'cf_' in c['name'].lower() or 'cloudflare' in c['name'].lower()}
                                _all_cookie_dict = {c['name']: c['value'] for c in _all_cookies}
                                _pw_ua = ctx.evaluate_handle('navigator.userAgent') if False else 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
                                logs.append({'nivel': 'info', 'msg': f'{label}: 🍪 Cookies extraídos: {list(_cf_cookies.keys())} (total={len(_all_cookie_dict)})'})
                                if _cf_cookies:
                                    # Tentar curl_cffi com cookies da sessão Playwright
                                    try:
                                        from curl_cffi import requests as _cffi2
                                        _s2 = _cffi2.Session(impersonate='chrome120', trust_env=False)
                                        _p2 = _get_proxy_requests()
                                        _h2 = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                                               'Accept-Language': 'pt-BR,pt;q=0.9', 'Referer': 'https://leismunicipais.com.br/'}
                                        _r2 = _s2.get(url, headers=_h2, cookies=_all_cookie_dict, timeout=15,
                                                      allow_redirects=True, proxies=(_p2 or {}))
                                        _preview2 = _r2.text[:150].replace('\n',' ')
                                        _is_cf2 = any(p in _preview2.lower() for p in ['just a moment','checking your browser','challenge-platform'])
                                        logs.append({'nivel': 'info', 'msg': f'{label}: curl_cffi+cf_cookies: status={_r2.status_code} cf={_is_cf2} preview={repr(_preview2[:100])}'})
                                        if _r2.status_code == 200 and not _is_cf2 and len(_r2.text) > 500:
                                            logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ curl_cffi+cf_cookies funcionou!'})
                                            browser.close()
                                            # Retornar HTML para ser processado pelo fluxo normal
                                            return [{'_html_override': _r2.text, 'url': url}]
                                    except Exception as _e2:
                                        logs.append({'nivel': 'info', 'msg': f'{label}: curl_cffi+cf_cookies falhou: {str(_e2)[:60]}'})
                            except Exception as _ec:
                                logs.append({'nivel': 'info', 'msg': f'{label}: extração de cookies falhou: {str(_ec)[:60]}'})
                            browser.close()
                            return []
...
def _acessar_pagina(url: str, termos: str, headers: dict, logs: list, label: str, tipo_lei: str = '', numero_lei: str = '', ano: str = '') -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            # ── FALLBACK CLOUDSCRAPER para leismunicipais.com.br (Cloudflare) ──
            if resp.status_code == 403 and 'leismunicipais.com.br' in url:
                logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 leismunicipais 403 — tentando cloudscraper...'})
                try:
                    try:
                        from curl_cffi import requests as _cffi_mod
                        _scraper = _cffi_mod.Session(impersonate='chrome120', trust_env=False)
                        _scraper_is_cffi = True
                    except ImportError:
                        import cloudscraper as _cs
                        _scraper = _cs.create_scraper(browser={'browser': 'chrome', 'platform': 'windows', 'mobile': False})
                        _scraper_is_cffi = False
                    _lm_proxy = _get_proxy_requests()
                    logs.append({'nivel': 'info', 'msg': f'{label}: 🔄 leismunicipais 403 — curl_cffi+proxy (cffi={_scraper_is_cffi} proxy={"sim" if _lm_proxy else "nao"})'})
                    _cffi_kwargs = {'timeout': 20, 'allow_redirects': True}
                    if _lm_proxy:
                        _cffi_kwargs['proxies'] = _lm_proxy
                    elif not _scraper_is_cffi:
                        _scraper.trust_env = False
                    _cs_resp = _scraper.get(url, **_cffi_kwargs)
                    _cs_ct = _cs_resp.headers.get('content-type','')[:40]
                    _cs_url = _cs_resp.url[:80]
                    _cs_preview = _cs_resp.text[:120].replace('\n',' ') if _cs_resp.text else ''
                    if _cs_resp.status_code == 200 and len(_cs_resp.text) > 500:
                        logs.append({'nivel': 'ok', 'msg': f'{label}: ✅ cloudscraper {len(_cs_resp.text)}chars | url={_cs_url} | ct={_cs_ct} | preview={repr(_cs_preview)}'})
                        resp = _cs_resp  # usar resposta do cloudscraper no fluxo normal abaixo
                    else:
                        logs.append({'nivel': 'info', 'msg': f'{label}: cloudscraper retornou {_cs_resp.status_code} | ct={_cs_ct} | {len(_cs_resp.text)}chars | preview={repr(_cs_preview)}'})
                except ImportError:
                    logs.append({'nivel': 'info', 'msg': f'{label}: cloudscraper não instalado — usando Playwright'})
                except Exception as _e_cs:
                    logs.append({'nivel': 'info', 'msg': f'{label}: cloudscraper falhou: {str(_e_cs)[:100]}'})
...
def _busca_manual_core(params, logs, fontes_status):
    ...
    logs.append({'nivel': 'info', 'msg': f'🔍 ETAPA 1: parâmetros — tipo={tipo!r} numero={numero!r} ano={ano!r} municipio={municipio!r} estado={estado!r} data_info={data_pub!r}'})
...
Se não encontrar, deixe "". NÃO invente."""
                ...
                try:
                    ...
                    if not data_descoberta and info.get('data_publicacao'):
                        ...
                        if ano and _data_cand[:4] != str(ano):
                            ...
                            _data_ok = False
                        # Cruzar com snippets web: se algum snippet menciona a data de forma
                        # explícita (DD/MM/AAAA ou "DD de mês de AAAA"), prefere esse dado
                        # sobre o que a IA inferiu — a IA pode confundir citações com publicação
                        if _data_ok and snippets_web:
                            _meses_v = {'janeiro':'01','fevereiro':'02','março':'03','marco':'03',
                                        'abril':'04','maio':'05','junho':'06','julho':'07',
                                        'agosto':'08','setembro':'09','outubro':'10',
                                        'novembro':'11','dezembro':'12'}
                            _snippet_dates = []
                            for _sv in snippets_web[:5]:
                                _tv = (_sv.get('titulo','') + ' ' + _sv.get('snippet','')).lower()
                                _mv = re.search(r'(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})', _tv)
                                if _mv and _mv.group(2) in _meses_v and (not ano or _mv.group(3) == str(ano)):
                                    _snippet_dates.append(f'{_mv.group(3)}-{_meses_v[_mv.group(2)]}-{_mv.group(1).zfill(2)}')
                                _mv2 = re.search(r'(\d{2})/(\d{2})/(\d{4})', _tv)
                                if _mv2 and (not ano or _mv2.group(3) == str(ano)):
                                    _snippet_dates.append(f'{_mv2.group(3)}-{_mv2.group(2)}-{_mv2.group(1)}')
                            # Se snippets concordam numa data diferente da IA, descartar IA
                            if _snippet_dates:
                                _dominant = max(set(_snippet_dates), key=_snippet_dates.count)
                                if _dominant != _data_cand:
                                    logs.append({'nivel': 'aviso', 'msg': f'📅 IA sugere {_data_cand} mas snippets indicam {_dominant} — usando snippets'})
                                    _data_cand = _dominant
                        if _data_ok:
        ...
        # Fallback: regex nos snippets — só aceita data se o número da lei estiver no mesmo snippet
        if not data_descoberta and numero and snippets_web:
            ...
            for snp in snippets_web:
                ...
                # Só considera o snippet se o número da lei for mencionado
                _num_str = str(numero).lower()
                if _num_str not in txt:
                    continue
    ...
    # ══════════════════════════════════════════════════════════════════
    # ETAPA 1B: PRÉ-CONFIRMAÇÃO DE DATA (condicional)
    # Ativa quando: data exata desconhecida E há DO nas fontes prioritárias.
    # Navega antecipadamente nas fontes não-DO (sites com metadados estruturados)
    # e leismunicipais para confirmar a data antes de enviar o DO para varredura.
    # Fontes que já rodaram aqui são marcadas para não repetir na ETAPA 2.
    # ══════════════════════════════════════════════════════════════════
    _fontes_ja_processadas = set()  # índices de fontes_prioritarias já navegadas

    _tem_do = any(_eh_diario_oficial(f) for f in fontes_prioritarias)
    _data_confirmada = bool(data_descoberta)  # data com dia exato

    if not _data_confirmada and _tem_do:
        logs.append({'nivel': 'info', 'msg': '📅 ETAPA 1B: Data incerta + DO detectado — confirmando data em fontes de metadados...'})

        def _extrair_data_do_texto(texto: str) -> str:
            """Tenta extrair data de publicação de um texto via regex e LLM."""
            if not texto:
                return ''
            # Regex primeiro (mais rápido)
            meses_map = {'janeiro':'01','fevereiro':'02','março':'03','marco':'03',
                         'abril':'04','maio':'05','junho':'06','julho':'07',
                         'agosto':'08','setembro':'09','outubro':'10',
                         'novembro':'11','dezembro':'12'}
            for pat in [
                r'publicad[ao].*?(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})',
                r'data.*?(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})',
                ...
                if m:
                    ...
                    if mes_str in meses_map:
                        _a = m.group(3)
                        if ano and _a != str(ano):
                            continue
                        return f'{_a}-{meses_map[mes_str]}-{m.group(1).zfill(2)}'
            for pat2 in [r'(\d{2})/(\d{2})/(\d{4})', r'(\d{4})-(\d{2})-(\d{2})']:
                m2 = re.search(pat2, texto)
                if m2:
                    g = m2.groups()
                    if pat2.startswith(r'(\d{4})'):
                        _a, _m, _d = g
                    else:
                        _d, _m, _a = g
                    if ano and _a != str(ano):
        ...
        # 1B-A: Fontes prioritárias não-DO (sites com metadados estruturados)
        for _i1b, _furl1b in enumerate(fontes_prioritarias):
            if _eh_diario_oficial(_furl1b):
                continue  # DO pula — é exatamente o que queremos confirmar a data antes
            if not _furl1b.startswith('http'):
                _furl1b = 'https://' + _furl1b
            _dom1b = re.sub(r'https?://', '', _furl1b.rstrip('/')).split('/')[0]
            _lbl1b = f'{medalhas[_i1b]} Fonte {_i1b+1} ({_dom1b}) [1B]'
            logs.append({'nivel': 'info', 'msg': f'📅 1B: Consultando {_dom1b} para confirmar data...'})

            # 1B usa modo somente_metadados=True: preenche formulário, lê HTML do resultado,
            # extrai data — NÃO tenta baixar o documento (evita cascade A½→J que queima sessão)
            _data1b_achada = _consultar_site_somente_data(
                _furl1b, tipo_inferido or tipo, numero, ano,
                logs, _lbl1b, municipio=municipio
            )
            # Não marcar como processado — ETAPA 2 ainda precisa navegar para baixar
            # _fontes_ja_processadas.add(_i1b)  # removido intencionalmente

            if _data1b_achada:
                data_descoberta = _data1b_achada
                _data_confirmada = True
                logs.append({'nivel': 'ok', 'msg': f'📅 1B: Data confirmada em {_dom1b}: {data_descoberta}'})
            if _data_confirmada:
                break

        # 1B-B: LeisMunicipais (sempre tem metadados de data)
        if not _data_confirmada and municipio and numero:
            logs.append({'nivel': 'info', 'msg': '📅 1B: Consultando LeisMunicipais para confirmar data...'})
            _lm1b = _buscar_leismunicipais_direto(municipio, estado, tipo_inferido or tipo, numero, ano, logs)
            for _lm in _lm1b[:2]:
                _res_lm = _acessar_pagina(
                    _lm['url'], termos_busca, headers_http, logs, '📖 LeisMunicipais [1B]',
                    tipo_lei=tipo_inferido or tipo, numero_lei=numero, ano=ano
                )
                if _res_lm:
                    _data1b = _extrair_data_do_texto(
                        (_res_lm.get('titulo','') or '') + ' ' + (_res_lm.get('texto','') or '')[:2000]
                    )
                    if _data1b:
                        data_descoberta = _data1b
                        _data_confirmada = True
        ...
        # 1B-C: LLM sobre o texto encontrado nas fontes não-DO (se regex falhou)
        if not _data_confirmada and textos_extraidos:
            _txt_amostra = ' '.join(t.get('texto','')[:500] for t in textos_extraidos[:2])
            if _txt_amostra.strip():
                _prompt_data = f"""Extraia a data de publicação no Diário Oficial desta legislação:
...
Responda APENAS com JSON: {{"data_publicacao": "AAAA-MM-DD"}}
Se não encontrar data exata, retorne {{"data_publicacao": ""}}. NÃO invente."""
                _resp_data = _chamar_llm(_prompt_data, logs, '📅 1B LLM')
                if _resp_data:
                    try:
                        _resp_data = re.sub(r'^```(?:json)?\s*|\s*```$', '', _resp_data.strip())
                        _d1b = json.loads(_resp_data).get('data_publicacao', '')
                        if _d1b and (not ano or _d1b[:4] == str(ano)):
                            data_descoberta = _d1b
                            _data_confirmada = True
                            logs.append({'nivel': 'ok', 'msg': f'📅 1B LLM: Data confirmada: {data_descoberta}'})
                    except Exception:
                        pass

        if not _data_confirmada:
            logs.append({'nivel': 'aviso', 'msg': f'📅 1B: Data não confirmada — DO fará varredura a partir do início do ano {ano or "?"}'})
        
        # Validar data confirmada contra ano informado
        if data_descoberta and ano and data_descoberta[:4] != str(ano):
            logs.append({'nivel': 'aviso', 'msg': f'📅 1B: Data {data_descoberta} descartada (ano {data_descoberta[:4]} != {ano})'})
            data_descoberta = ''

    # ══════════════════════════════════════════════════════════════════
    # ETAPA 2: FONTES PRIORITÁRIAS — na ordem 1 → 2 → 3
    # (fontes já processadas na ETAPA 1B são puladas)
    # ══════════════════════════════════════════════════════════════════
    ...
    for site in sites_prioritarios:
        # Pular fontes não-DO já navegadas na ETAPA 1B
        if site['_idx'] in _fontes_ja_processadas and not _eh_diario_oficial(site['url']):
            logs.append({'nivel': 'info', 'msg': f'{site["label"]}: ⏭️ Já consultado na etapa 1B — pulando'})
            continue
            ...
            for nav_r in nav_results_sorted[:8]:
                ...
                if nav_r.get('pdf_path') and os.path.isfile(nav_r['pdf_path']):
                    try:
                        ...
                        if _confirmada_nav:
                            logs.append({'nivel': 'ok', 'msg': f'{site["label"]}: ✅ PDF pré-validado ({len(_paginas_rel)} pág(s) relevantes de {len(_paginas_all)}, {len(texto_pdf)} chars)'})
                        else:
                            logs.append({'nivel': 'ok', 'msg': f'{site["label"]}: ✅ PDF extraído ({len(_paginas_rel)} pág(s) de {len(_paginas_all)}, {len(texto_pdf)} chars)'})
                        ...
                        if len(texto_pdf) > 100:
                            textos_extraidos.append({
                                ...
                                'texto': texto_pdf,
                                'nome': site['label'],
                                'relevancia': 1.0 if _confirmada_nav else 0.9,
                                '_fonte': site['tipo_fonte'],
                                'pdf_path': nav_r['pdf_path'],
                                '_confirmada': _confirmada_nav,
                            })
                ...
                logs.append({'nivel': 'info', 'msg': f'{site["label"]}: 🌐 acessando: {nav_r["url"][:80]}'})
                ...
                if result:
                    ...
                    fontes_status.append({'nome': site['label'], 'url': nav_r['url'], 'encontrou': True})
                    encontrou_site = True
                    _rel = result.get('relevancia', 0)
                    logs.append({'nivel': 'ok', 'msg': f'{site["label"]}: ✅ Legislação encontrada! relevancia={_rel:.2f} chars={len(result.get("texto",""))}'})
                    break
                else:
                    ...
                    _motivo_descarte = 'HTTP/browser falhou (retornou None)'
                    logs.append({'nivel': 'aviso', 'msg': f'{site["label"]}: ❌ resultado descartado | url={nav_r.get("url","")[:60]} | motivo={_motivo_descarte}'})
