...
def _buscar_duckduckgo(query: str, headers: dict, logs: list, label: str, max_urls: int = 3) -> list:
    """
    Pesquisa no DuckDuckGo HTML (não bloqueia bots) e retorna lista de
    {'url', 'titulo', 'snippet'} encontrados.
    """
    import requests as req
    resultados = []
    try:
        ddg_url = f'https://html.duckduckgo.com/html/?q={req.utils.quote(query)}'
        resp = req.get(ddg_url, headers=headers, timeout=10)
        if resp.status_code != 200:
            logs.append({'nivel': 'aviso', 'msg': f'{label}: DuckDuckGo HTTP {resp.status_code}'})
            return []

        # Extrair links de resultado do DuckDuckGo HTML
        # Formato: <a rel="nofollow" class="result__a" href="URL">TITULO</a>
        links = re.findall(r'class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)</a>', resp.text, re.DOTALL)
        # Extrair snippets
        snippets = re.findall(r'class="result__snippet"[^>]*>(.*?)</(?:a|td|div)', resp.text, re.DOTALL)

        for i, (url, titulo) in enumerate(links[:max_urls]):
            ...
            resultados.append({'url': url, 'titulo': titulo_limpo, 'snippet': snip})

        logs.append({'nivel': 'ok' if resultados else 'aviso',
                     'msg': f'{label}: {len(resultados)} resultado(s) no DuckDuckGo'})
    except Exception as e:
        logs.append({'nivel': 'aviso', 'msg': f'{label}: erro DuckDuckGo: {str(e)[:100]}'})

    return resultados


def _acessar_pagina(url: str, termos: str, headers: dict, logs: list, label: str) -> Optional[dict]:
    """Acessa uma URL e retorna texto extraído com score de relevância."""
    import requests as req
    try:
        ...
        return {'url': url, 'texto': texto[:4000], 'nome': label, 'relevancia': relevancia}
    except Exception as e:
...
def busca_manual(params: dict) -> dict:
    ...
    if url_diario:
        ...
        ddg_results = _buscar_duckduckgo(busca_diario, headers_http, logs, '🏛️ Diário Oficial')
        encontrou_diario = False
        for ddg in ddg_results[:2]:
            result = _acessar_pagina(ddg['url'], termos_busca, headers_http, logs, '🏛️ Diário Oficial')
            if result and result['relevancia'] > 0.2:
                ...
                encontrou_diario = True
                break
        if not encontrou_diario:
            # Incluir snippets do DuckDuckGo como fallback
            if ddg_results:
                ...
                if len(snippet_text) > 50:
                    ...
                    logs.append({'nivel': 'info', 'msg': '🏛️ Usando snippets do DuckDuckGo como fonte parcial'})
                else:
    ...
    if urls_extras:
        ...
        for i, url_extra in enumerate(urls_extras[:5]):
            ...
            ddg_results = _buscar_duckduckgo(busca_site, headers_http, logs, nome)
            encontrou = False
            for ddg in ddg_results[:2]:
                result = _acessar_pagina(ddg['url'], termos_busca, headers_http, logs, nome)
                if result and result['relevancia'] > 0.2:
                    result['_fonte'] = 'site_informado'
                    textos_extraidos.append(result)
                    fontes_status.append({'nome': nome, 'url': ddg['url'], 'encontrou': True})
                    encontrou = True
                    break
            if not encontrou:
    ...
    # ══════════════════════════════════════════════════════════════════
    # ETAPA 3: LEISMUNICIPAIS.COM.BR (pesquisa via DuckDuckGo)
    # ══════════════════════════════════════════════════════════════════
    if municipio:
        ...
        ddg_results = _buscar_duckduckgo(busca_lm, headers_http, logs, '📖 LeisMunicipais')
        encontrou_lm = False
        for ddg in ddg_results[:2]:
            result = _acessar_pagina(ddg['url'], termos_busca, headers_http, logs, '📖 LeisMunicipais')
            if result and result['relevancia'] > 0.2:
                result['_fonte'] = 'leismunicipais'
                textos_extraidos.append(result)
                fontes_status.append({'nome': '📖 LeisMunicipais', 'url': ddg['url'], 'encontrou': True})
                encontrou_lm = True
                break
        if not encontrou_lm:
    ...
    # ══════════════════════════════════════════════════════════════════
    # ETAPA 4: BUSCA GERAL (DuckDuckGo sem filtro de site)
    # ══════════════════════════════════════════════════════════════════
    logs.append({'nivel': 'info', 'msg': f'🔎 ETAPA 4: Busca geral: "{query_google}"...'})
    ddg_results = _buscar_duckduckgo(query_google, headers_http, logs, '🔎 Web')
    encontrou_web = False
    for ddg in ddg_results[:3]:
        # Evitar duplicatas
        if any(t['url'] == ddg['url'] for t in textos_extraidos):
            continue
        result = _acessar_pagina(ddg['url'], termos_busca, headers_http, logs, '🔎 Web')
        if result and result['relevancia'] > 0.2:
            result['_fonte'] = 'google'
            textos_extraidos.append(result)
            fontes_status.append({'nome': '🔎 Web', 'url': ddg['url'], 'encontrou': True})
            encontrou_web = True
            break
    if not encontrou_web:
        # Usar snippets como fallback
        if ddg_results:
            snippet_text = ' | '.join([f"{d['titulo']}: {d['snippet']}" for d in ddg_results])
            if len(snippet_text) > 50:
                ...
                logs.append({'nivel': 'info', 'msg': '🔎 Usando snippets dos resultados como fonte parcial'})
            else:
