...
def executar_ciclo_completo(bridge_callback=None, disparado_por=None):
    logger.info("=== Ciclo de monitoramento v4.0 (por legislação) iniciado ===")
    inicio = datetime.now()
    ...
    try:
        ...
        cur.execute("""
            SELECT l.id, l.tipo_nome, l.numero, l.ano, l.ementa,
                   ...
                   l.data_inicio_monitoramento, l.ultima_verificacao_monitoramento,
                   m.url_diario, m.nome as mun_nome
            FROM legislacoes l
        ...
        for mun_key, mun_info in municipios_dict.items():
            ...
            try:
                ...
                if not SCRAPER_OK or not url_diario:
                    msg = "URL do diário não configurada" if not url_diario else "Scraper não disponível"
